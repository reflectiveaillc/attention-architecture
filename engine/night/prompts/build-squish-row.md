...
1. `web/site/games/squish-row/index.html` — the game (single self-contained file)
2. `web/site/games/squish-row/copy.md` — social captions
...
- **Conditioned path (the hook):** squish-toy speed-run videos where cramming every toy in a row as fast as possible is the dare. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A row of squishy toys appears. Press each in order from left to right before it reinflates. Speed up across five rounds; the reinflation timer shrinks. Score is your fastest clean sweep.
...
- Includes in order: `window.LOOP_GAME='squish-row'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: squish-row" when finished.
