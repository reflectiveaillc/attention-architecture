...
1. `web/site/games/caption-rest/index.html` — the game (single self-contained file)
2. `web/site/games/caption-rest/copy.md` — social captions
...
## The game: CAPTION REST

- **Conditioned path (the hook):** caption-writing anxiety (the blank caption field and the pressure to say it right). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A blank caption field with a softly blinking cursor; you don't type. Hold the screen and the 'right words' surface as a gentle fade — read them, then the post button stays disabled and the thought simply dissolves. The caption was enough; it never needed to publish.
...
- Includes in order: `window.LOOP_GAME='caption-rest'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: caption-rest" when finished.
