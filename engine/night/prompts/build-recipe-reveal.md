...
1. `web/site/games/recipe-reveal/index.html` — the game (single self-contained file)
2. `web/site/games/recipe-reveal/copy.md` — social captions
...
## The game: Recipe Reveal

- **Conditioned path (the hook):** cooking / recipe reveal videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Swipe ingredients into a bowl in the right order. The final swipe bakes the dish and reveals what you made; wrong order gives a comedic fail but still closes the loop.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 05-ease.
- **Video trick:** wait for the final dish — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='recipe-reveal'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'dishes'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Recipe Reveal — copy
## TikTok (3 captions ≤100 chars, hook-first, wait for the final dish angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: recipe-reveal" when finished.
