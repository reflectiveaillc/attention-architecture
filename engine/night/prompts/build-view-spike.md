...
1. `web/site/games/view-spike/index.html` — the game (single self-contained file)
2. `web/site/games/view-spike/copy.md` — social captions
...
- **Conditioned path (the hook):** live viewer counter flicker. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Your stream's viewer counter jitters up and down in real time; tap BOOST only on a rising tick to bank viewers — tap on a falling tick and they leave. Catch the wave or watch it crash.
...
- Includes in order: `window.LOOP_GAME='view-spike'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'peak viewers'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: view-spike" when finished.
