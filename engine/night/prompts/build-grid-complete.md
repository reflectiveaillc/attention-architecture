...
1. `web/site/games/grid-complete/index.html` — the game (single self-contained file)
2. `web/site/games/grid-complete/copy.md` — social captions
...
## The game: GRID COMPLETE

- **Conditioned path (the hook):** bingo / collection-tracker grids that keep one square empty forever. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 5x5 grid of soft-colored tiles sits on screen, most dim. Tap a tile to light it; tiles light in a hidden unlock order revealed one hint at a time. Fill all 25 and the grid ripples once and holds — done, no new tiles, no infinite expansion.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 02-novelty.
- **Video trick:** calm completion — every cell finally lit — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='grid-complete'` → loop-events.js → juice.js → share.js.
...
```
# GRID COMPLETE — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — every cell finally lit angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: grid-complete" when finished.
