...
1. `web/site/games/snap-to-grid/index.html` — the game (single self-contained file)
2. `web/site/games/snap-to-grid/copy.md` — social captions
...
## The game: SNAP TO GRID

- **Conditioned path (the hook):** oddly-satisfying restock and shelf-filling loops on TikTok. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Scattered tiles float on a canvas; drag each one to its ghost outline until it snaps perfectly into place. Every clean snap fires a soft click and the tile locks with a glimmer, building a complete orderly grid from chaos.
...
- Includes in order: `window.LOOP_GAME='snap-to-grid'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: snap-to-grid" when finished.
