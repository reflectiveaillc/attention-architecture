...
1. `web/site/games/frost-scratch/index.html` — the game (single self-contained file)
2. `web/site/games/frost-scratch/copy.md` — social captions
...
## The game: FROST SCRATCH

- **Conditioned path (the hook):** frost-window and fogged-glass scratching ASMR where revealing what is underneath gives the squeak-and-reveal payoff. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag across a frost-covered canvas to scratch away frost particles with a high-pitched scrape audio. The clear path widens as you drag faster or slower, pitch-shifting with speed. Uncover the full canvas to reveal a soft glow beneath and end the session.
...
- Includes in order: `window.LOOP_GAME='frost-scratch'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: frost-scratch" when finished.
