...
1. `web/site/games/haul-unpack/index.html` — the game (single self-contained file)
2. `web/site/games/haul-unpack/copy.md` — social captions
...
## The game: HAUL UNPACK

- **Conditioned path (the hook):** unboxing haul videos where each item is revealed with mounting anticipation. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A box sits on screen; swipe down to open the flap and an item reveals with a pop animation. Each item goes straight to a visible collection shelf. The box contains exactly 10 items, no random-rare gatekeeping. Open all 10 and the haul is complete.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 05-ease.
- **Video trick:** completion-satisfaction — the shelf full after the last item — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='haul-unpack'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'items'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# HAUL UNPACK — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the shelf full after the last item angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: haul-unpack" when finished.
