...
1. `web/site/games/safe-list/index.html` — the game (single self-contained file)
2. `web/site/games/safe-list/copy.md` — social captions
...
- **Conditioned path (the hook):** pinning saved posts as a security blanket — hoarding reassuring content to return to. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A blank list sits on screen with a soft prompt: "what do you know for certain right now?" Tap to add one soft tile at a time — each tile is a gentle color block, no words required. The list fills from the bottom. When you stop adding, tap CLOSE and the list folds away like a notebook. You made the list; you ended it.
- **Engine:** calm. **Circuits:** 05-ease, 08-investment, 06-zeigarnik.
- **Video trick:** calm completion — closing the notebook is the ending ritual — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='safe-list'` → loop-events.js → juice.js → share.js.
...
```
# SAFE LIST — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — closing the notebook is the ending ritual angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: safe-list" when finished.
