...
1. `web/site/games/slow-likes/index.html` — the game (single self-contained file)
2. `web/site/games/slow-likes/copy.md` — social captions
...
- **Conditioned path (the hook):** like-count refresh compulsive checking. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Likes arrive on your post one at a time, slowly, with no action required — just watch the heart fill and the number tick. The discipline is to observe without refreshing.
...
- Includes in order: `window.LOOP_GAME='slow-likes'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: slow-likes" when finished.
