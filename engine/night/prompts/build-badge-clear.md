...
1. `web/site/games/badge-clear/index.html` — the game (single self-contained file)
2. `web/site/games/badge-clear/copy.md` — social captions
...
## The game: Badge Clear

- **Conditioned path (the hook):** red notification badge clearing. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Notification badges stack up; tap to clear each one — but every clear spawns 1-3 new badges at random. Keep the badge at zero as long as you can.
...
- Includes in order: `window.LOOP_GAME='badge-clear'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: badge-clear" when finished.
