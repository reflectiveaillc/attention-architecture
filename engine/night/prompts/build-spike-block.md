...
1. `web/site/games/spike-block/index.html` — the game (single self-contained file)
2. `web/site/games/spike-block/copy.md` — social captions
...
## The game: SPIKE BLOCK

- **Conditioned path (the hook):** threat-rush edits where something flies at the camera. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Spikes shoot in from the right edge; tap to raise your shield at the last instant. Too early and you tire; too late and the spike gets through.
...
- Includes in order: `window.LOOP_GAME='spike-block'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: spike-block" when finished.
