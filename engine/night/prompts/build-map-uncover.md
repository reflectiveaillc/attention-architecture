...
1. `web/site/games/map-uncover/index.html` — the game (single self-contained file)
2. `web/site/games/map-uncover/copy.md` — social captions
...
## The game: Map Uncover

- **Conditioned path (the hook):** where-is-this / travel reveal videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag a flashlight across a dark map to reveal landmarks one by one. Find the hidden marked spot and the destination unlocks with a short payoff card.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 02-novelty.
- **Video trick:** reveal the location — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='map-uncover'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'spots'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Map Uncover — copy
## TikTok (3 captions ≤100 chars, hook-first, reveal the location angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: map-uncover" when finished.
