...
1. `web/site/games/rare-catch/index.html` — the game (single self-contained file)
2. `web/site/games/rare-catch/copy.md` — social captions
...
- **Conditioned path (the hook):** rare-find hunting content where creators tease the one item that completes a set. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Items fall fast across the screen; tap only the ones matching the “missing” highlight shown at the top to add them to your set tracker. Common items are decoys. Catch all 6 rare targets and the tracker fills to 100%. A fixed session of 90 seconds and exactly 6 rares hidden in the stream.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 04-loss.
- **Video trick:** rare reveal — the moment the missing piece appears — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='rare-catch'` → loop-events.js → juice.js → share.js.
...
```
# RARE CATCH — copy
## TikTok (3 captions ≤100 chars, hook-first, rare reveal — the moment the missing piece appears angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rare-catch" when finished.
