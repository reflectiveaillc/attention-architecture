...
1. `web/site/games/echo-sequence/index.html` — the game (single self-contained file)
2. `web/site/games/echo-sequence/copy.md` — social captions
...
## The game: ECHO SEQUENCE

- **Conditioned path (the hook):** audio-pattern loops — ASMR tap sequences and rhythmic repeat content that rewards sustained sensory attention. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A sequence of tones plays; tap the pads in the same order to echo it back. Each correct echo plays the sequence one note longer. The audio is the entire feedback loop — precise, predictable, and never random. Reach the full 12-note sequence and it resolves to a chord.
- **Engine:** calm. **Circuits:** 01-rpe, 05-ease, 06-zeigarnik.
- **Video trick:** participation-comment-bait — how far did you echo? — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='echo-sequence'` → loop-events.js → juice.js → share.js.
...
```
# ECHO SEQUENCE — copy
## TikTok (3 captions ≤100 chars, hook-first, participation-comment-bait — how far did you echo? angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: echo-sequence" when finished.
