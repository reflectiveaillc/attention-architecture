...
1. `web/site/games/nostalgia-name/index.html` — the game (single self-contained file)
2. `web/site/games/nostalgia-name/copy.md` — social captions
...
## The game: NOSTALGIA NAME

- **Conditioned path (the hook):** 'name it if you remember it' challenge posts that weaponize the recall reflex for shares. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A blurred or pixelated icon from a shared-era cultural object (game, toy, show) fades in slowly; tap the correct name from three choices before it fully reveals. Score is how early you tapped — the earlier the blur-stage, the more points.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 02-novelty.
- **Video trick:** ego bait — prove you remember faster than friends — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='nostalgia-name'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'pts'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# NOSTALGIA NAME — copy
## TikTok (3 captions ≤100 chars, hook-first, ego bait — prove you remember faster than friends angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: nostalgia-name" when finished.
