...
1. `web/site/games/volume-fade/index.html` — the game (single self-contained file)
2. `web/site/games/volume-fade/copy.md` — social captions
...
## The game: Volume Fade

- **Conditioned path (the hook):** autoplay audio — the sound that keeps the room from going quiet. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A low tone plays; hold to slowly lower the volume and dim the screen together. Release early and both creep back up. Hold through to silence and black and the session closes. You fade the stimulus out instead of letting it autoplay into the night.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 06-zeigarnik.
- **Video trick:** calm completion — fade the sound to nothing — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='volume-fade'` → loop-events.js → juice.js → share.js.
...
```
# Volume Fade — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — fade the sound to nothing angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: volume-fade" when finished.
