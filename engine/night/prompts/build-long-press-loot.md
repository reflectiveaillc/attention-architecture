...
1. `web/site/games/long-press-loot/index.html` — the game (single self-contained file)
2. `web/site/games/long-press-loot/copy.md` — social captions
...
## The game: Long Press Loot

- **Conditioned path (the hook):** long-press. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold your thumb on a glowing chest. A bar fills from green to red. Release inside the narrow gold zone to bank the loot; hold too long and the chest explodes. Each round the zone shrinks.
...
- Includes in order: `window.LOOP_GAME='long-press-loot'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'gems'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: long-press-loot" when finished.
