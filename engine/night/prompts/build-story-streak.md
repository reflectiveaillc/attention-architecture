...
1. `web/site/games/story-streak/index.html` — the game (single self-contained file)
2. `web/site/games/story-streak/copy.md` — social captions
...
## The game: Story Streak

- **Conditioned path (the hook):** swipe-up stories. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Swipe up through a deck of story cards; hit the hidden 'next story' rhythm without skipping the mark. The tempo accelerates and the gap shrinks. Miss once and the streak burns like a 24h story.
...
- Includes in order: `window.LOOP_GAME='story-streak'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'stories'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: story-streak" when finished.
