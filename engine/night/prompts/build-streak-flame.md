...
1. `web/site/games/streak-flame/index.html` — the game (single self-contained file)
2. `web/site/games/streak-flame/copy.md` — social captions
...
- **Mechanic:** A flame burns at the center. Swipe up to feed it once per beat. Miss a beat and the flame shrinks; three misses and it dies. The streak is visible, growing, and fragile.
...
- Includes in order: `window.LOOP_GAME='streak-flame'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: streak-flame" when finished.
