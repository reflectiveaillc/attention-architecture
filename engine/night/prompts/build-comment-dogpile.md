...
1. `web/site/games/comment-dogpile/index.html` — the game (single self-contained file)
2. `web/site/games/comment-dogpile/copy.md` — social captions
...
## The game: Comment Dogpile

- **Conditioned path (the hook):** heart-liking incoming comments. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Comments scroll up the post; tap each comment's heart before it scrolls off — every missed heart turns the crowd against you and speeds the scroll up.
...
- Includes in order: `window.LOOP_GAME='comment-dogpile'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'comments liked'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: comment-dogpile" when finished.
