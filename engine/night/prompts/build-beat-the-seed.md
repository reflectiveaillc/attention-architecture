...
1. `web/site/games/beat-the-seed/index.html` — the game (single self-contained file)
2. `web/site/games/beat-the-seed/copy.md` — social captions
...
## The game: Beat the Seed

- **Conditioned path (the hook):** replying to viral challenge posts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A dare-link plants a 'seed score' from the sender. You get one 60-second run to beat it. Win and you uproot the seed; lose and the seed stands. The seed travels with the link and dies after one reply.
...
- Includes in order: `window.LOOP_GAME='beat-the-seed'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: beat-the-seed" when finished.
