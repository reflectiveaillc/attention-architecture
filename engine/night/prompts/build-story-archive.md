...
1. `web/site/games/story-archive/index.html` — the game (single self-contained file)
2. `web/site/games/story-archive/copy.md` — social captions
...
- **Mechanic:** A story ring appears and completes without disappearing. Tap once to stamp it as a soft memory. The ring stays on screen and slowly forms a calm collage. Nothing expires.
...
- Includes in order: `window.LOOP_GAME='story-archive'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: story-archive" when finished.
