...
1. `web/site/games/grid-fill-perfect/index.html` — the game (single self-contained file)
2. `web/site/games/grid-fill-perfect/copy.md` — social captions
...
- **Conditioned path (the hook):** tetris-filling and inventory-management ASMR where one block always sticks out. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** An irregular grid has empty cells of varying shapes; drag puzzle-piece fragments into the empty spaces to fill the grid without gaps or overlaps. When the last piece locks in and the grid is complete, the entire surface glows and dims to black.
...
- Includes in order: `window.LOOP_GAME='grid-fill-perfect'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: grid-fill-perfect" when finished.
