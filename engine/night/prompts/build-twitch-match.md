...
1. `web/site/games/twitch-match/index.html` — the game (single self-contained file)
2. `web/site/games/twitch-match/copy.md` — social captions
...
## The game: TWITCH MATCH

- **Conditioned path (the hook):** fast Q&A and split-second comparison videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Two shapes flash side by side; tap when they match, do nothing when they differ. Decoy near-matches appear faster and faster until the round ends.
...
- Includes in order: `window.LOOP_GAME='twitch-match'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: twitch-match" when finished.
