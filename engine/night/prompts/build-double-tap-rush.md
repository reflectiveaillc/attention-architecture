...
1. `web/site/games/double-tap-rush/index.html` — the game (single self-contained file)
2. `web/site/games/double-tap-rush/copy.md` — social captions
...
## The game: DOUBLE-TAP RUSH

- **Conditioned path (the hook):** double-tap like (the IG muscle-memory double-tap that fires a heart animation). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A feed scrolls upward; photo cards stream past and you double-tap each one to drop a heart on it. The double-tap is the whole verb — clean doubles score, sloppy single-taps miss. Hearts pile up as a visible like-count you're racing to grow.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 05-ease.
- **Video trick:** completion-satisfaction — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='double-tap-rush'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'likes/min'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: double-tap-rush" when finished.
