...
1. `web/site/games/slow-the-loop/index.html` — the game (single self-contained file)
2. `web/site/games/slow-the-loop/copy.md` — social captions
...
## The game: Slow the Loop

- **Conditioned path (the hook):** seamless video loops that play forever with no exit cue. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A short looping animation plays; tap once per loop to lengthen its period. Each tap stretches the loop longer and dims the screen, until the loop period is many seconds long and the screen is black. You break the seamless loop by making it too slow to hold you.
- **Engine:** calm. **Circuits:** 01-rpe, 05-ease, 07-timing.
- **Video trick:** the feed that ends — the loop you slow until it stops — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='slow-the-loop'` → loop-events.js → juice.js → share.js.
...
```
# Slow the Loop — copy
## TikTok (3 captions ≤100 chars, hook-first, the feed that ends — the loop you slow until it stops angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: slow-the-loop" when finished.
