...
1. `web/site/games/draft-courage/index.html` — the game (single self-contained file)
2. `web/site/games/draft-courage/copy.md` — social captions
...
## The game: DRAFT COURAGE

- **Conditioned path (the hook):** posting anxiety + the drafts folder (the hover-before-post hesitation). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 'Post' button glows; press and hold to build courage — a meter rises. Release inside the green zone to actually post and bank it; release too early or too late and it dumps to Drafts and you score nothing. Each round the green zone shrinks.
- **Engine:** viral. **Circuits:** 04-loss, 06-zeigarnik, 07-timing.
...
- Includes in order: `window.LOOP_GAME='draft-courage'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'posts'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: draft-courage" when finished.
