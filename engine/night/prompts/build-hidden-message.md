...
1. `web/site/games/hidden-message/index.html` — the game (single self-contained file)
2. `web/site/games/hidden-message/copy.md` — social captions
...
## The game: Hidden Message

- **Conditioned path (the hook):** "read the caption to the end" / cryptic text posts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap scrambled letter tiles to flip them into the correct word order. The final sentence is the punchline; once it reads clean, the message is delivered and the loop closes.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe.
- **Video trick:** last-line payoff — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='hidden-message'` → loop-events.js → juice.js → share.js.
...
```
# Hidden Message — copy
## TikTok (3 captions ≤100 chars, hook-first, last-line payoff angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: hidden-message" when finished.
