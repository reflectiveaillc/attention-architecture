...
1. `web/site/games/signal-sort/index.html` — the game (single self-contained file)
2. `web/site/games/signal-sort/copy.md` — social captions
...
## The game: SIGNAL SORT

- **Conditioned path (the hook):** signal-vs-noise rabbit-holes — the thread identifying the one real pattern buried in a noisy dataset. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A stream of icons flows across the canvas; tap only the ones that match a hidden signal pattern. The noise icons are similar but subtly wrong — wrong rotation, wrong count of dots, wrong gap. Each correct tap adds a soft chime; each miss adds a subtle dissonant tone. 20 correct picks and the signal prints.
- **Engine:** calm. **Circuits:** 01-rpe, 05-ease, 06-zeigarnik.
- **Video trick:** play-along comment bait — can you spot the signal by round 5? — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='signal-sort'` → loop-events.js → juice.js → share.js.
...
```
# SIGNAL SORT — copy
## TikTok (3 captions ≤100 chars, hook-first, play-along comment bait — can you spot the signal by round 5? angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: signal-sort" when finished.
