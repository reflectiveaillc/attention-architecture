...
1. `web/site/games/flick-flip/index.html` — the game (single self-contained file)
2. `web/site/games/flick-flip/copy.md` — social captions
...
- **Conditioned path (the hook):** trending-challenge flips — the same challenge re-posts with the rules subtly inverted. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Flick tiles into the matching bin; every 5 seconds the bins silently swap labels — left was recycle, now left is trash. Quick adaptation to a rule that flips like a re-posted trend. One wrong bin ends it; the format-flip is the whole game.
- **Engine:** viral. **Circuits:** 02-novelty, 05-ease, 04-loss.
- **Video trick:** shock-honesty — 'the bins just swapped on me' is the clip — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='flick-flip'` → loop-events.js → juice.js → share.js.
...
```
# FLICK FLIP — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty — 'the bins just swapped on me' is the clip angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: flick-flip" when finished.
