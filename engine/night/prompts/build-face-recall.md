...
1. `web/site/games/face-recall/index.html` — the game (single self-contained file)
2. `web/site/games/face-recall/copy.md` — social captions
...
## The game: FACE RECALL

- **Conditioned path (the hook):** 'do you remember them?' celebrity throwback posts that farm recognition memory. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Three illustrated faces flash for 2 seconds; a lineup of six appears and the thumb must tap the three shown. Faces share features to confuse recognition. Score is correct picks; dare-link sends the lineup to friends.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 02-novelty.
- **Video trick:** ego bait — 'I got all three, think you can?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='face-recall'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'correct'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# FACE RECALL — copy
## TikTok (3 captions ≤100 chars, hook-first, ego bait — 'I got all three, think you can?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: face-recall" when finished.
