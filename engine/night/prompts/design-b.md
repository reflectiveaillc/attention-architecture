...
`engine/state/designs/batch-b.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 25 game designs.
...
SOCIAL STATUS AND COMPARISON: hijack rank anxiety — follower counts, like counts, 'seen' receipts, live viewer counters, comment dogpiles, ratio dynamics, duet/stitch remixing, leaderboard envy. Single-player games that SIMULATE or subvert these social pressures (no real multiplayer — dare links only).
