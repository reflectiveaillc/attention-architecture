...
1. `web/site/games/storm-clear/index.html` — the game (single self-contained file)
2. `web/site/games/storm-clear/copy.md` — social captions
...
## The game: STORM CLEAR

- **Conditioned path (the hook):** news-feed scanning for threats — the loop of checking to see if anything new is bad. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A storm of icon-sized threat words rains down the screen. Tap each to neutralize it — it dissolves into a soft ring. Clear them all before they pile up below the line. When the sky is empty a calm horizon fades in and your "cleared" count is your dare score. The anxiety discharge made finite and winnable.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 06-zeigarnik.
- **Video trick:** completion-satisfaction — the clear sky unlocking after the last word dissolves — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='storm-clear'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'cleared'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# STORM CLEAR — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the clear sky unlocking after the last word dissolves angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: storm-clear" when finished.
