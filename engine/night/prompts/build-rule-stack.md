...
1. `web/site/games/rule-stack/index.html` — the game (single self-contained file)
2. `web/site/games/rule-stack/copy.md` — social captions
...
- **Conditioned path (the hook):** feature-creep feeds — each scroll adds a new UI rule on top of the last. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Rules compound each round: round 1 'tap red', round 2 'tap red but not if it's big', round 3 'tap red small but only on the left'. Every round adds one rule to the stack — how deep can you hold the whole rule-set before you slip? The novelty is a thicker ruleset each beat.
- **Engine:** viral. **Circuits:** 02-novelty, 06-zeigarnik, 01-rpe.
- **Video trick:** participation-comment-bait — 'I held 6 rules, you?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='rule-stack'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'rules'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# RULE STACK — copy
## TikTok (3 captions ≤100 chars, hook-first, participation-comment-bait — 'I held 6 rules, you?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rule-stack" when finished.
