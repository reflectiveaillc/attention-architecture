...
1. `web/site/games/before-after-wipe/index.html` — the game (single self-contained file)
2. `web/site/games/before-after-wipe/copy.md` — social captions
...
## The game: Before After Wipe

- **Conditioned path (the hook):** before/after transformation wipe videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag a slider across a split image to reveal the after. Land the wipe inside the target reveal zone and the transformation payoff flashes; miss and you only get a tease.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 05-ease.
- **Video trick:** wait for the transformation — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='before-after-wipe'` → loop-events.js → juice.js → share.js.
...
```
# Before After Wipe — copy
## TikTok (3 captions ≤100 chars, hook-first, wait for the transformation angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: before-after-wipe" when finished.
