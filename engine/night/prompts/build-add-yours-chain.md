...
1. `web/site/games/add-yours-chain/index.html` — the game (single self-contained file)
2. `web/site/games/add-yours-chain/copy.md` — social captions
...
- **Conditioned path (the hook):** 'Add Yours' sticker prompt chain. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A prompt sticker names a theme; thumbnail frames scroll past fast — tap the frame that matches the theme to append it to the public chain. Wrong frame and the chain breaks.
...
- Includes in order: `window.LOOP_GAME='add-yours-chain'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: add-yours-chain" when finished.
