...
1. `web/site/games/code-breaker/index.html` — the game (single self-contained file)
2. `web/site/games/code-breaker/copy.md` — social captions
...
- **Conditioned path (the hook):** decode this audio/text" puzzle posts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap digits on a keypad to crack a short code. Each correct digit reveals one word of the hidden message; the final word resolves the whole clue.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe.
- **Video trick:** the last word changes everything — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='code-breaker'` → loop-events.js → juice.js → share.js.
...
```
# Code Breaker — copy
## TikTok (3 captions ≤100 chars, hook-first, the last word changes everything angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: code-breaker" when finished.
