...
1. `web/site/games/knockout-dare/index.html` — the game (single self-contained file)
2. `web/site/games/knockout-dare/copy.md` — social captions
...
## The game: Knockout Dare

- **Conditioned path (the hook):** elimination-style live battles. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A dare-link sends one tough round. Both players get exactly one attempt. Higher score survives; lower score is knocked out. The dare auto-expires once both have played, so the result is final.
...
- Includes in order: `window.LOOP_GAME='knockout-dare'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'survived'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: knockout-dare" when finished.
