...
1. `web/site/games/sort-rush/index.html` — the game (single self-contained file)
2. `web/site/games/sort-rush/copy.md` — social captions
...
- **Conditioned path (the hook):** sorting and categorizing ASMR that speeds up until items fall off the edge. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Items with labels (color, shape, size) pour in from the top; swipe each one left or right into the correct bin before the lane clogs. Wrong sorts cost a second; a clean run chains into a speed multiplier. Dare-share your sort streak.
...
- Includes in order: `window.LOOP_GAME='sort-rush'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sort-rush" when finished.
