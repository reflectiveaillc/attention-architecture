...
1. `web/site/games/ink-fill/index.html` — the game (single self-contained file)
2. `web/site/games/ink-fill/copy.md` — social captions
...
- **Conditioned path (the hook):** paint-fill and ink-drip ASMR where the color always stops before the last corner. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** An outlined shape sits empty; hold your thumb to let ink spread from your touch point and flood-fill the outline. Lift too early and a gap remains; hold until ink reaches every edge and the outline dissolves into a clean solid shape.
...
- Includes in order: `window.LOOP_GAME='ink-fill'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: ink-fill" when finished.
