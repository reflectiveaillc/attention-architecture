...
1. `web/site/games/stack-spell/index.html` — the game (single self-contained file)
2. `web/site/games/stack-spell/copy.md` — social captions
...
## The game: Stack Spell

- **Conditioned path (the hook):** building to a reveal" process videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap to stack blocks; each block adds a letter to a hidden word. The tower spells the answer that resolves the prompt at the top and the payoff lands.
- **Engine:** viral. **Circuits:** 01-rpe, 08-investment, 06-zeigarnik.
- **Video trick:** wait for the final word — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='stack-spell'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'letters'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Stack Spell — copy
## TikTok (3 captions ≤100 chars, hook-first, wait for the final word angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: stack-spell" when finished.
