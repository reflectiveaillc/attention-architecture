...
1. `web/site/games/pattern-break/index.html` — the game (single self-contained file)
2. `web/site/games/pattern-break/copy.md` — social captions
...
## The game: PATTERN BREAK

- **Conditioned path (the hook):** the 'what changed?' edit-bait clip — same clip, one frame altered, spot it. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A pattern flashes, then repeats with one element changed — tap the changed element. But what counts as 'the pattern' rotates each round: color change, then position change, then size change. Re-orient your attention every 5s, exactly the orienting response the FYP farms. Ends at a miss.
- **Engine:** viral. **Circuits:** 02-novelty, 06-zeigarnik, 01-rpe.
- **Video trick:** play-along comment bait — 'did you catch round 9?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='pattern-break'` → loop-events.js → juice.js → share.js.
...
```
# PATTERN BREAK — copy
## TikTok (3 captions ≤100 chars, hook-first, play-along comment bait — 'did you catch round 9?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pattern-break" when finished.
