...
1. `web/site/games/level-it/index.html` — the game (single self-contained file)
2. `web/site/games/level-it/copy.md` — social captions
...
- **Conditioned path (the hook):** slightly-tilted photo and crooked-shelf videos that halt before they're fixed. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A framed picture or object sits at a slight angle on the wall; drag the rotation dial slowly until the bubble in the spirit-level indicator settles dead center. Hold it level for two seconds and it clicks into place and hangs straight.
...
- Includes in order: `window.LOOP_GAME='level-it'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: level-it" when finished.
