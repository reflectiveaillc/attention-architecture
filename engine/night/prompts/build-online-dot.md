...
1. `web/site/games/online-dot/index.html` — the game (single self-contained file)
2. `web/site/games/online-dot/copy.md` — social captions
...
- **Conditioned path (the hook):** 'someone is online' green dot. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Green dots appear and fade across a grid of profiles. Tap only the dot that stays solid for a full second. Fake dots flicker to exploit the green-dot impulse.
...
- Includes in order: `window.LOOP_GAME='online-dot'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: online-dot" when finished.
