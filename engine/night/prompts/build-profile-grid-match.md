...
1. `web/site/games/profile-grid-match/index.html` — the game (single self-contained file)
2. `web/site/games/profile-grid-match/copy.md` — social captions
...
## The game: Profile Grid Match

- **Conditioned path (the hook):** profile grid browsing. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 3x3 grid of colored tiles flashes like a profile page. Tap the tile that changed. As rounds speed up, the grid gains 'posts' that distract from the changed one.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 03-social.
- **Video trick:** play-along comment bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='profile-grid-match'` → loop-events.js → juice.js → share.js.
...
```
# Profile Grid Match — copy
## TikTok (3 captions ≤100 chars, hook-first, play-along comment bait angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: profile-grid-match" when finished.
