...
1. `web/site/games/shadow-match/index.html` — the game (single self-contained file)
2. `web/site/games/shadow-match/copy.md` — social captions
...
## The game: SHADOW MATCH

- **Conditioned path (the hook):** 'guess the silhouette' nostalgia clips that bait the recognition snap with partial cues. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A silhouette flashes for one second; six choices appear and the thumb taps the match. Silhouettes get more ambiguous each round and choices more similar. Streak ends on a miss — the brain\'s recognition memory exercised at real speed.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 06-zeigarnik.
- **Video trick:** play-along comment bait — 'did you get it before the reveal?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='shadow-match'` → loop-events.js → juice.js → share.js.
...
```
# SHADOW MATCH — copy
## TikTok (3 captions ≤100 chars, hook-first, play-along comment bait — 'did you get it before the reveal?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: shadow-match" when finished.
