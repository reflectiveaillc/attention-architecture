...
1. `web/site/games/ink-drop/index.html` — the game (single self-contained file)
2. `web/site/games/ink-drop/copy.md` — social captions
...
- **Conditioned path (the hook):** ink-in-water ASMR where a slow bloom of color spreading through liquid is the entire satisfaction. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap anywhere on a water surface to drop a bead of ink; it blooms outward in procedural fluid simulation with a soft hiss. Each tap adds a new color that bleeds into the last. Ten drops create a full-canvas tie-dye — then the animation slowly stills.
...
- Includes in order: `window.LOOP_GAME='ink-drop'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: ink-drop" when finished.
