...
1. `web/site/games/pull-to-pop/index.html` — the game (single self-contained file)
2. `web/site/games/pull-to-pop/copy.md` — social captions
...
- **Mechanic:** Drag the feed down like a pull-to-refresh, then release at the exact elastic snap. Too early = limp reload; too late = the bubble bursts and you lose. Each clean pop spawns a bigger reward bubble.
...
- Includes in order: `window.LOOP_GAME='pull-to-pop'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pull-to-pop" when finished.
