...
1. `web/site/games/color-set/index.html` — the game (single self-contained file)
2. `web/site/games/color-set/copy.md` — social captions
...
- **Conditioned path (the hook):** palette-collection content where creators unbox every shade of a makeup line. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Swatches drift across the canvas; tap each one to add it to a growing palette row. The palette has exactly 16 slots — one per hue in the spectrum. Fill the row left to right and the completed rainbow palette glows softly and holds still.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 02-novelty.
- **Video trick:** calm completion — the full spectrum, side by side — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='color-set'` → loop-events.js → juice.js → share.js.
...
```
# COLOR SET — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the full spectrum, side by side angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: color-set" when finished.
