...
`engine/state/designs/cat-memory.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "memory"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: memory · label: Memory & Recall · engine_lean: viral
trait: recall-driven; nostalgia and 'remember this?'
algo_hook: fed nostalgia bait and 'only 90s kids remember' hooks that farm the recall reflex
wean: a real memory test — sequence, match, recall — that exercises the faculty instead of merely baiting it
mechanics: sequence recall, match pairs, spot-the-change, working-memory span, nostalgia recall
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: memory x20" when finished.
