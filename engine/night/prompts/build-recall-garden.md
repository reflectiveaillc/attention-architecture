...
1. `web/site/games/recall-garden/index.html` — the game (single self-contained file)
2. `web/site/games/recall-garden/copy.md` — social captions
...
## The game: RECALL GARDEN

- **Conditioned path (the hook):** 'on this day' memory-bait posts that pull the nostalgia reflex into a feed loop. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A soft sequence of illustrated memory-motes (shapes, colors, small icons) floats past one at a time; after the last one, the thumb gently taps each that appeared, in any order. No timer, no miss penalty — the session ends when all are acknowledged and a garden tile blooms.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** calm completion — the recall loop that resolves instead of repeating — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='recall-garden'` → loop-events.js → juice.js → share.js.
...
```
# RECALL GARDEN — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the recall loop that resolves instead of repeating angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: recall-garden" when finished.
