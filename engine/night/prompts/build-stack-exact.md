...
1. `web/site/games/stack-exact/index.html` — the game (single self-contained file)
2. `web/site/games/stack-exact/copy.md` — social captions
...
## The game: STACK EXACT

- **Conditioned path (the hook):** block-stacking and book-stacking videos where the pile always wobbles. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Blocks drop one at a time from above; tap at the precise moment each block aligns with the one below to stack it flush. A perfect stack accumulates without wobble; at ten blocks the tower glows and the session completes.
...
- Includes in order: `window.LOOP_GAME='stack-exact'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: stack-exact" when finished.
