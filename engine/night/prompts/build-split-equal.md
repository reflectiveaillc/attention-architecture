...
1. `web/site/games/split-equal/index.html` — the game (single self-contained file)
2. `web/site/games/split-equal/copy.md` — social captions
...
## The game: Split Equal

- **Conditioned path (the hook):** satisfying equal-split / portion-perfect content that rewards flawless division. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A line travels across the screen; tap to divide it into equal segments. Each round demands more cuts, and the variance from equal is your score — lower is better.
...
- Includes in order: `window.LOOP_GAME='split-equal'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: split-equal" when finished.
