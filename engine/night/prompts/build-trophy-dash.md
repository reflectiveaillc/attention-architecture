...
1. `web/site/games/trophy-dash/index.html` — the game (single self-contained file)
2. `web/site/games/trophy-dash/copy.md` — social captions
...
## The game: Trophy Dash

- **Conditioned path (the hook):** achievement / trophy collecting. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Race a friend through a 60-second obstacle course; trophies spawn on the path. Most trophies at the finish wins the dare. Both players see the identical course, so the better run takes it.
...
- Includes in order: `window.LOOP_GAME='trophy-dash'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'trophies'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: trophy-dash" when finished.
