...
1. `web/site/games/duet-chain/index.html` — the game (single self-contained file)
2. `web/site/games/duet-chain/copy.md` — social captions
...
- **Conditioned path (the hook):** duet/stitch remix chain. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A beat plays under a looping clip; tap on each beat to attach your frame to the public remix chain — one mistimed tap and the chain snaps.
...
- Includes in order: `window.LOOP_GAME='duet-chain'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: duet-chain" when finished.
