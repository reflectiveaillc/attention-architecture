...
1. `web/site/games/wire-walk/index.html` — the game (single self-contained file)
2. `web/site/games/wire-walk/copy.md` — social captions
...
- **Conditioned path (the hook):** steady-hand / 'don't touch the wire' challenge videos that turn precision into spectacle. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A dot travels a winding wire; tap and hold to guide it along the exact center. Any deviation from the wire ends the run. PB is the farthest distance you kept centered.
- **Engine:** viral. **Circuits:** 01-rpe, 05-ease, 04-loss.
- **Video trick:** steady-hand flex — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='wire-walk'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'cm'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Wire Walk — copy
## TikTok (3 captions ≤100 chars, hook-first, steady-hand flex angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: wire-walk" when finished.
