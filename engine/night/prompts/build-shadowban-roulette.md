...
1. `web/site/games/shadowban-roulette/index.html` — the game (single self-contained file)
2. `web/site/games/shadowban-roulette/copy.md` — social captions
...
## The game: Shadowban Roulette

- **Conditioned path (the hook):** 'your video is being reviewed' / shadowban anxiety. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Posts sit in a 'reviewing…' state; tap CHECK to reveal status — but every check lowers your trust score. Check too often and the post goes shadowbanned forever.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 04-loss, 03-social.
...
- Includes in order: `window.LOOP_GAME='shadowban-roulette'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'posts cleared'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: shadowban-roulette" when finished.
