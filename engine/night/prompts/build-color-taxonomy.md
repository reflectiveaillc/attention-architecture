...
1. `web/site/games/color-taxonomy/index.html` — the game (single self-contained file)
2. `web/site/games/color-taxonomy/copy.md` — social captions
...
## The game: COLOR TAXONOMY

- **Conditioned path (the hook):** colour-theory and palette rabbit-holes — the deep-dives on hue families and complementary systems. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Swatches arrive one at a time; drag each into the correct position on a colour wheel taxonomy. There are no labels — only the relationship between neighbours tells you where a swatch belongs. A perfectly placed swatch hums softly; a wrong placement nudges back. 24 swatches, one clean wheel.
- **Engine:** calm. **Circuits:** 01-rpe, 05-ease, 06-zeigarnik.
- **Video trick:** near-perfection itch — almost right, nudge it one slot — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='color-taxonomy'` → loop-events.js → juice.js → share.js.
...
```
# COLOR TAXONOMY — copy
## TikTok (3 captions ≤100 chars, hook-first, near-perfection itch — almost right, nudge it one slot angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: color-taxonomy" when finished.
