...
1. `web/site/games/switch-stance/index.html` — the game (single self-contained file)
2. `web/site/games/switch-stance/copy.md` — social captions
...
## The game: SWITCH STANCE

- **Conditioned path (the hook):** control-scheme shifts mid-scroll — swipe gestures that invert between apps without warning. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** One endless runner track; every 6 seconds the control scheme flips: left/right invert, tap-to-jump becomes tap-to-duck, swipe-up becomes swipe-down. Adapt to the new stance or crash. The novelty is your own muscle memory betraying you — and a crash ends it.
- **Engine:** viral. **Circuits:** 02-novelty, 05-ease, 04-loss.
- **Video trick:** shock-honesty — 'I just died because the controls flipped' is the share line — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='switch-stance'` → loop-events.js → juice.js → share.js.
...
```
# SWITCH STANCE — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty — 'I just died because the controls flipped' is the share line angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: switch-stance" when finished.
