...
1. `web/site/games/aim-snap/index.html` — the game (single self-contained file)
2. `web/site/games/aim-snap/copy.md` — social captions
...
- **Conditioned path (the hook):** precision-tap mobile-game clips and speed-aim challenges. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A crosshair drifts across targets; tap when it perfectly centers the active target. The drift speeds up and decoy targets pop to steal the aim.
...
- Includes in order: `window.LOOP_GAME='aim-snap'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: aim-snap" when finished.
