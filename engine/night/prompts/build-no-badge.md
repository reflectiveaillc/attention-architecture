...
1. `web/site/games/no-badge/index.html` — the game (single self-contained file)
2. `web/site/games/no-badge/copy.md` — social captions
...
- **Mechanic:** Red dots appear. You do not tap them. They fade on their own if left alone. Each un-tapped badge dissolves into a soft ripple. The point is to unlearn the swipe-to-clear reflex.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 04-loss.
...
- Includes in order: `window.LOOP_GAME='no-badge'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: no-badge" when finished.
