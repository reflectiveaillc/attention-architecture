...
1. `web/site/games/drafts-release/index.html` — the game (single self-contained file)
2. `web/site/games/drafts-release/copy.md` — social captions
...
## The game: DRAFTS RELEASE

- **Conditioned path (the hook):** posting anxiety + the drafts folder (the hoard of unposted thoughts, inverted to letting go). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A stack of 'draft' cards (half-written thoughts) sits on screen; swipe each one gently upward and it dissolves with a soft exhale. No posting, no saving — just releasing the pressure to say it. The session ends when the drafts are clear.
...
- Includes in order: `window.LOOP_GAME='drafts-release'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: drafts-release" when finished.
