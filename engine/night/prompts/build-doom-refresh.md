...
1. `web/site/games/doom-refresh/index.html` — the game (single self-contained file)
2. `web/site/games/doom-refresh/copy.md` — social captions
...
## The game: Doom Refresh

- **Conditioned path (the hook):** refresh roulette. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Pull down to refresh. The spinner pauses at random. Release exactly when it lands on green. Release on red and you 'lose connection.' Each refresh adds a fake hang that tests your nerve.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 04-loss.
- **Video trick:** variable-reward roulette — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='doom-refresh'` → loop-events.js → juice.js → share.js.
...
```
# Doom Refresh — copy
## TikTok (3 captions ≤100 chars, hook-first, variable-reward roulette angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: doom-refresh" when finished.
