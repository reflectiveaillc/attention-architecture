...
1. `web/site/games/declutter-tap/index.html` — the game (single self-contained file)
2. `web/site/games/declutter-tap/copy.md` — social captions
...
## The game: DECLUTTER TAP

- **Conditioned path (the hook):** minimalism and throw-away ASMR that lingers on the crowded surface before release. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A shelf of objects appears; some have a faint grey glow marking them as clutter. Tap each glowing object to remove it with a gentle poof. When only the essential items remain and the shelf breathes, the session closes with a quiet exhale sound.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 04-loss.
...
- Includes in order: `window.LOOP_GAME='declutter-tap'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: declutter-tap" when finished.
