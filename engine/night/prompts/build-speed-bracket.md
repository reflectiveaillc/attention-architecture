...
1. `web/site/games/speed-bracket/index.html` — the game (single self-contained file)
2. `web/site/games/speed-bracket/copy.md` — social captions
...
## The game: Speed Bracket

- **Conditioned path (the hook):** live tournament brackets / elimination streams. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A dare-link starts a 4-player bracket. Each match is one 30-second thumb duel; winners advance, losers are out. The bracket resolves in under 3 minutes and the winner holds the bracket badge.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 04-loss.
...
- Includes in order: `window.LOOP_GAME='speed-bracket'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'bracket'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: speed-bracket" when finished.
