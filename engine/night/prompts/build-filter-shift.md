...
1. `web/site/games/filter-shift/index.html` — the game (single self-contained file)
2. `web/site/games/filter-shift/copy.md` — social captions
...
## The game: Filter Shift

- **Conditioned path (the hook):** filter face-transform. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A face-like mask morphs through filters. Tap when the filter matches the target prompt (e.g. 'old', 'neon', 'cartoon'). The mask only holds the target for a split second before it slides to the next trend.
...
- Includes in order: `window.LOOP_GAME='filter-shift'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: filter-shift" when finished.
