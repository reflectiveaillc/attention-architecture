...
1. `web/site/games/color-chain/index.html` — the game (single self-contained file)
2. `web/site/games/color-chain/copy.md` — social captions
...
## The game: COLOR CHAIN

- **Conditioned path (the hook):** trending color-sequence challenges that spread because scores are socially comparable. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A chain of colored dots pulses in sequence; replay the chain by tapping the dots in the same order. Each correct replay adds one dot to the chain. The longest unbroken chain is the dare unit sent to friends.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 06-zeigarnik.
...
- Includes in order: `window.LOOP_GAME='color-chain'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'chain'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: color-chain" when finished.
