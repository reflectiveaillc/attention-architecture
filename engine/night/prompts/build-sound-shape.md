...
1. `web/site/games/sound-shape/index.html` — the game (single self-contained file)
2. `web/site/games/sound-shape/copy.md` — social captions
...
## The game: SOUND SHAPE

- **Conditioned path (the hook):** sound-reactive ASMR and Cymatics videos where sound visibly reshapes sand or water into patterns. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap a tone button to emit a synthesized frequency; the particle field on screen rearranges into the standing-wave pattern for that tone. Choose from five tones. Hold the last tone until the pattern locks and hums into stillness.
...
- Includes in order: `window.LOOP_GAME='sound-shape'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sound-shape" when finished.
