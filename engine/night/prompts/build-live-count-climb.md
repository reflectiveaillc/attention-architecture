...
1. `web/site/games/live-count-climb/index.html` — the game (single self-contained file)
2. `web/site/games/live-count-climb/copy.md` — social captions
...
## The game: Live Count Climb

- **Conditioned path (the hook):** live viewer count. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A number ticks up and down like a live viewer count. Tap the screen to 'go live' when the count hits a target. The count jitters unpredictably, turning social proof into a twitch-aim game.
...
- Includes in order: `window.LOOP_GAME='live-count-climb'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'viewers'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: live-count-climb" when finished.
