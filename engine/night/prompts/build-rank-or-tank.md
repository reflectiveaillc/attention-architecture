...
1. `web/site/games/rank-or-tank/index.html` — the game (single self-contained file)
2. `web/site/games/rank-or-tank/copy.md` — social captions
...
## The game: Rank or Tank

- **Conditioned path (the hook):** all-or-nothing algorithm gamble. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** One 60-second round, no retries. Your score maps to a single session rank badge — Bronze, Silver, or Gold. Share the badge as a dare; the receiver must also play one round and either outrank you or tank.
...
- Includes in order: `window.LOOP_GAME='rank-or-tank'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rank-or-tank" when finished.
