...
1. `web/site/games/archive-walk/index.html` — the game (single self-contained file)
2. `web/site/games/archive-walk/copy.md` — social captions
...
## The game: ARCHIVE WALK

- **Conditioned path (the hook):** memory/archive 'on this day' (the IG archive recap, without the recomputation anxiety). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A slow vertical scroll through your 'archive' — soft dated photo cards drift up one at a time. The scroll is pace-locked so you can't rush; each card lingers, then passes. The walk has a last card and a gentle close. Memory as a stroll, not a feed.
- **Engine:** calm. **Circuits:** 07-timing, 05-ease, 08-garden.
...
- Includes in order: `window.LOOP_GAME='archive-walk'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: archive-walk" when finished.
