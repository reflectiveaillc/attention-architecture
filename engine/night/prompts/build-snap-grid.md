...
1. `web/site/games/snap-grid/index.html` — the game (single self-contained file)
2. `web/site/games/snap-grid/copy.md` — social captions
...
- **Conditioned path (the hook):** organization / 'everything in its place' satisfying grid content. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag a floating shape and release when it snaps perfectly into the grid. Snap distance is scored in pixels; later levels shrink the grid and add misaligned decoys.
...
- Includes in order: `window.LOOP_GAME='snap-grid'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: snap-grid" when finished.
