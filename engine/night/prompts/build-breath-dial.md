...
1. `web/site/games/breath-dial/index.html` — the game (single self-contained file)
2. `web/site/games/breath-dial/copy.md` — social captions
...
## The game: BREATH DIAL

- **Conditioned path (the hook):** doomscrolling as a nervous-system coping loop — chasing the next card to feel in control. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A single dial sits center-screen; drag it clockwise to inhale (circle expands, tone rises), counterclockwise to exhale (circle contracts, tone falls). The dial moves only as fast as you drag — you set every pace. No wrong speed, no failure state. Five full slow cycles and the session softly closes.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 01-rpe.
- **Video trick:** calm completion — the circle expanding and contracting is the whole reveal — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='breath-dial'` → loop-events.js → juice.js → share.js.
...
```
# BREATH DIAL — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the circle expanding and contracting is the whole reveal angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: breath-dial" when finished.
