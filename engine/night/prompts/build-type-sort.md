...
1. `web/site/games/type-sort/index.html` — the game (single self-contained file)
2. `web/site/games/type-sort/copy.md` — social captions
...
- **Conditioned path (the hook):** typology rabbit-holes — MBTI, Enneagram, plant genus, bird species ID threads the algorithm surfaces to systemising brains. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Specimens slide in one at a time (abstract shapes with coded traits). Drag each into the correct type-bucket based on a key you build up as you go — early specimens are textbook examples, later ones are edge cases. 24 specimens. When the last one slots in, every bucket shows its complete set.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** completion-satisfaction — the full taxonomy displayed — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='type-sort'` → loop-events.js → juice.js → share.js.
...
```
# TYPE SORT — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the full taxonomy displayed angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: type-sort" when finished.
