...
1. `web/site/games/crunch-stamp/index.html` — the game (single self-contained file)
2. `web/site/games/crunch-stamp/copy.md` — social captions
...
## The game: CRUNCH STAMP

- **Conditioned path (the hook):** packing-foam and crunch-squish ASMR where the thumb pressing down is the whole satisfaction. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Press and slowly release each packing-foam cell on a grid; each cell crumples inward with a crunch sound scaled to how hard you press. Softer press = softer crunch. Fill all twenty cells to complete the sheet and end the session.
...
- Includes in order: `window.LOOP_GAME='crunch-stamp'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: crunch-stamp" when finished.
