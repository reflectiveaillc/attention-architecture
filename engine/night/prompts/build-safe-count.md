...
1. `web/site/games/safe-count/index.html` — the game (single self-contained file)
2. `web/site/games/safe-count/copy.md` — social captions
...
- **Conditioned path (the hook):** the reassurance-seeking scroll — counting wins, good news, safe signals in a feed that keeps the alarm on. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Safe-signal tiles (soft green shapes) and threat tiles (soft red) drift down mixed together. Tap only the green ones to bank them before they reach the bottom. Miss a green and lose one from your tally; tap a red and your hand shakes for a second. Score is the highest safe-count you reach. Finding the safe signals and naming them — bounded, winnable.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 05-ease.
- **Video trick:** near-perfection itch — the almost-missed green tile that haunts the retry — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='safe-count'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'safe'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# SAFE COUNT — copy
## TikTok (3 captions ≤100 chars, hook-first, near-perfection itch — the almost-missed green tile that haunts the retry angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: safe-count" when finished.
