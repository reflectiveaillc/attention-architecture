...
1. `web/site/games/tile-fill/index.html` — the game (single self-contained file)
2. `web/site/games/tile-fill/copy.md` — social captions
...
- **Conditioned path (the hook):** incomplete mosaic and pixel-fill ASMR where the last tile never arrives. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A grid has a handful of empty cells; tap each empty cell to fill it with the matching color drawn from the surrounding pattern. When every cell is filled the mosaic pulses once in satisfaction and the game ends.
...
- Includes in order: `window.LOOP_GAME='tile-fill'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tile-fill" when finished.
