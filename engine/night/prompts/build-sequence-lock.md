...
1. `web/site/games/sequence-lock/index.html` — the game (single self-contained file)
2. `web/site/games/sequence-lock/copy.md` — social captions
...
## The game: SEQUENCE LOCK

- **Conditioned path (the hook):** hyper-specific procedure loops — the forum thread that lays out the exact 12-step ritual for something obscure. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A combination lock shows 6 dials; each dial has a hidden rule for which symbol it cycles to next. Tap each dial in the correct sequence to open the lock. No guidance — pattern-watch two full cycles, then execute. Click, click, click, open. A finite vault with a clean end.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 01-rpe, 08-investment.
- **Video trick:** completion-satisfaction — the lock clicks open — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='sequence-lock'` → loop-events.js → juice.js → share.js.
...
```
# SEQUENCE LOCK — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the lock clicks open angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sequence-lock" when finished.
