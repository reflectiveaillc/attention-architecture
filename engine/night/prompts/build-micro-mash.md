...
1. `web/site/games/micro-mash/index.html` — the game (single self-contained file)
2. `web/site/games/micro-mash/copy.md` — social captions
...
- **Conditioned path (the hook):** TikTok trend-mashup feeds — rapid-cut comps of 15 different trends stitched into one 30s clip. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A fixed deck of 15 micro-games, each 3 seconds with a one-word instruction (TAP, HOLD, SWIPE, DODGE, LIFT, CATCH). Clear one, the next flashes up instantly — a fresh novelty hit every 3s. The deck has a bottom card: clear all 15 and it prints FIN. Variety shuffle with a real ending.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 05-ease.
- **Video trick:** WarioWare-style rapid-cut — each 3s clip is a different mini-hit, the pace is the hook — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='micro-mash'` → loop-events.js → juice.js → share.js.
...
```
# MICRO MASH — copy
## TikTok (3 captions ≤100 chars, hook-first, WarioWare-style rapid-cut — each 3s clip is a different mini-hit, the pace is the hook angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: micro-mash" when finished.
