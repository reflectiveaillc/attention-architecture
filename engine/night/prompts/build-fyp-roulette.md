...
1. `web/site/games/fyp-roulette/index.html` — the game (single self-contained file)
2. `web/site/games/fyp-roulette/copy.md` — social captions
...
## The game: FYP Roulette

- **Conditioned path (the hook):** FYP unpredictability. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A vertical feed of colored tiles flies by. Swipe up to skip, swipe down to save. One random tile is the 'jackpot clip'. The faster the feed, the more the variable reward anticipation screams.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 05-ease.
- **Video trick:** variable reward cliffhanger — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='fyp-roulette'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'jackpots'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# FYP Roulette — copy
## TikTok (3 captions ≤100 chars, hook-first, variable reward cliffhanger angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: fyp-roulette" when finished.
