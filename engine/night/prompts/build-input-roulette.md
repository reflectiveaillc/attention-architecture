...
1. `web/site/games/input-roulette/index.html` — the game (single self-contained file)
2. `web/site/games/input-roulette/copy.md` — social captions
...
## The game: INPUT ROULETTE

- **Conditioned path (the hook):** app onboarding — every new screen demands a different gesture you've never seen. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Each round flashes a random input icon (tap, long-press, swipe, double-tap, drag) for 2 seconds; do the right gesture before the window closes. The demanded verb is the novelty — re-learn the move each round. Streak builds until a wrong gesture ends it.
- **Engine:** viral. **Circuits:** 02-novelty, 05-ease, 01-rpe.
- **Video trick:** split-second judgment — 'tap or hold?!' panic is the clip — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='input-roulette'` → loop-events.js → juice.js → share.js.
...
```
# INPUT ROULETTE — copy
## TikTok (3 captions ≤100 chars, hook-first, split-second judgment — 'tap or hold?!' panic is the clip angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: input-roulette" when finished.
