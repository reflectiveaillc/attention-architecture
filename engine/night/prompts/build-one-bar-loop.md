...
1. `web/site/games/one-bar-loop/index.html` — the game (single self-contained file)
2. `web/site/games/one-bar-loop/copy.md` — social captions
...
## The game: ONE BAR LOOP

- **Conditioned path (the hook):** one-bar loop audio memes that auto-replay exactly one measure until the listener tunes out. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A single 4-beat bar repeats; tap on every beat of every loop. The challenge: how many perfect loops in a row without a miss? Each perfect loop tightens the accuracy window. 16 loops and the track signs off — the loop that ends.
- **Engine:** viral. **Circuits:** 01-rpe, 08-investment, 06-zeigarnik.
- **Video trick:** clean-run chase — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='one-bar-loop'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'loops'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# ONE BAR LOOP — copy
## TikTok (3 captions ≤100 chars, hook-first, clean-run chase angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: one-bar-loop" when finished.
