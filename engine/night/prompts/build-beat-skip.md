...
1. `web/site/games/beat-skip/index.html` — the game (single self-contained file)
2. `web/site/games/beat-skip/copy.md` — social captions
...
- **Conditioned path (the hook):** edited loop clips that drop one beat every few bars to create the "wait, something\'s off" hook. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A steady beat plays — but every 8th beat is silent. Tap on all normal beats; do NOT tap on the silent one. Your streak is how many bars you navigate without tapping the hole. The loop ends at bar 16.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 04-loss.
...
- Includes in order: `window.LOOP_GAME='beat-skip'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: beat-skip" when finished.
