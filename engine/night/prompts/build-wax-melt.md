...
1. `web/site/games/wax-melt/index.html` — the game (single self-contained file)
2. `web/site/games/wax-melt/copy.md` — social captions
...
- **Conditioned path (the hook):** wax-melting and candle-drip ASMR where the slow inevitability of the drip is hypnotic. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap the top of a procedural wax pillar to ignite it; heat radiates downward in a slow melt with drip audio cues. Swipe to rotate the pillar and guide the drip. The candle fully melts over ninety seconds, then the screen fades to a soft glow.
...
- Includes in order: `window.LOOP_GAME='wax-melt'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: wax-melt" when finished.
