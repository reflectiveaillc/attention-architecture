...
1. `web/site/games/seen-receipt/index.html` — the game (single self-contained file)
2. `web/site/games/seen-receipt/copy.md` — social captions
...
## The game: Seen Receipt

- **Conditioned path (the hook):** seen-receipt anticipation. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A message bubble pulses; a tiny 'seen' check fades in and out. Tap exactly as the second check appears. Early = desperate; late = ignored. The anxiety of waiting becomes the input cue.
...
- Includes in order: `window.LOOP_GAME='seen-receipt'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: seen-receipt" when finished.
