...
1. `web/site/games/quick-quest/index.html` — the game (single self-contained file)
2. `web/site/games/quick-quest/copy.md` — social captions
...
## The game: QUICK QUEST

- **Conditioned path (the hook):** the 5-part 'wait for it' challenge reel — a stacked to-do list of micro-dares. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A fixed quest log of 10 micro-missions, each 5 seconds and totally different: catch 3, avoid 2, hold 1s, swipe 4, trace the line. Check one off, the next appears. The quest log visibly completes — every open loop actually closes, then FIN.
- **Engine:** viral. **Circuits:** 02-novelty, 06-zeigarnik, 01-rpe.
- **Video trick:** completion-satisfaction — watch the quest log tick down to zero — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='quick-quest'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'quests'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# QUICK QUEST — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — watch the quest log tick down to zero angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: quick-quest" when finished.
