...
1. `web/site/games/species-log/index.html` — the game (single self-contained file)
2. `web/site/games/species-log/copy.md` — social captions
...
## The game: SPECIES LOG

- **Conditioned path (the hook):** bird-watching and wildlife photography posts that tease a life-list never finished. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Silhouette cards drift past like sightings; tap each silhouette you recognize to log it in a field guide of 15 species. Each logged species fills a page in a finite guide. When all 15 pages are filled the guide closes and the session ends.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 02-novelty.
- **Video trick:** calm completion — the life-list closed, nothing missing — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='species-log'` → loop-events.js → juice.js → share.js.
...
```
# SPECIES LOG — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the life-list closed, nothing missing angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: species-log" when finished.
