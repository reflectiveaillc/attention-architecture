...
1. `web/site/games/one-stroke-maze/index.html` — the game (single self-contained file)
2. `web/site/games/one-stroke-maze/copy.md` — social captions
...
## The game: One Stroke Maze

- **Conditioned path (the hook):** one-stroke drawing / 'perfect line' art challenges that reward a single clean motion. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Trace a maze in one continuous thumb stroke without lifting. Scored on smoothness and time; the PB is a combined clean-run score that rewards grace over speed.
- **Engine:** calm. **Circuits:** 01-soft-rpe, 05-ease, 07-timing.
...
- Includes in order: `window.LOOP_GAME='one-stroke-maze'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: one-stroke-maze" when finished.
