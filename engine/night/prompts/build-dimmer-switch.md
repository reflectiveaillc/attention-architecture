...
1. `web/site/games/dimmer-switch/index.html` — the game (single self-contained file)
2. `web/site/games/dimmer-switch/copy.md` — social captions
...
## The game: Dimmer Switch

- **Conditioned path (the hook):** the 2am doomscroll where the screen is the only light left on in the room. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Press and hold anywhere; the screen brightness decays the longer you hold, lifting toward black. Release and it creeps back up. The whole game is holding the dimmer down through one long exhale until the screen goes fully dark — that black frame is the end.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 06-zeigarnik.
- **Video trick:** the feed that ends — the screen itself is the object being put down — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='dimmer-switch'` → loop-events.js → juice.js → share.js.
...
```
# Dimmer Switch — copy
## TikTok (3 captions ≤100 chars, hook-first, the feed that ends — the screen itself is the object being put down angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: dimmer-switch" when finished.
