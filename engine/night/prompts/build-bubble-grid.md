...
1. `web/site/games/bubble-grid/index.html` — the game (single self-contained file)
2. `web/site/games/bubble-grid/copy.md` — social captions
...
## The game: BUBBLE GRID

- **Conditioned path (the hook):** pop-it and bubble-wrap ASMR loops that deliver a pop hit on every tap. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A grid of soft bubble circles fills the canvas; tap each to pop it with a satisfying synthesized micro-burst. Bubbles refill slowly in waves after the grid is cleared, giving a definite full-pop moment then a gentle reset.
...
- Includes in order: `window.LOOP_GAME='bubble-grid'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: bubble-grid" when finished.
