...
1. `web/site/games/profile-settle/index.html` — the game (single self-contained file)
2. `web/site/games/profile-settle/copy.md` — social captions
...
## The game: PROFILE SETTLE

- **Conditioned path (the hook):** profile picture / glow-up editing (the endless profile-polish, inverted to done). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** An avatar sits with filter effects slowly spinning around it (polish, glow, sharpen). Hold still and the effects gradually calm and fall away, settling into one quiet resting portrait. When it's still, the session closes — you're done, you can stop editing.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 08-garden.
...
- Includes in order: `window.LOOP_GAME='profile-settle'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: profile-settle" when finished.
