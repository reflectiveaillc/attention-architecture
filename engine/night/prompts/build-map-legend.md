...
1. `web/site/games/map-legend/index.html` — the game (single self-contained file)
2. `web/site/games/map-legend/copy.md` — social captions
...
- **Conditioned path (the hook):** cartography and classification rabbit-holes — the deep-dive on map symbology, legend systems, and geographic taxonomy. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A map displays regions marked with unknown symbols; a legend panel shows symbol definitions in scrambled order. Drag each legend label to its correct symbol. The map's symbols are consistent — once you decode one region, the pattern cascades. All labels matched, the map unlocks its full annotated view.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** completion-satisfaction — the map finally makes sense — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='map-legend'` → loop-events.js → juice.js → share.js.
...
```
# MAP LEGEND — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the map finally makes sense angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: map-legend" when finished.
