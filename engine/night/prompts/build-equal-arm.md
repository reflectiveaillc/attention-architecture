...
1. `web/site/games/equal-arm/index.html` — the game (single self-contained file)
2. `web/site/games/equal-arm/copy.md` — social captions
...
- **Conditioned path (the hook):** balance / physics-perfect setup videos where equilibrium is the satisfying end-state. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Weights slide along a balance beam; tap to lock each weight so the arms balance exactly. Error is shown in grams, and the PB is the lowest total imbalance.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 05-ease.
- **Video trick:** balance PB — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='equal-arm'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'g'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Equal Arm — copy
## TikTok (3 captions ≤100 chars, hook-first, balance PB angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: equal-arm" when finished.
