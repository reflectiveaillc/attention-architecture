...
1. `web/site/games/completion-meter/index.html` — the game (single self-contained file)
2. `web/site/games/completion-meter/copy.md` — social captions
...
## The game: COMPLETION METER

- **Conditioned path (the hook):** progress bars on social profiles that sit at 85% and itch for one more action. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A progress bar shows a collection at 80% complete. Scattered collectibles fall across the screen; tap each to add it to the bar. The bar climbs to 100%, the collectibles stop falling, and the meter seals with a final ding. 100% is reachable in under 2 minutes.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 04-loss.
- **Video trick:** near-perfection itch — the meter at 95% screams for one more tap — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='completion-meter'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: '%'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# COMPLETION METER — copy
## TikTok (3 captions ≤100 chars, hook-first, near-perfection itch — the meter at 95% screams for one more tap angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: completion-meter" when finished.
