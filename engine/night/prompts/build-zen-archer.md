...
1. `web/site/games/zen-archer/index.html` — the game (single self-contained file)
2. `web/site/games/zen-archer/copy.md` — social captions
...
- **Conditioned path (the hook):** archery / target-sport PB compilations that glorify the one perfect release. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold to draw the bow, release at peak tension for a bullseye. Release too early or late and the arrow drifts. Wind adds micro-variance so mastery is measurable in millimeters.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 07-timing.
- **Video trick:** precision flex — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='zen-archer'` → loop-events.js → juice.js → share.js.
...
```
# Zen Archer — copy
## TikTok (3 captions ≤100 chars, hook-first, precision flex angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: zen-archer" when finished.
