...
1. `web/site/games/live-count/index.html` — the game (single self-contained file)
2. `web/site/games/live-count/copy.md` — social captions
...
- **Mechanic:** A viewer-count number flickers under your stream. Hold to keep going; release to cash out followers. Sudden viewer drops force a split-second decision: hold for the comeback or bail before zero.
...
- Includes in order: `window.LOOP_GAME='live-count'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: live-count" when finished.
