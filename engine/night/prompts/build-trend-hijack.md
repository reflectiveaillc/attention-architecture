...
1. `web/site/games/trend-hijack/index.html` — the game (single self-contained file)
2. `web/site/games/trend-hijack/copy.md` — social captions
...
## The game: Trend Hijack

- **Conditioned path (the hook):** trending-hashtag riding. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A hashtag's reach curve rises and falls; tap to jump on as it climbs, tap to jump off at the peak — stay on past the peak and your reach craters.
...
- Includes in order: `window.LOOP_GAME='trend-hijack'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'peak reach'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: trend-hijack" when finished.
