...
1. `web/site/games/viral-wave/index.html` — the game (single self-contained file)
2. `web/site/games/viral-wave/copy.md` — social captions
...
- **Conditioned path (the hook):** going-viral view-count spike. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** View count spins up fast; tap RIDE to multiply while it rises — the instant it peaks, STOP. One tap past the peak and the post crashes back to zero.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 02-novelty.
- **Video trick:** ecstasy fail — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='viral-wave'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'peak views'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Viral Wave — copy
## TikTok (3 captions ≤100 chars, hook-first, ecstasy fail angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: viral-wave" when finished.
