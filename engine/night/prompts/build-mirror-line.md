...
1. `web/site/games/mirror-line/index.html` — the game (single self-contained file)
2. `web/site/games/mirror-line/copy.md` — social captions
...
## The game: MIRROR LINE

- **Conditioned path (the hook):** symmetry-check and before/after cleaning videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** One half of the canvas shows a fixed pattern of shapes; drag shapes onto the blank half to mirror it perfectly. A symmetry meter ticks toward 100% as you match each element across the center axis.
...
- Includes in order: `window.LOOP_GAME='mirror-line'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: mirror-line" when finished.
