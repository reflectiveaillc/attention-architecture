...
1. `web/site/games/top-ten-today/index.html` — the game (single self-contained file)
2. `web/site/games/top-ten-today/copy.md` — social captions
...
## The game: Top Ten Today

- **Conditioned path (the hook):** daily leaderboard rank envy. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Play once. If your score lands in today's top 10% you get a gold badge and a dare-link to challenge the player directly above you. The ranking resets tomorrow; nothing persists beyond the day.
...
- Includes in order: `window.LOOP_GAME='top-ten-today'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'rank'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: top-ten-today" when finished.
