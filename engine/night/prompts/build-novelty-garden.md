...
1. `web/site/games/novelty-garden/index.html` — the game (single self-contained file)
2. `web/site/games/novelty-garden/copy.md` — social captions
...
## The game: NOVELTY GARDEN

- **Conditioned path (the hook):** the novelty-craving itself — the brain that wants a fresh hit every few seconds, met without the spike. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A daily deck of 10 tiny varied sensory micro-toys — pop one, ripple one, spin one, tap one — each a fresh little input, but slow and unhurried. Draw one card at your own pace; the deck has a last card, and finishing plants a small garden tile. Bounded variety with no spike, then it's done for the day.
- **Engine:** calm. **Circuits:** 01-soft-rpe, 02-novelty, 08-garden.
- **Video trick:** the feed that ends — novelty that closes itself instead of scrolling forever — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='novelty-garden'` → loop-events.js → juice.js → share.js.
...
```
# NOVELTY GARDEN — copy
## TikTok (3 captions ≤100 chars, hook-first, the feed that ends — novelty that closes itself instead of scrolling forever angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: novelty-garden" when finished.
