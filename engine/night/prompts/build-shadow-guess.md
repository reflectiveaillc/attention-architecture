...
1. `web/site/games/shadow-guess/index.html` — the game (single self-contained file)
2. `web/site/games/shadow-guess/copy.md` — social captions
...
## The game: Shadow Guess

- **Conditioned path (the hook):** guess the object" shadow videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A shadow slowly rotates; tap when you recognize the object before the full reveal. Faster correct guesses unlock harder silhouettes and end with a final master shape.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 06-zeigarnik.
- **Video trick:** guess before the reveal — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='shadow-guess'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'guesses'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Shadow Guess — copy
## TikTok (3 captions ≤100 chars, hook-first, guess before the reveal angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: shadow-guess" when finished.
