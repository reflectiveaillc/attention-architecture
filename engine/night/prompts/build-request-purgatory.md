...
1. `web/site/games/request-purgatory/index.html` — the game (single self-contained file)
2. `web/site/games/request-purgatory/copy.md` — social captions
...
## The game: Request Purgatory

- **Conditioned path (the hook):** DM request/unread indicator. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Message-request bubbles float up the screen. Swipe right to accept, left to decline. Accept spam and you lose a life; decline a real one and you lose two. The icons grow more identical as you climb.
- **Engine:** viral. **Circuits:** 03-social, 04-loss, 02-novelty.
- **Video trick:** split-second judgment — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='request-purgatory'` → loop-events.js → juice.js → share.js.
...
```
# Request Purgatory — copy
## TikTok (3 captions ≤100 chars, hook-first, split-second judgment angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: request-purgatory" when finished.
