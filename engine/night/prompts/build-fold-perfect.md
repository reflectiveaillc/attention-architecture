...
1. `web/site/games/fold-perfect/index.html` — the game (single self-contained file)
2. `web/site/games/fold-perfect/copy.md` — social captions
...
## The game: FOLD PERFECT

- **Conditioned path (the hook):** laundry-folding and closet-organization ASMR that pauses on the messy drawer. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A shirt shape sits on the canvas with fold-line guides; swipe along each guide in sequence to fold it into a tight square. Imprecise swipes leave a crease indicator; a clean fold earns a crisp snap and the shirt stacks perfectly.
...
- Includes in order: `window.LOOP_GAME='fold-perfect'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: fold-perfect" when finished.
