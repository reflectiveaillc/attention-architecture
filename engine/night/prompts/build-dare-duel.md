...
1. `web/site/games/dare-duel/index.html` — the game (single self-contained file)
2. `web/site/games/dare-duel/copy.md` — social captions
...
- **Conditioned path (the hook):** direct-message / story dare challenges. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap to send a dare-link to one friend; both of you get exactly one 60-second run. The higher score wins and the loser gets a 'settled' stamp. The dare closes after both play — no ladder, just the score.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 04-loss.
- **Video trick:** head-to-head split — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='dare-duel'` → loop-events.js → juice.js → share.js.
...
```
# Dare Duel — copy
## TikTok (3 captions ≤100 chars, hook-first, head-to-head split angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: dare-duel" when finished.
