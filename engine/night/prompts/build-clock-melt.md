...
1. `web/site/games/clock-melt/index.html` — the game (single self-contained file)
2. `web/site/games/clock-melt/copy.md` — social captions
...
- **Conditioned path (the hook):** the 2am bedside-clock-watching that makes time stretch instead of pass. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A bedside clock face shows; hold to slow the minute hand. The longer you hold, the more the hand drags and the screen dims, stretching time toward stillness. When the hand stops and the face goes black, time has finally let go and the session ends.
- **Engine:** calm. **Circuits:** 07-timing, 05-ease, 06-zeigarnik.
- **Video trick:** shock-honesty — the clock you've been staring at, defused — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='clock-melt'` → loop-events.js → juice.js → share.js.
...
```
# Clock Melt — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty — the clock you've been staring at, defused angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: clock-melt" when finished.
