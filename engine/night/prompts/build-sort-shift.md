...
1. `web/site/games/sort-shift/index.html` — the game (single self-contained file)
2. `web/site/games/sort-shift/copy.md` — social captions
...
- **Conditioned path (the hook):** re-categorization churn — the algorithm silently re-tags your interests mid-session. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Items fall into two lanes; swipe each to the right bin — but the sorting rule changes every 4 seconds: by color, then by shape, then by size, then by symbol. Quick re-categorization every few beats; one mis-sort ends it. The rule churns like a feed re-tagging itself.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 05-ease.
- **Video trick:** near-perfection itch — the mis-sort at the rule-flip haunts you — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='sort-shift'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'sorted'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# SORT SHIFT — copy
## TikTok (3 captions ≤100 chars, hook-first, near-perfection itch — the mis-sort at the rule-flip haunts you angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sort-shift" when finished.
