...
1. `web/site/games/color-sort-jar/index.html` — the game (single self-contained file)
2. `web/site/games/color-sort-jar/copy.md` — social captions
...
## The game: COLOR SORT JAR

- **Conditioned path (the hook):** color-matching and candy-sorting ASMR feeds. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Mixed colored beads fill a jar; tap a bead to select it, then tap a target jar to transfer it. Sort every bead into its matching jar until each container holds only one color and the screen settles into stillness.
...
- Includes in order: `window.LOOP_GAME='color-sort-jar'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: color-sort-jar" when finished.
