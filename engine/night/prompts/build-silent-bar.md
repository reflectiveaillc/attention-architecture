...
1. `web/site/games/silent-bar/index.html` — the game (single self-contained file)
2. `web/site/games/silent-bar/copy.md` — social captions
...
- **Conditioned path (the hook):** viral 4'33" / silent-drop memes that train anticipation of a beat that never comes. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A track plays normally for 3 bars then cuts to silence. Tap on the exact phantom beat where bar 4 should land. The grid shows where the beat was — your timing distance is your score. Silence as the highest-difficulty rhythm challenge.
- **Engine:** viral. **Circuits:** 01-rpe, 07-timing, 06-zeigarnik.
- **Video trick:** anticipation agony — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='silent-bar'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'ms'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# SILENT BAR — copy
## TikTok (3 captions ≤100 chars, hook-first, anticipation agony angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: silent-bar" when finished.
