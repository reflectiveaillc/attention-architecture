...
`engine/state/designs/cat-competitor.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "competitor"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: competitor · label: Rank Chasers · engine_lean: viral
trait: status-driven; rank, clout, beating others
algo_hook: fed ratio wars, follower counts, and leaderboard clout that weaponize social comparison
wean: a clean head-to-head via dare-links — beat a friend's real score, then it's settled, no permanent ladder to obsess over
mechanics: dare a friend, beat the score, percentile rank, head-to-head, top-N% today
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: competitor x20" when finished.
