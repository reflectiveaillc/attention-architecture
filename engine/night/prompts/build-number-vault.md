...
1. `web/site/games/number-vault/index.html` — the game (single self-contained file)
2. `web/site/games/number-vault/copy.md` — social captions
...
## The game: NUMBER VAULT

- **Conditioned path (the hook):** digit-span recall games popularized as 'memory IQ' viral clips. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A number string flashes for 1.5 seconds per two digits; then type it back in full. Length increases by one digit each round. Score is the longest string correctly recalled — the honest working-memory digit span.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 03-social.
- **Video trick:** benchmark ego bait — digit span as social currency — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='number-vault'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'digits'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# NUMBER VAULT — copy
## TikTok (3 captions ≤100 chars, hook-first, benchmark ego bait — digit span as social currency angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: number-vault" when finished.
