...
1. `web/site/games/save-or-lose/index.html` — the game (single self-contained file)
2. `web/site/games/save-or-lose/copy.md` — social captions
...
## The game: SAVE OR LOSE

- **Conditioned path (the hook):** saved/bookmark posts (the IG ribbon for save-for-later, hoarded against a felt scarcity). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Cards drift down a feed; tap the bookmark ribbon on the ones worth saving — but your Saves slot has limited capacity. Fill it and a card you wanted slips past unsaved. The compulsion is choosing what's worth keeping under a hard cap.
- **Engine:** viral. **Circuits:** 04-loss, 03-social, 06-zeigarnik.
...
- Includes in order: `window.LOOP_GAME='save-or-lose'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'saved'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: save-or-lose" when finished.
