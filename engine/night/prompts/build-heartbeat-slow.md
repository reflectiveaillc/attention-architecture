...
1. `web/site/games/heartbeat-slow/index.html` — the game (single self-contained file)
2. `web/site/games/heartbeat-slow/copy.md` — social captions
...
## The game: Heartbeat Slow

- **Conditioned path (the hook):** the elevated heart rate the feed leaves you with at 2am. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A heartbeat thumps on screen; tap in time with each beat. Every matched tap drops the heart rate a few bpm and dims the screen. The goal is to walk the beat all the way down to a resting sleep rate and a black screen — slowing the body, not the feed.
- **Engine:** calm. **Circuits:** 01-rpe, 07-timing, 05-ease.
- **Video trick:** calm completion — bring the rate down yourself — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='heartbeat-slow'` → loop-events.js → juice.js → share.js.
...
```
# Heartbeat Slow — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — bring the rate down yourself angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: heartbeat-slow" when finished.
