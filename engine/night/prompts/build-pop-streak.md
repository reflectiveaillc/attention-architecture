...
1. `web/site/games/pop-streak/index.html` — the game (single self-contained file)
2. `web/site/games/pop-streak/copy.md` — social captions
...
- **Conditioned path (the hook):** pop-it challenge videos where the speed of clearing an entire board is measured and dared. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A pop-it board appears. Tap every bubble as fast as possible to clear the grid. Your time is stamped and shareable as a dare. Each round adds one extra row, raising the ceiling but never the floor.
...
- Includes in order: `window.LOOP_GAME='pop-streak'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pop-streak" when finished.
