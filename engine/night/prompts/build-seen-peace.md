...
1. `web/site/games/seen-peace/index.html` — the game (single self-contained file)
2. `web/site/games/seen-peace/copy.md` — social captions
...
- **Mechanic:** Messages send without status labels. The screen shows only a slow ripple; you wait until it fades. The loop teaches that 'delivered' does not need a reply.
- **Engine:** calm. **Circuits:** 03-social, 05-ease, 07-timing.
...
- Includes in order: `window.LOOP_GAME='seen-peace'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: seen-peace" when finished.
