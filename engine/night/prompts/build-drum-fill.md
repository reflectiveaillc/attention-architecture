...
1. `web/site/games/drum-fill/index.html` — the game (single self-contained file)
2. `web/site/games/drum-fill/copy.md` — social captions
...
- **Conditioned path (the hook):** drum-fill reaction clips where the fill is the only moment that matters in an otherwise repetitive track. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A steady beat loops, then a fill window opens for 2 seconds. Tap a freeform burst of hits in that window; a groove-accuracy meter scores how well your hits land in sub-beat pockets. One fill, one score, one dare.
- **Engine:** viral. **Circuits:** 01-rpe, 07-timing, 05-ease.
- **Video trick:** mastery flex — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='drum-fill'` → loop-events.js → juice.js → share.js.
...
```
# DRUM FILL — copy
## TikTok (3 captions ≤100 chars, hook-first, mastery flex angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: drum-fill" when finished.
