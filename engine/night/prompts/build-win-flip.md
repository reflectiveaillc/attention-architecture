...
1. `web/site/games/win-flip/index.html` — the game (single self-contained file)
2. `web/site/games/win-flip/copy.md` — social captions
...
- **Conditioned path (the hook):** algorithm re-ranking — the thing that won yesterday flops today, the rules silently invert. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A simple tap game, but the win condition inverts every 5 seconds: tap FAST to score, then tap SLOW to score, then DON'T tap to score, then tap only on green. The brain that craves novelty gets it from rule-flips instead of new content — and a wrong move ends it.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 05-ease.
- **Video trick:** shock-honesty — 'the win condition literally flipped on me' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='win-flip'` → loop-events.js → juice.js → share.js.
...
```
# WIN FLIP — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty — 'the win condition literally flipped on me' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: win-flip" when finished.
