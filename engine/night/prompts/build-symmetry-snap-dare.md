...
1. `web/site/games/symmetry-snap-dare/index.html` — the game (single self-contained file)
2. `web/site/games/symmetry-snap-dare/copy.md` — social captions
...
- **Conditioned path (the hook):** spot-the-difference and symmetry-challenge clips where the answer is withheld. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Two halves of a pattern appear; drag the movable piece until both halves are perfectly symmetrical. Your accuracy percentage is calculated to two decimal places and stamped as your shareable score — dare friends to beat your %.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 06-zeigarnik.
- **Video trick:** benchmark ego bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='symmetry-snap-dare'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: symmetry-snap-dare" when finished.
