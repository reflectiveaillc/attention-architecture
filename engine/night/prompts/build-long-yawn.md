...
1. `web/site/games/long-yawn/index.html` — the game (single self-contained file)
2. `web/site/games/long-yawn/copy.md` — social captions
...
- **Conditioned path (the hook):** the contagious-yawn reflex the body uses to signal sleep-onset. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A yawn ripple begins; hold to stretch it out longer. Each held yawn dims the screen a notch and slows the next ripple. Three full yawns and the screen is black — hijacking the body's own sleep-contagion cue as the mechanic.
- **Engine:** calm. **Circuits:** 03-social, 07-timing, 05-ease.
- **Video trick:** calm completion — the contagious yawn as the cue — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='long-yawn'` → loop-events.js → juice.js → share.js.
...
```
# Long Yawn — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the contagious yawn as the cue angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: long-yawn" when finished.
