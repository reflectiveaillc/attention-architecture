...
1. `web/site/games/thread-needle/index.html` — the game (single self-contained file)
2. `web/site/games/thread-needle/copy.md` — social captions
...
- **Conditioned path (the hook):** sewing / craft precision tutorials where the clean thread is the whole payoff. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A tiny gap pulses open and shut; tap to send a thread through at the exact widest moment. Later levels add vibration that makes the window feel impossible.
...
- Includes in order: `window.LOOP_GAME='thread-needle'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'threads'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: thread-needle" when finished.
