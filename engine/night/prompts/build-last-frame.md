...
1. `web/site/games/last-frame/index.html` — the game (single self-contained file)
2. `web/site/games/last-frame/copy.md` — social captions
...
- **Conditioned path (the hook):** 'wait for it' reveal clips where the payoff lasts one frame. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A clip plays toward a final reveal; tap during the single valid frame before the video cuts to black. Fake-out endings appear to trick the eager thumb.
...
- Includes in order: `window.LOOP_GAME='last-frame'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: last-frame" when finished.
