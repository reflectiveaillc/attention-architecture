...
1. `web/site/games/flash-lock/index.html` — the game (single self-contained file)
2. `web/site/games/flash-lock/copy.md` — social captions
...
- **Conditioned path (the hook):** flash-warning / strobe edits that train the eye to lock on brightness. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** The screen strobes white; tap when the flash hits peak brightness. Dim decoy flashes flicker before the real one, forcing the player to wait for the true peak.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 07-timing.
- **Video trick:** sensory overload bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='flash-lock'` → loop-events.js → juice.js → share.js.
...
```
# FLASH LOCK — copy
## TikTok (3 captions ≤100 chars, hook-first, sensory overload bait angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: flash-lock" when finished.
