...
1. `web/site/games/story-expire/index.html` — the game (single self-contained file)
2. `web/site/games/story-expire/copy.md` — social captions
...
## The game: Story Expire

- **Conditioned path (the hook):** stories 24h FOMO. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A ring timer shrinks like a story circle. Tap when the remaining arc hits a hidden target zone before it disappears. Each level hides the target sooner and speeds the decay.
- **Engine:** viral. **Circuits:** 04-loss, 01-rpe, 06-zeigarnik.
- **Video trick:** countdown loss aversion — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='story-expire'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'rings'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Story Expire — copy
## TikTok (3 captions ≤100 chars, hook-first, countdown loss aversion angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: story-expire" when finished.
