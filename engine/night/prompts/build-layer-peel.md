...
1. `web/site/games/layer-peel/index.html` — the game (single self-contained file)
2. `web/site/games/layer-peel/copy.md` — social captions
...
- **Conditioned path (the hook):** what's inside / layer-cake reveal videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Swipe to peel layers off a shape; each layer changes color and sound. Reach the center object and the final reveal plays with a satisfying bloom.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 05-ease.
- **Video trick:** peel to the center — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='layer-peel'` → loop-events.js → juice.js → share.js.
...
```
# Layer Peel — copy
## TikTok (3 captions ≤100 chars, hook-first, peel to the center angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: layer-peel" when finished.
