...
1. `web/site/games/story-peace/index.html` — the game (single self-contained file)
2. `web/site/games/story-peace/copy.md` — social captions
...
## The game: Story Peace

- **Conditioned path (the hook):** story viewer-list compulsive checking. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A full-screen story plays with NO viewer list and NO count — just the content fading gently frame to frame. The whole game is watching without checking who saw it.
...
- Includes in order: `window.LOOP_GAME='story-peace'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: story-peace" when finished.
