...
1. `web/site/games/sort-by-rule/index.html` — the game (single self-contained file)
2. `web/site/games/sort-by-rule/copy.md` — social captions
...
## The game: SORT BY RULE

- **Conditioned path (the hook):** taxonomy rabbit-holes — the Reddit thread classifying every sub-type of a thing with rigorous criteria. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Items drift in and you sort them into bins by a rule you must infer from the first few items. Each correct sort reveals more of the rule; each wrong sort gently returns the item. When all 20 items are correctly sorted the rule prints and the tray seals.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** shock-honesty — the rule was hiding in plain sight the whole time — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='sort-by-rule'` → loop-events.js → juice.js → share.js.
...
```
# SORT BY RULE — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty — the rule was hiding in plain sight the whole time angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sort-by-rule" when finished.
