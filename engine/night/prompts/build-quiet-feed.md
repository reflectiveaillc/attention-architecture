...
1. `web/site/games/quiet-feed/index.html` — the game (single self-contained file)
2. `web/site/games/quiet-feed/copy.md` — social captions
...
- **Conditioned path (the hook):** visible like/view count comparison. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A feed scrolls where every like and view count is blurred to '???' and only resolves if you linger. The game is to scroll PAST each post before the numbers resolve — letting the comparison stay unseen.
- **Engine:** calm. **Circuits:** 01-soft-rpe, 05-ease, 08-garden, 02-novelty.
...
- Includes in order: `window.LOOP_GAME='quiet-feed'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: quiet-feed" when finished.
