...
1. `web/site/games/badge-sit/index.html` — the game (single self-contained file)
2. `web/site/games/badge-sit/copy.md` — social captions
...
- **Conditioned path (the hook):** red notification badge compulsion. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Notification badges appear one by one — and you do NOTHING. Let them sit; each fades away on its own. Your score is how many you let dissolve without tapping.
...
- Includes in order: `window.LOOP_GAME='badge-sit'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: badge-sit" when finished.
