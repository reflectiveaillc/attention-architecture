...
1. `web/site/games/escape-tap/index.html` — the game (single self-contained file)
2. `web/site/games/escape-tap/copy.md` — social captions
...
- **Conditioned path (the hook):** escape-room / puzzle-solving videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap objects in a tiny room to find hidden clues. Each clue unlocks the next drawer; the key opens the door and the escape is complete.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 08-investment.
- **Video trick:** will they escape — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='escape-tap'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'rooms'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Escape Tap — copy
## TikTok (3 captions ≤100 chars, hook-first, will they escape angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: escape-tap" when finished.
