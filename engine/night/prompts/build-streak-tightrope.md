...
1. `web/site/games/streak-tightrope/index.html` — the game (single self-contained file)
2. `web/site/games/streak-tightrope/copy.md` — social captions
...
## The game: Streak Tightrope

- **Conditioned path (the hook):** streak anxiety. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Your streak number climbs automatically toward midnight. Tap once to 'send' and bank it, but the clock randomly speeds up or stalls. Miss the window and the number burns to zero.
...
- Includes in order: `window.LOOP_GAME='streak-tightrope'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'days'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: streak-tightrope" when finished.
