...
1. `web/site/games/dead-center/index.html` — the game (single self-contained file)
2. `web/site/games/dead-center/copy.md` — social captions
...
## The game: Dead Center

- **Conditioned path (the hook):** aim-lab / target-accuracy skill tutorials that make you itch to hit pixel-perfect. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A reticle shrinks toward the bullseye; tap when it covers the exact center pixel. Your PB is the closest center-distance in pixels — lower is better, and the margin tightens each level.
...
- Includes in order: `window.LOOP_GAME='dead-center'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: dead-center" when finished.
