...
1. `web/site/games/pipeline-trace/index.html` — the game (single self-contained file)
2. `web/site/games/pipeline-trace/copy.md` — social captions
...
## The game: PIPELINE TRACE

- **Conditioned path (the hook):** systems-diagram rabbit-holes — the flowchart and process-map content the algorithm feeds to pipeline-obsessed brains. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A tangle of pipe segments sits on the canvas; tap each segment to rotate it until water can flow from source to drain without leaks. Each connected segment glows; each completed path fills with a gentle flow animation. All paths flowing is the end state.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** completion-satisfaction — the system flows — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='pipeline-trace'` → loop-events.js → juice.js → share.js.
...
```
# PIPELINE TRACE — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the system flows angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pipeline-trace" when finished.
