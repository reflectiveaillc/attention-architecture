...
1. `web/site/games/poll-fever/index.html` — the game (single self-contained file)
2. `web/site/games/poll-fever/copy.md` — social captions
...
- **Conditioned path (the hook):** poll/vote anticipation. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Two poll bars race toward 50%. Tap when your chosen bar crosses the line. The bars jitter, stall, and reverse; the result stays uncertain until the final frame.
...
- Includes in order: `window.LOOP_GAME='poll-fever'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: poll-fever" when finished.
