...
1. `web/site/games/rhythm-repeat/index.html` — the game (single self-contained file)
2. `web/site/games/rhythm-repeat/copy.md` — social captions
...
## The game: RHYTHM REPEAT

- **Conditioned path (the hook):** drum-pattern / handclap recall challenges where the rhythm plants in short-term memory. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A tap-rhythm pattern plays as haptic pulses on screen; the thumb replays it by tapping. Pattern length grows each round. Miss-timing by more than 100ms ends the streak — sequence recall through the motor channel.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 07-timing.
- **Video trick:** participation-comment-bait — 'can you feel the pattern and send it back?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='rhythm-repeat'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'beats'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# RHYTHM REPEAT — copy
## TikTok (3 captions ≤100 chars, hook-first, participation-comment-bait — 'can you feel the pattern and send it back?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rhythm-repeat" when finished.
