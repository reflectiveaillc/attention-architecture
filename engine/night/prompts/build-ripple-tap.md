...
1. `web/site/games/ripple-tap/index.html` — the game (single self-contained file)
2. `web/site/games/ripple-tap/copy.md` — social captions
...
- **Conditioned path (the hook):** the refresh lever — pulling to trigger a predictable response because unpredictability is unbearable. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A still water surface fills the screen. Tap anywhere and a ripple expands outward perfectly, predictably, always the same. Every tap produces the exact same beautiful ring. No variation, no surprise, no score. The nervous system learns: tap, ripple, safe. Tap as many times as you want; there is no end state.
- **Engine:** calm. **Circuits:** 05-ease, 01-rpe, 07-timing.
- **Video trick:** calm completion — the perfect predictable ripple is the hook — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='ripple-tap'` → loop-events.js → juice.js → share.js.
...
```
# RIPPLE TAP — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the perfect predictable ripple is the hook angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: ripple-tap" when finished.
