...
1. `web/site/games/active-dot/index.html` — the game (single self-contained file)
2. `web/site/games/active-dot/copy.md` — social captions
...
- **Conditioned path (the hook):** seen-receipt / active-now green dot (the anticipation of catching someone online). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A DM-style contact list scrolls; most dots are grey, but one flickers green for a brief window. Tap the green dot in its window to 'catch them online' — miss it and they go back to grey. The green windows get shorter and rarer.
...
- Includes in order: `window.LOOP_GAME='active-dot'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'catches'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: active-dot" when finished.
