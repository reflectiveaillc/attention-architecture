...
1. `web/site/games/viral-dare/index.html` — the game (single self-contained file)
2. `web/site/games/viral-dare/copy.md` — social captions
...
- **Conditioned path (the hook):** watching a post's view count explode. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** You post a dare; every friend who opens the link adds to a view counter. The goal is to make the dare 'go viral' among your friends within a single day, then the link dies — a bounded popularity contest.
...
- Includes in order: `window.LOOP_GAME='viral-dare'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'views'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: viral-dare" when finished.
