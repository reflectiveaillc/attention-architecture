...
1. `web/site/games/quiet-reveal/index.html` — the game (single self-contained file)
2. `web/site/games/quiet-reveal/copy.md` — social captions
...
## The game: Quiet Reveal

- **Conditioned path (the hook):** satisfying reveal videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Swipe to peel translucent layers off a soft shape. Each layer reveals a warmer tone until the center object rests fully exposed; no timer, just the closed loop of the completed image.
...
- Includes in order: `window.LOOP_GAME='quiet-reveal'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: quiet-reveal" when finished.
