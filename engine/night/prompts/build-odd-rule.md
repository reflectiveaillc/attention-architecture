...
1. `web/site/games/odd-rule/index.html` — the game (single self-contained file)
2. `web/site/games/odd-rule/copy.md` — social captions
...
- **Conditioned path (the hook):** spot-the-difference / 'which one doesn't belong' trend clips. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Spot the odd tile in a grid — but the definition of 'odd' changes each round: odd color, then odd shape, then odd count, then odd rotation. You re-discover the rule every 5s, exactly the pattern-hunt the FYP feeds. 15 rounds, then done.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 06-zeigarnik.
- **Video trick:** play-along comment bait — 'can you spot it before round 8?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='odd-rule'` → loop-events.js → juice.js → share.js.
...
```
# ODD RULE — copy
## TikTok (3 captions ≤100 chars, hook-first, play-along comment bait — 'can you spot it before round 8?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: odd-rule" when finished.
