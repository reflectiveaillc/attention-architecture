...
1. `web/site/games/verified-chase/index.html` — the game (single self-contained file)
2. `web/site/games/verified-chase/copy.md` — social captions
...
## The game: Verified Chase

- **Conditioned path (the hook):** verified blue-badge aspiration. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A checklist of criteria (profile pic, bio, posts, follower threshold) appears; tap each requirement into place in the right order before the review window closes to earn the blue check.
- **Engine:** viral. **Circuits:** 03-social, 06-zeigarnik, 08-investment.
...
- Includes in order: `window.LOOP_GAME='verified-chase'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'criteria'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: verified-chase" when finished.
