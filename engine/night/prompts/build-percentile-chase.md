...
1. `web/site/games/percentile-chase/index.html` — the game (single self-contained file)
2. `web/site/games/percentile-chase/copy.md` — social captions
...
## The game: Percentile Chase

- **Conditioned path (the hook):** percentile-rank envy on analytics dashboards. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** One 90-second run. At the end the game shows your percentile against everyone who played today. Cross the 90th percentile and it generates a dare-link to challenge the next player above you.
...
- Includes in order: `window.LOOP_GAME='percentile-chase'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: percentile-chase" when finished.
