...
1. `web/site/games/rule-reveal/index.html` — the game (single self-contained file)
2. `web/site/games/rule-reveal/copy.md` — social captions
...
## The game: RULE REVEAL

- **Conditioned path (the hook):** niche deep-dive rabbit-holes where the algorithm rewards hyper-specific rule-hunting without ever surfacing the actual rule. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A sequence of colored shapes appears; tap the next shape in the pattern before the blank slot fades. The rule is never stated — you discover it by watching the sequence repeat and vary. When you've correctly predicted 10 in a row the rule surfaces as text and the session closes.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 01-rpe, 05-ease.
- **Video trick:** curiosity gap — the rule is right there if you look close enough — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='rule-reveal'` → loop-events.js → juice.js → share.js.
...
```
# RULE REVEAL — copy
## TikTok (3 captions ≤100 chars, hook-first, curiosity gap — the rule is right there if you look close enough angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rule-reveal" when finished.
