...
1. `web/site/games/final-bar/index.html` — the game (single self-contained file)
2. `web/site/games/final-bar/copy.md` — social captions
...
- **Conditioned path (the hook):** endless loop tracks engineered to never signal that the song is over. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A short musical phrase plays and you tap along gently; after exactly 8 bars a resolution chord lands and the screen fades to soft black. Nothing loops. The whole game is the satisfaction of an 8-bar phrase that actually ends — flow state, then done.
...
- Includes in order: `window.LOOP_GAME='final-bar'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: final-bar" when finished.
