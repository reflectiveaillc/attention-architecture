...
1. `web/site/games/counting-sheep/index.html` — the game (single self-contained file)
2. `web/site/games/counting-sheep/copy.md` — social captions
...
## The game: Counting Sheep

- **Conditioned path (the hook):** the autoplay loop you can't close, replaced by the oldest wind-down ritual. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Sheep drift one at a time over a fence; tap each as it leaps to count it. Every counted sheep slows the next one's drift and darkens the sky a shade. After the last sheep the field is black and the session closes — a loop with a built-in finish line.
- **Engine:** calm. **Circuits:** 02-novelty, 07-timing, 06-zeigarnik.
- **Video trick:** calm completion — the ritual that actually ends — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='counting-sheep'` → loop-events.js → juice.js → share.js.
...
```
# Counting Sheep — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the ritual that actually ends angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: counting-sheep" when finished.
