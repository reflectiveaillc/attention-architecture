...
1. `web/site/games/poll-no-stakes/index.html` — the game (single self-contained file)
2. `web/site/games/poll-no-stakes/copy.md` — social captions
...
## The game: POLL NO STAKES

- **Conditioned path (the hook):** story sticker poll (the two-option vote, without majority pressure or a wrong answer). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A gentle poll appears ('tea or coffee', 'morning or night'); tap either option and the bar softly reveals both sides — no winner, no score, just your pick noted. Each poll fades into the next at a slow, even pace. Opinion without competition.
- **Engine:** calm. **Circuits:** 05-ease, 03-social.
...
- Includes in order: `window.LOOP_GAME='poll-no-stakes'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: poll-no-stakes" when finished.
