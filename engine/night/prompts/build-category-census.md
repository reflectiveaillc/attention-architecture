...
1. `web/site/games/category-census/index.html` — the game (single self-contained file)
2. `web/site/games/category-census/copy.md` — social captions
...
## The game: CATEGORY CENSUS

- **Conditioned path (the hook):** classification rabbit-holes — the thread sorting every entry of a thing into a precise taxonomy with argued edge cases. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Items drop from above; tap the correct category bin for each. The bins are labelled but the items are ambiguous — a few rounds reveal which edge cases belong where. Each correctly categorised item clicks into its bin; a wrong bin gently bounces it back. 30 items, then the census is complete.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 01-rpe.
- **Video trick:** shock-honesty — that edge case belonged in the other bin all along — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='category-census'` → loop-events.js → juice.js → share.js.
...
```
# CATEGORY CENSUS — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty — that edge case belonged in the other bin all along angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: category-census" when finished.
