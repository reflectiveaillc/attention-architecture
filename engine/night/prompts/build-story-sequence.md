...
1. `web/site/games/story-sequence/index.html` — the game (single self-contained file)
2. `web/site/games/story-sequence/copy.md` — social captions
...
## The game: STORY SEQUENCE

- **Conditioned path (the hook):** 'put this story in order' narrative-recall posts that spread on nostalgia for shared moments. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Four illustrated story panels flash in order for 2 seconds each; they scramble and the thumb drags them back into the correct narrative sequence. Error count is zero or greater — a perfect run is the only dare worth sharing.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 03-social.
- **Video trick:** completion-satisfaction — the clean reorder lands like a solved puzzle — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='story-sequence'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'errors'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# STORY SEQUENCE — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the clean reorder lands like a solved puzzle angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: story-sequence" when finished.
