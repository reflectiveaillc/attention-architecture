...
1. `web/site/games/mystery-box-stack/index.html` — the game (single self-contained file)
2. `web/site/games/mystery-box-stack/copy.md` — social captions
...
## The game: Mystery Box Stack

- **Conditioned path (the hook):** unboxing videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap each box in a vertical stack to unwrap it; every box holds a clue tile that narrows the final item. Open the last box to solve the mystery and bank the set.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 08-investment.
- **Video trick:** unboxing payoff — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='mystery-box-stack'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'boxes'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Mystery Box Stack — copy
## TikTok (3 captions ≤100 chars, hook-first, unboxing payoff angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: mystery-box-stack" when finished.
