...
1. `web/site/games/seen-wait/index.html` — the game (single self-contained file)
2. `web/site/games/seen-wait/copy.md` — social captions
...
- **Conditioned path (the hook):** seen-receipt anticipation in DMs. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap to send a DM, then watch the typing dots; the instant the 'seen' stamp flashes, tap to claim it — but tap BEFORE it appears and you've double-texted and lost the round.
...
- Includes in order: `window.LOOP_GAME='seen-wait'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: seen-wait" when finished.
