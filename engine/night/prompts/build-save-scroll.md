...
1. `web/site/games/save-scroll/index.html` — the game (single self-contained file)
2. `web/site/games/save-scroll/copy.md` — social captions
...
## The game: Save Scroll

- **Conditioned path (the hook):** save/bookmark compulsion. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Precious items flicker past in a feed. Swipe right to save, left to skip. The catch: you have only three save slots and the best items appear when your slots are full, forcing a painful curation choice.
- **Engine:** viral. **Circuits:** 04-loss, 08-investment, 01-rpe.
...
- Includes in order: `window.LOOP_GAME='save-scroll'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'saves'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: save-scroll" when finished.
