...
1. `web/site/games/like-guess/index.html` — the game (single self-contained file)
2. `web/site/games/like-guess/copy.md` — social captions
...
- **Conditioned path (the hook):** like-count hiding (the IG 'hidden likes' curiosity that makes you want to know the number). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A post appears with its like-count blurred out; a slider lets you guess the number, then the blur clears. Score by how close your guess lands — within 1% is a perfect. Streak across posts; the counts get weirder each round.
- **Engine:** viral. **Circuits:** 02-novelty, 06-zeigarnik, 03-social.
...
- Includes in order: `window.LOOP_GAME='like-guess'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: like-guess" when finished.
