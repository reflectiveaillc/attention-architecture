...
1. `web/site/games/ghost-beat/index.html` — the game (single self-contained file)
2. `web/site/games/ghost-beat/copy.md` — social captions
...
- **Conditioned path (the hook):** layered loop tracks that add invisible percussion hits the ear barely registers. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A beat plays with one ghost hit hidden in each bar — no visual cue, only ear. Tap when you feel the ghost percussion. Score is how many ghost hits you catch across 16 bars. Trains the ear the loop has been hijacking.
...
- Includes in order: `window.LOOP_GAME='ghost-beat'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'ghosts'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: ghost-beat" when finished.
