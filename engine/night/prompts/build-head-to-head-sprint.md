...
1. `web/site/games/head-to-head-sprint/index.html` — the game (single self-contained file)
2. `web/site/games/head-to-head-sprint/copy.md` — social captions
...
## The game: Head-to-Head Sprint

- **Conditioned path (the hook):** live comment-section speed battles. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Two players open the same dare-link and race split-screen for 45 seconds on identical targets. The winner keeps a session-only crown sticker; the loser sees 'next time' and the session closes.
...
- Includes in order: `window.LOOP_GAME='head-to-head-sprint'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: head-to-head-sprint" when finished.
