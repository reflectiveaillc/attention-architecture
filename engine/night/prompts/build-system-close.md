...
1. `web/site/games/system-close/index.html` — the game (single self-contained file)
2. `web/site/games/system-close/copy.md` — social captions
...
## The game: SYSTEM CLOSE

- **Conditioned path (the hook):** system-mastery niche loops — the deep-dive that pulls you in because you need to see the complete system before you can stop. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 5-stage machine shows its gears; each stage has a hidden trigger condition. Discover and satisfy each condition in order — the machine advances one stage at a time with a satisfying mechanical click. All 5 stages triggered, the machine runs a full cycle and prints SYSTEM COMPLETE. Share your run count.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 08-investment.
- **Video trick:** completion-satisfaction — the machine runs its first clean cycle — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='system-close'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'runs'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# SYSTEM CLOSE — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the machine runs its first clean cycle angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: system-close" when finished.
