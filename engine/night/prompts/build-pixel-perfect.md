...
1. `web/site/games/pixel-perfect/index.html` — the game (single self-contained file)
2. `web/site/games/pixel-perfect/copy.md` — social captions
...
## The game: Pixel Perfect

- **Conditioned path (the hook):** pixel art / perfect-block placement clips where one wrong pixel ruins the piece. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap to place a pixel at exact coordinates on a grid. The grid zooms in and tolerance shrinks each level. PB is the highest level reached with zero misplaced pixels.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 05-ease.
- **Video trick:** pixel-perfect flex — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='pixel-perfect'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'level'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Pixel Perfect — copy
## TikTok (3 captions ≤100 chars, hook-first, pixel-perfect flex angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pixel-perfect" when finished.
