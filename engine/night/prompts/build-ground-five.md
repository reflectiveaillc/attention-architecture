...
1. `web/site/games/ground-five/index.html` — the game (single self-contained file)
2. `web/site/games/ground-five/copy.md` — social captions
...
## The game: GROUND FIVE

- **Conditioned path (the hook):** the sensory-scan doomscroll — scrolling to find something that makes the body feel present and safe. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Five sense prompts appear one at a time — SEE, HEAR, FEEL, SMELL, TASTE — each on a soft card. Tap the card to confirm you've noticed that sense right now. Each confirmed tap fills a gentle arc. When all five arcs are full the screen glows warmly for three seconds and closes. The 5-4-3-2-1 grounding technique as a one-thumb session.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 07-timing.
- **Video trick:** calm completion — the final warm glow when all five senses are named — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='ground-five'` → loop-events.js → juice.js → share.js.
...
```
# GROUND FIVE — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the final warm glow when all five senses are named angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: ground-five" when finished.
