...
1. `web/site/games/comment-race/index.html` — the game (single self-contained file)
2. `web/site/games/comment-race/copy.md` — social captions
...
## The game: Comment Race

- **Conditioned path (the hook):** comment-section speed. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Comments fly upward like a live comment section. Tap the ones that match a hidden keyword before they scroll past. The faster the thread, the more the speed-reading brain locks in.
...
- Includes in order: `window.LOOP_GAME='comment-race'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'comments'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: comment-race" when finished.
