...
1. `web/site/games/bpm-duel/index.html` — the game (single self-contained file)
2. `web/site/games/bpm-duel/copy.md` — social captions
...
- **Conditioned path (the hook):** viral 'who has better rhythm' challenge clips where two people race the same beat. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A shared beat plays; your accuracy % after 32 taps is your dare score. Send the link with your score visible — the receiver sees your bar when they start, daring them to beat it. One song, one minute, settled.
...
- Includes in order: `window.LOOP_GAME='bpm-duel'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: bpm-duel" when finished.
