...
1. `web/site/games/power-wash-tile/index.html` — the game (single self-contained file)
2. `web/site/games/power-wash-tile/copy.md` — social captions
...
## The game: POWER WASH TILE

- **Conditioned path (the hook):** pressure-washing and power-wash ASMR that never fully cleans the last corner. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A dirty tiled floor fills the canvas; drag your thumb slowly across it to reveal the clean surface underneath. The percentage cleaned ticks upward in the corner; when it hits 100% the final reveal blooms and the session ends.
...
- Includes in order: `window.LOOP_GAME='power-wash-tile'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: power-wash-tile" when finished.
