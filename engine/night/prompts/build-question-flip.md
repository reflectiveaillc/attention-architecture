...
1. `web/site/games/question-flip/index.html` — the game (single self-contained file)
2. `web/site/games/question-flip/copy.md` — social captions
...
## The game: Question Flip

- **Conditioned path (the hook):** answer in the comments" flip-card posts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap cards to flip them; one hides the answer to the riddle at the top. Find the answer before you run out of flips and the card locks with the payoff.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 04-loss.
- **Video trick:** the answer is behind one card — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='question-flip'` → loop-events.js → juice.js → share.js.
...
```
# Question Flip — copy
## TikTok (3 captions ≤100 chars, hook-first, the answer is behind one card angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: question-flip" when finished.
