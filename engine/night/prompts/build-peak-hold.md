...
1. `web/site/games/peak-hold/index.html` — the game (single self-contained file)
2. `web/site/games/peak-hold/copy.md` — social captions
...
- **Conditioned path (the hook):** breath-hold / plank PB challenges where releasing at the right instant is everything. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold your thumb down while a meter rises to a green target zone, then release at the exact peak. Early or late costs distance from perfect. PB is the closest peak release in %.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 07-timing.
- **Video trick:** threshold PB bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='peak-hold'` → loop-events.js → juice.js → share.js.
...
```
# Peak Hold — copy
## TikTok (3 captions ≤100 chars, hook-first, threshold PB bait angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: peak-hold" when finished.
