...
1. `web/site/games/split-sort/index.html` — the game (single self-contained file)
2. `web/site/games/split-sort/copy.md` — social captions
...
- **Conditioned path (the hook):** rapid-fire 'left or right' sorting and reaction-test videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Shapes slide down the center; tap left or right to sort them into matching zones before they cross the deadline. The stream gets faster and shapes become more ambiguous.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 05-ease.
- **Video trick:** play-along bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='split-sort'` → loop-events.js → juice.js → share.js.
...
```
# SPLIT SORT — copy
## TikTok (3 captions ≤100 chars, hook-first, play-along bait angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: split-sort" when finished.
