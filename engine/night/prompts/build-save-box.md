...
1. `web/site/games/save-box/index.html` — the game (single self-contained file)
2. `web/site/games/save-box/copy.md` — social captions
...
- **Conditioned path (the hook):** saved/bookmark posts (the save-for-later ribbon, without the storage scarcity). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Soft cards drift across the screen; tap a card to lift it gently into a keepsake box at the bottom. There is no capacity limit and no urgency — each save lands with a quiet chime. Keep what matters; the box fills at whatever pace you choose.
...
- Includes in order: `window.LOOP_GAME='save-box'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: save-box" when finished.
