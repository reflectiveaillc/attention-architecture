...
1. `web/site/games/tap-twin/index.html` — the game (single self-contained file)
2. `web/site/games/tap-twin/copy.md` — social captions
...
- **Conditioned path (the hook):** split-screen comparison clips that spark a tribal 'which side is you?' engagement loop. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Two rhythm tracks play simultaneously at different tempos; tap along with whichever you choose. Your locked-in tempo vs. the one you didn't pick is the rivalry score. Share the result with your twin and see whose rhythm held.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 07-timing.
...
- Includes in order: `window.LOOP_GAME='tap-twin'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tap-twin" when finished.
