...
1. `web/site/games/plan-the-day/index.html` — the game (single self-contained file)
2. `web/site/games/plan-the-day/copy.md` — social captions
...
## The game: PLAN THE DAY

- **Conditioned path (the hook):** the productive-scroll loop — consuming planning content to feel prepared without actually committing to anything. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A blank day-grid of six soft time-blocks sits on screen. Drag a pre-made activity tile (rest, move, eat, connect, work, play) into any block. No wrong arrangement, no judgment. Swap tiles freely — all moves reversible. When you tap DONE the grid softly locks, the session closes, and the day has a shape. Control imposed on uncertainty.
- **Engine:** calm. **Circuits:** 05-ease, 08-investment, 06-zeigarnik.
- **Video trick:** calm completion — the locked grid, the day with a visible shape — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='plan-the-day'` → loop-events.js → juice.js → share.js.
...
```
# PLAN THE DAY — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the locked grid, the day with a visible shape angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: plan-the-day" when finished.
