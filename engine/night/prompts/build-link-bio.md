...
1. `web/site/games/link-bio/index.html` — the game (single self-contained file)
2. `web/site/games/link-bio/copy.md` — social captions
...
## The game: Link in Bio

- **Conditioned path (the hook):** 'link in bio' friction. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A bio link loads with a progress bar. Tap exactly when it hits 100% before the page 'redirects' to a fake ad. The bar stutters, backtracks, and pauses to punish rushing.
- **Engine:** viral. **Circuits:** 05-ease, 06-zeigarnik, 01-rpe.
...
- Includes in order: `window.LOOP_GAME='link-bio'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: link-bio" when finished.
