...
1. `web/site/games/mention-dodge/index.html` — the game (single self-contained file)
2. `web/site/games/mention-dodge/copy.md` — social captions
...
## The game: Mention Dodge

- **Conditioned path (the hook):** @-mention / tagging in spreading threads. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Your @handle gets flung into the conversation from both sides; tap to block each incoming mention before it sticks to your name — one that lands and the dogpile begins.
- **Engine:** viral. **Circuits:** 03-social, 04-loss, 05-ease.
...
- Includes in order: `window.LOOP_GAME='mention-dodge'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'mentions blocked'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: mention-dodge" when finished.
