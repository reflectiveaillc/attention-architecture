...
1. `web/site/games/stack-level/index.html` — the game (single self-contained file)
2. `web/site/games/stack-level/copy.md` — social captions
...
## The game: Stack Level

- **Conditioned path (the hook):** perfect-stacking / Jenga / building tutorials where one misaligned block ruins the tower. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Blocks swing in from the sides; tap to drop each one flush on the stack. Misalignment accumulates until the tower topples. PB is the tallest perfectly aligned tower.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 08-investment.
- **Video trick:** tower PB chase — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='stack-level'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'blocks'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Stack Level — copy
## TikTok (3 captions ≤100 chars, hook-first, tower PB chase angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: stack-level" when finished.
