...
1. `web/site/games/match-the-gap/index.html` — the game (single self-contained file)
2. `web/site/games/match-the-gap/copy.md` — social captions
...
## The game: MATCH THE GAP

- **Conditioned path (the hook):** spacing-check and whitespace-OCD in design tutorials that zoom in on uneven margins. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Three objects sit in a row with one gap visibly wrong; drag the middle object left or right until the two gaps are exactly equal. When the gaps match within a pixel, the objects lock and the uneven tension releases into a satisfied chime.
...
- Includes in order: `window.LOOP_GAME='match-the-gap'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: match-the-gap" when finished.
