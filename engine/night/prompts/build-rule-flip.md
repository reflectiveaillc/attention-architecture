...
1. `web/site/games/rule-flip/index.html` — the game (single self-contained file)
2. `web/site/games/rule-flip/copy.md` — social captions
...
- **Conditioned path (the hook):** FYP format-flipping — the feed that re-novelties itself by swapping layouts (text → photo → video → carousel) before you habituate. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap the green tile, dodge the red — except every 4 seconds the rule inverts: now tap red, dodge green, then tap nothing, then tap both. The brain that craves novelty gets it from a rule that keeps flipping, not an endless feed. Survive 10 flips and it ENDS.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 05-ease.
- **Video trick:** rule-inversion bait — 'wait, it just flipped again' keeps the hook clip re-watching — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='rule-flip'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'flips'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# RULE FLIP — copy
## TikTok (3 captions ≤100 chars, hook-first, rule-inversion bait — 'wait, it just flipped again' keeps the hook clip re-watching angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rule-flip" when finished.
