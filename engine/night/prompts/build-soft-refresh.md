...
1. `web/site/games/soft-refresh/index.html` — the game (single self-contained file)
2. `web/site/games/soft-refresh/copy.md` — social captions
...
- **Mechanic:** Pull the screen down slowly. The further you pull, the slower your breath is guided. Release inside the calm zone to plant a seed. No score; the feedback is a garden tile that appears once per day.
- **Engine:** calm. **Circuits:** 01-soft-rpe, 05-ease, 07-timing, 08-garden.
...
- Includes in order: `window.LOOP_GAME='soft-refresh'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: soft-refresh" when finished.
