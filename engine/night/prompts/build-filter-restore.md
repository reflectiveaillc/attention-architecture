...
1. `web/site/games/filter-restore/index.html` — the game (single self-contained file)
2. `web/site/games/filter-restore/copy.md` — social captions
...
## The game: FILTER RESTORE

- **Conditioned path (the hook):** filter face-transform (the same beauty-filter conditioning, inverted toward the real face). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A face starts buried under stacked filters (smooth, saturated, enlarged). Tap gently to peel one filter layer at a time, breathing back toward the unedited face. When the natural face is fully restored, the screen softly settles and the session ends.
- **Engine:** calm. **Circuits:** 05-ease, 01-soft-rpe, 08-garden.
...
- Includes in order: `window.LOOP_GAME='filter-restore'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: filter-restore" when finished.
