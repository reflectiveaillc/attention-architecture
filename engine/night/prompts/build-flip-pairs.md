...
1. `web/site/games/flip-pairs/index.html` — the game (single self-contained file)
2. `web/site/games/flip-pairs/copy.md` — social captions
...
- **Conditioned path (the hook):** nostalgia-bait photo pairs ('here vs now') that farm the match-recognition reflex. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A grid of face-down cards; flip two at a time to find matching pairs. All cards are cleared in one session — the score is total flip count, lower is better, daring friends to beat the memory efficiency.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 03-social.
- **Video trick:** competitive rank — 'I cleared it in 18 flips, beat that' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='flip-pairs'` → loop-events.js → juice.js → share.js.
...
```
# FLIP PAIRS — copy
## TikTok (3 captions ≤100 chars, hook-first, competitive rank — 'I cleared it in 18 flips, beat that' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: flip-pairs" when finished.
