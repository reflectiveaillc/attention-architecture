...
1. `web/site/games/tag-everyone/index.html` — the game (single self-contained file)
2. `web/site/games/tag-everyone/copy.md` — social captions
...
## The game: TAG EVERYONE

- **Conditioned path (the hook):** tagging people in photos (the IG compulsion to tag every face before the post feels complete). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A group photo shows faces as glowing target dots; tap each face to tag it before the upload timer runs out. Missed faces are 'untagged' and the post stays incomplete — tag them all for a clean upload, the count of faces grows each round.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 03-social, 01-rpe.
- **Video trick:** near-perfection itch — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='tag-everyone'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'tagged'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tag-everyone" when finished.
