...
1. `web/site/games/stamp-rally/index.html` — the game (single self-contained file)
2. `web/site/games/stamp-rally/copy.md` — social captions
...
## The game: STAMP RALLY

- **Conditioned path (the hook):** loyalty-stamp cards and punch-card FOMO that dangle the free reward one stamp away. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Stamp icons appear one at a time on a 3x3 card; tap each before it fades to claim it. Stamp all 9 to redeem the card and see the full set revealed. The card has exactly 9 slots and the deck is finite at 3 cards total — 27 taps and it’s done.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 04-loss.
- **Video trick:** completion-satisfaction — the final stamp, the card complete — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='stamp-rally'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'stamps'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: stamp-rally" when finished.
