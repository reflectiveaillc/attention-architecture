...
1. `web/site/games/tempo-tether/index.html` — the game (single self-contained file)
2. `web/site/games/tempo-tether/copy.md` — social captions
...
## The game: Tempo Tether

- **Conditioned path (the hook):** the rapid-cut hype edits that clock your resting heart rate up. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A soft orb pulses on a beat; tap as it peaks. Each clean tap lengthens the interval to the next pulse and dims the screen a notch. You are literally slowing your own tempo — by the end the beat is once every few seconds and the screen is black.
- **Engine:** calm. **Circuits:** 01-rpe, 07-timing, 05-ease.
- **Video trick:** calm completion — you slow the beat yourself — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='tempo-tether'` → loop-events.js → juice.js → share.js.
...
```
# Tempo Tether — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — you slow the beat yourself angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tempo-tether" when finished.
