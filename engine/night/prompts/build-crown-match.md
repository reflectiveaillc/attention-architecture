...
1. `web/site/games/crown-match/index.html` — the game (single self-contained file)
2. `web/site/games/crown-match/copy.md` — social captions
...
## The game: Crown Match

- **Conditioned path (the hook):** pinned top-comment / top-donator crown status. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A single head-to-head duel: tap rapidly to keep a crown in your half of the screen. After 30 seconds the crown holder wins and earns a session-only crown emoji to stamp the dare-link.
...
- Includes in order: `window.LOOP_GAME='crown-match'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'crowns'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: crown-match" when finished.
