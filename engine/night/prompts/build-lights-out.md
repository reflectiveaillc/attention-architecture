...
1. `web/site/games/lights-out/index.html` — the game (single self-contained file)
2. `web/site/games/lights-out/copy.md` — social captions
...
- **Conditioned path (the hook):** the lit-room / blue-light exposure that keeps the brain reading 'daytime'. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A grid of soft glowing lights fills the screen; tap each to switch it off. Every light extinguished dims the whole screen and lowers a background hum. Turn them all off and the room is dark — the last light off is the session end.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 07-timing.
- **Video trick:** calm completion — darken the room one light at a time — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='lights-out'` → loop-events.js → juice.js → share.js.
...
```
# Lights Out — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — darken the room one light at a time angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: lights-out" when finished.
