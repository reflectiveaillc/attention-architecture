...
1. `web/site/games/prime-sieve/index.html` — the game (single self-contained file)
2. `web/site/games/prime-sieve/copy.md` — social captions
...
## The game: PRIME SIEVE

- **Conditioned path (the hook):** number-pattern rabbit-holes — the maths-nerd thread walking through elegant integer sequences. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A grid of numbers 1-50 lights up one by one; tap each composite to grey it out, leaving only primes glowing. There is no timer — just you and the sieve of Eratosthenes, a 2,000-year-old rule you discover (or remember) at your own pace. When all composites are greyed the primes pulse in unison.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** completion-satisfaction — the primes emerge from the noise — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='prime-sieve'` → loop-events.js → juice.js → share.js.
...
```
# PRIME SIEVE — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the primes emerge from the noise angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: prime-sieve" when finished.
