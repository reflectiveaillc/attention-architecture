...
1. `web/site/games/follower-face-off/index.html` — the game (single self-contained file)
2. `web/site/games/follower-face-off/copy.md` — social captions
...
## The game: Follower Face-Off

- **Conditioned path (the hook):** follower-count milestone chasing. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** You and a friend each get 60 seconds to accept the right follower notifications and reject bots. First to the milestone wins; the dare-link encodes both scores and a single winner flag.
...
- Includes in order: `window.LOOP_GAME='follower-face-off'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: follower-face-off" when finished.
