...
1. `web/site/games/typing-standoff/index.html` — the game (single self-contained file)
2. `web/site/games/typing-standoff/copy.md` — social captions
...
## The game: Typing Standoff

- **Conditioned path (the hook):** 'typing…' indicator anticipation. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A typing bubble pulses in the chat window. Tap the instant the dots vanish and a message appears. Tap early and you look desperate; tap late and the conversation dies. Each round the pause grows longer.
...
- Includes in order: `window.LOOP_GAME='typing-standoff'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: typing-standoff" when finished.
