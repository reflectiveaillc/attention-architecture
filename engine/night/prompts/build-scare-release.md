...
1. `web/site/games/scare-release/index.html` — the game (single self-contained file)
2. `web/site/games/scare-release/copy.md` — social captions
...
## The game: SCARE RELEASE

- **Conditioned path (the hook):** jump-scare videos that train the viewer to flinch. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold your thumb down during a quiet buildup; release exactly when the jump scare triggers. Release early and you chicken out; release late and the scare already got you.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 07-timing.
- **Video trick:** jump-scare bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='scare-release'` → loop-events.js → juice.js → share.js.
...
```
# SCARE RELEASE — copy
## TikTok (3 captions ≤100 chars, hook-first, jump-scare bait angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: scare-release" when finished.
