...
1. `web/site/games/sequence-echo/index.html` — the game (single self-contained file)
2. `web/site/games/sequence-echo/copy.md` — social captions
...
## The game: SEQUENCE ECHO

- **Conditioned path (the hook):** 'only 90s kids remember' nostalgia-chain posts that bait recall of childhood sequences. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A colored tile sequence flashes one by one; the thumb must replay the sequence in exact order by tapping the same tiles. Each correct echo adds one more tile to the next round, growing the span until a miss ends the run and prints the final sequence length.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 04-loss.
- **Video trick:** near-perfection itch — the last tile missed haunts the rewatch — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='sequence-echo'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'tiles'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# SEQUENCE ECHO — copy
## TikTok (3 captions ≤100 chars, hook-first, near-perfection itch — the last tile missed haunts the rewatch angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sequence-echo" when finished.
