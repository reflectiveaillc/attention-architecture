...
1. `web/site/games/spot-the-sponsored/index.html` — the game (single self-contained file)
2. `web/site/games/spot-the-sponsored/copy.md` — social captions
...
- **Conditioned path (the hook):** trained ad-detection reflex (years of sponsored-post pattern recognition). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A fake feed scrolls by; one post per round is an ad, disguised progressively more natively. Tap it the instant you spot it — tapping an organic post ends the run. Rounds escalate from obvious #ad to influencer-native camouflage.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 03-social.
- **Video trick:** participation-comment-bait (freeze on round 7: can YOU spot it? pause now) — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='spot-the-sponsored'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'rounds'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# SPONSORED — copy
## TikTok (3 captions ≤100 chars, hook-first, participation-comment-bait (freeze on round 7: can YOU spot it? pause now) angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: spot-the-sponsored" when finished.
