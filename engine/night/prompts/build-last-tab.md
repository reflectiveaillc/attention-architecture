...
1. `web/site/games/last-tab/index.html` — the game (single self-contained file)
2. `web/site/games/last-tab/copy.md` — social captions
...
- **Conditioned path (the hook):** the 2am tab-hoarding rabbit hole — one more article, one more wiki. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Browser-style tabs stack on screen; swipe each one gently upward to close it. Each closed tab dims the screen and quiets the ambient sound. Close them all and the browser is empty and black — the open-loop tab hoard resolved to zero.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 08-investment, 05-ease.
- **Video trick:** calm completion — close every loop you opened — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='last-tab'` → loop-events.js → juice.js → share.js.
...
```
# Last Tab — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — close every loop you opened angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: last-tab" when finished.
