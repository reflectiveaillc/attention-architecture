...
1. `web/site/games/gate-snap/index.html` — the game (single self-contained file)
2. `web/site/games/gate-snap/copy.md` — social captions
...
- **Conditioned path (the hook):** CapCut 'shape-match' transition trends that close around the subject. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A shrinking gate closes around a silhouette; tap exactly when the gate matches the outline. The gate jitters and fake silhouettes flash to bait bad taps.
...
- Includes in order: `window.LOOP_GAME='gate-snap'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: gate-snap" when finished.
