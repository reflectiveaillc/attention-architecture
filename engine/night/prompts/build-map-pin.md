...
1. `web/site/games/map-pin/index.html` — the game (single self-contained file)
2. `web/site/games/map-pin/copy.md` — social captions
...
- **Conditioned path (the hook):** travel map collection content where every new pin signals a place you haven’t been. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A simple world-map outline sits on screen with 12 destination circles unmarked. Small destination cards drift in; tap each card then tap its matching circle on the map to pin it. Pin all 12 and the map holds the complete set — every pin placed, the journey closed.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** calm completion — every pin placed, the map full — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='map-pin'` → loop-events.js → juice.js → share.js.
...
```
# MAP PIN — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — every pin placed, the map full angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: map-pin" when finished.
