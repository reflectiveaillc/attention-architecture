...
1. `web/site/games/texture-tap/index.html` — the game (single self-contained file)
2. `web/site/games/texture-tap/copy.md` — social captions
...
## The game: TEXTURE TAP

- **Conditioned path (the hook):** random-texture ASMR scrolls where each new surface delivers a fresh tactile-audio hit before the feed moves on. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A new procedurally-generated surface fills the screen every fifteen seconds: corduroy, velvet, bark, mesh. Tap anywhere to hear and feel the texture respond with unique synthesized audio. Five textures complete the session.
- **Engine:** calm. **Circuits:** 02-novelty, 05-ease, 01-rpe.
...
- Includes in order: `window.LOOP_GAME='texture-tap'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: texture-tap" when finished.
