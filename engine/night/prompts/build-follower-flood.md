...
1. `web/site/games/follower-flood/index.html` — the game (single self-contained file)
2. `web/site/games/follower-flood/copy.md` — social captions
...
## The game: Follower Flood

- **Conditioned path (the hook):** 'X started following you' notification flood. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Follow notifications stream up from the bottom; tap each to accept before it scrolls off-screen — every missed accept costs your follower total, which climbs toward a milestone.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 08-investment.
- **Video trick:** benchmark — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='follower-flood'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'followers'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Follower Flood — copy
## TikTok (3 captions ≤100 chars, hook-first, benchmark angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: follower-flood" when finished.
