...
1. `web/site/games/deflect-zone/index.html` — the game (single self-contained file)
2. `web/site/games/deflect-zone/copy.md` — social captions
...
## The game: DEFLECT ZONE

- **Conditioned path (the hook):** deflection / 'catch and throw' reflex videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A projectile enters from the top; tap when it crosses the glowing deflection zone to bat it back. The zone shrinks and the projectile speed rises each round.
...
- Includes in order: `window.LOOP_GAME='deflect-zone'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'deflects'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: deflect-zone" when finished.
