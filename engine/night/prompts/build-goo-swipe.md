...
1. `web/site/games/goo-swipe/index.html` — the game (single self-contained file)
2. `web/site/games/goo-swipe/copy.md` — social captions
...
- **Conditioned path (the hook):** slime-wiping and putty-spread ASMR where dragging a slow streak of goo across a surface is the hook. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag your thumb to smear a thick stripe of generative goo across the canvas, leaving a glossy trail with a wet-drag audio cue. Swipe stripes until the entire canvas is covered in one solid color — then the screen exhales and the session ends.
...
- Includes in order: `window.LOOP_GAME='goo-swipe'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: goo-swipe" when finished.
