...
1. `web/site/games/dare-chain/index.html` — the game (single self-contained file)
2. `web/site/games/dare-chain/copy.md` — social captions
...
- **Conditioned path (the hook):** repost / share-forward chains. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A friend dares you; beat their score and the link auto-updates with your score so you dare the next friend. The chain stops after five links or when someone fails — the final score is the chain record.
...
- Includes in order: `window.LOOP_GAME='dare-chain'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'links'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: dare-chain" when finished.
