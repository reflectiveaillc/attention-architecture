...
1. `web/site/games/memory-trail/index.html` — the game (single self-contained file)
2. `web/site/games/memory-trail/copy.md` — social captions
...
## The game: MEMORY TRAIL

- **Conditioned path (the hook):** path-tracing / route-memory challenge clips where watching the trail is the hook. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A dot traces a winding path across the canvas for 3 seconds; then trace the same path from memory with one finger. Deviation from the original path is scored in pixels — the PB is the tightest trace.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 08-investment.
- **Video trick:** mastery flex — route memory tested with a real score — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='memory-trail'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'px'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# MEMORY TRAIL — copy
## TikTok (3 captions ≤100 chars, hook-first, mastery flex — route memory tested with a real score angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: memory-trail" when finished.
