...
1. `web/site/games/score-tag/index.html` — the game (single self-contained file)
2. `web/site/games/score-tag/copy.md` — social captions
...
- **Conditioned path (the hook):** @-mention challenges in comment sections. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Finish a run, then tap a friend's handle to tag them in a dare-link. They get exactly one attempt to beat your score; after that the tag is settled and the link goes dead.
...
- Includes in order: `window.LOOP_GAME='score-tag'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: score-tag" when finished.
