...
1. `web/site/games/soap-squeeze/index.html` — the game (single self-contained file)
2. `web/site/games/soap-squeeze/copy.md` — social captions
...
## The game: SOAP SQUEEZE

- **Conditioned path (the hook):** soap-cutting and glycerin-squish ASMR where pressing a waxy bar and hearing it crumble is the entire loop. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap to press a procedural soap bar that deforms under your thumb with a chalky squish sound. Each press leaves a thumb-shaped dent; ten presses fully crumble the bar into a pile of shavings. The crumble animation is the reward that ends the session.
...
- Includes in order: `window.LOOP_GAME='soap-squeeze'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: soap-squeeze" when finished.
