...
1. `web/site/games/rhythm-mirror/index.html` — the game (single self-contained file)
2. `web/site/games/rhythm-mirror/copy.md` — social captions
...
## The game: RHYTHM MIRROR

- **Conditioned path (the hook):** TikTok duet rhythmic challenges where one side plays a pattern and the other must mirror it. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 4-beat pattern plays on the left half of the screen; tap the right half to repeat it back perfectly. Pattern grows by one beat each round. Dare score is the longest pattern mirrored before a miss.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 08-investment.
...
- Includes in order: `window.LOOP_GAME='rhythm-mirror'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rhythm-mirror" when finished.
