...
1. `web/site/games/control-tower/index.html` — the game (single self-contained file)
2. `web/site/games/control-tower/copy.md` — social captions
...
## The game: CONTROL TOWER

- **Conditioned path (the hook):** the god-complex feed-curation loop — long-pressing to teach the algorithm, seeking the fantasy of total feed control. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Inbound events stream across a radar screen. Tap each to reroute it safely before it reaches the center. Events are color-coded by type — predictable, fully legible, no surprises. Score is events routed without a center breach. The player is the air traffic controller — nothing happens without their approval.
- **Engine:** viral. **Circuits:** 01-rpe, 05-ease, 06-zeigarnik.
- **Video trick:** benchmark ego bait — "I cleared 47 without a breach" — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='control-tower'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'routed'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# CONTROL TOWER — copy
## TikTok (3 captions ≤100 chars, hook-first, benchmark ego bait — "I cleared 47 without a breach" angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: control-tower" when finished.
