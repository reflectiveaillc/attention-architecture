...
1. `web/site/games/slime-stretch/index.html` — the game (single self-contained file)
2. `web/site/games/slime-stretch/copy.md` — social captions
...
## The game: SLIME STRETCH

- **Conditioned path (the hook):** slime-stretching ASMR reels where the pull-and-snap rhythm is the entire hook. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag a blob of generative slime upward; it stretches with elastic resistance and synthesized squeak. Release to snap it back with a satisfying thwack. The texture shifts color as it cools — a self-contained stretch-and-settle loop that ends after ten pulls.
...
- Includes in order: `window.LOOP_GAME='slime-stretch'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: slime-stretch" when finished.
