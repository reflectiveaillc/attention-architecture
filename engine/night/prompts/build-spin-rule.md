...
1. `web/site/games/spin-rule/index.html` — the game (single self-contained file)
2. `web/site/games/spin-rule/copy.md` — social captions
...
- **Conditioned path (the hook):** trend-roulette — the wheel-of-trends format where each spin lands on a fresh challenge. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A spinner lands on a new rule every 4 seconds — TAP ALL, TAP NONE, TAP ONLY BLUE, TAP THE BIGGEST, AVOID RED. Execute the landed rule on the tiles before it spins again. The wheel is the novelty engine; 12 spins and the session closes.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 05-ease.
- **Video trick:** variable-reward roulette — the spin itself is the cliffhanger — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='spin-rule'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'spins'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# SPIN RULE — copy
## TikTok (3 captions ≤100 chars, hook-first, variable-reward roulette — the spin itself is the cliffhanger angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: spin-rule" when finished.
