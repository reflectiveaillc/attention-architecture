...
1. `web/site/games/no-cash-gacha/index.html` — the game (single self-contained file)
2. `web/site/games/no-cash-gacha/copy.md` — social captions
...
## The game: NO-CASH GACHA

- **Conditioned path (the hook):** gacha and loot-box content that dangles pulls behind paywalls and rates. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap the capsule to spin a free pull; every pull is guaranteed unique from a set of 15 collectibles shown in a tracker. Duplicate protection means each tap makes progress. The 15th pull fills the tracker and the machine locks — complete, no premium currency, no infinite loop.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 02-novelty.
- **Video trick:** variable reward with a guaranteed end — the pull that completes it — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='no-cash-gacha'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'pulls'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# NO-CASH GACHA — copy
## TikTok (3 captions ≤100 chars, hook-first, variable reward with a guaranteed end — the pull that completes it angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: no-cash-gacha" when finished.
