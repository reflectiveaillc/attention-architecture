...
1. `web/site/games/number-riddle/index.html` — the game (single self-contained file)
2. `web/site/games/number-riddle/copy.md` — social captions
...
## The game: Number Riddle

- **Conditioned path (the hook):** only smart people solve this" puzzle posts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A sequence flashes; tap the missing number before the timer runs out. Solving N sequences reveals the hidden meta-pattern that ties them all together.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 06-zeigarnik.
- **Video trick:** can you solve it — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='number-riddle'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'sequences'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Number Riddle — copy
## TikTok (3 captions ≤100 chars, hook-first, can you solve it angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: number-riddle" when finished.
