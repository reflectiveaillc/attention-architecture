...
1. `web/site/games/rank-roulette/index.html` — the game (single self-contained file)
2. `web/site/games/rank-roulette/copy.md` — social captions
...
## The game: Rank Roulette

- **Conditioned path (the hook):** engagement-ratio gambling. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** You have one ranked bet per session: choose a shareable score, then tap to stop a spinning multiplier. Beat your bet and the dare-link upgrades; lose and the link shows the honest result with no retry.
...
- Includes in order: `window.LOOP_GAME='rank-roulette'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rank-roulette" when finished.
