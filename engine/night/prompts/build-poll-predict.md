...
1. `web/site/games/poll-predict/index.html` — the game (single self-contained file)
2. `web/site/games/poll-predict/copy.md` — social captions
...
## The game: POLL PREDICT

- **Conditioned path (the hook):** story sticker poll (the two-option poll you vote on to see which side wins). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A story poll appears with two options and a live results bar creeping toward the majority. Tap your pick before the timer ends; you score only if you predicted the winning side. Streak builds across consecutive correct predictions.
- **Engine:** viral. **Circuits:** 03-social, 01-rpe, 02-novelty.
- **Video trick:** participation-comment-bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='poll-predict'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: poll-predict" when finished.
