...
1. `web/site/games/missing-tile/index.html` — the game (single self-contained file)
2. `web/site/games/missing-tile/copy.md` — social captions
...
## The game: MISSING TILE

- **Conditioned path (the hook):** 'what\'s wrong with this picture?' change-blindness clips that make the viewer feel foolish then share. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A full grid of colored tiles shows for 3 seconds; one tile vanishes and the grid reshuffles. Tap the empty slot where the missing tile belongs. The grid grows each level and reshuffles more aggressively.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 06-zeigarnik.
- **Video trick:** shock-honesty — 'change blindness is real, I missed it on round 4' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='missing-tile'` → loop-events.js → juice.js → share.js.
...
```
# MISSING TILE — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty — 'change blindness is real, I missed it on round 4' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: missing-tile" when finished.
