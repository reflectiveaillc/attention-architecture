...
1. `web/site/games/routine-builder/index.html` — the game (single self-contained file)
2. `web/site/games/routine-builder/copy.md` — social captions
...
## The game: ROUTINE BUILDER

- **Conditioned path (the hook):** optimised-routine content — the morning-routine, workflow-setup, and system-design deep-dives the algorithm feeds to routine-mastery brains. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag step cards into a vertical sequence to build a routine. Each card snaps with a satisfying click; misplaced cards push back softly. A dependency checker lights green when all steps are in the correct logical order. Lock the routine and a clean summary card slides down — your system, complete.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** completion-satisfaction — the routine locked and running — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='routine-builder'` → loop-events.js → juice.js → share.js.
...
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the routine locked and running angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: routine-builder" when finished.
