...
1. `web/site/games/story-set/index.html` — the game (single self-contained file)
2. `web/site/games/story-set/copy.md` — social captions
...
- **Conditioned path (the hook):** stories 24h FOMO (the same story-ring conditioning, without the scramble). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A row of story bubbles with depleting rings sits on screen; you don't race them. Each ring empties on its own and the bubble fades with a soft chime; tap one only to acknowledge it before it goes. It's okay that they expire — you watch them settle and the bar empties.
- **Engine:** calm. **Circuits:** 05-ease, 04-loss, 07-timing.
- **Video trick:** the feed that ends — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='story-set'` → loop-events.js → juice.js → share.js.
...
```
# STORY SET — copy
## TikTok (3 captions ≤100 chars, hook-first, the feed that ends angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: story-set" when finished.
