...
1. `web/site/games/boost-button/index.html` — the game (single self-contained file)
2. `web/site/games/boost-button/copy.md` — social captions
...
## The game: Boost Button

- **Conditioned path (the hook):** boost / promote-post button. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap the BOOST button to pump reach into a post — each tap adds viewers but fills a shadowban meter; tap BANK to lock in reach before the meter trips and wipes everything.
...
- Includes in order: `window.LOOP_GAME='boost-button'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'reach'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: boost-button" when finished.
