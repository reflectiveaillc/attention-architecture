...
1. `web/site/games/pinned-top/index.html` — the game (single self-contained file)
2. `web/site/games/pinned-top/copy.md` — social captions
...
- **Conditioned path (the hook):** pinned top-comment status. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Comments rise and sink in a live thread; tap to pin the one climbing hardest to the top before it falls — a successful pin earns the top-comment crown and points.
...
- Includes in order: `window.LOOP_GAME='pinned-top'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'top pins'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pinned-top" when finished.
