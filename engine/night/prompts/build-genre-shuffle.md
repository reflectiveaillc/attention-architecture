...
1. `web/site/games/genre-shuffle/index.html` — the game (single self-contained file)
2. `web/site/games/genre-shuffle/copy.md` — social captions
...
## The game: GENRE SHUFFLE

- **Conditioned path (the hook):** the FYP genre-whiplash — cooking clip, then prank, then dance, then ASMR, back to back. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Every 10 seconds the game swaps genre entirely: catch falling dots → dodge rising bars → tap the lit tile → trace the line. One thumb, four genres in 40s, then FIN. The format-flip IS the novelty — the same way the FYP hits you with a different genre each swipe, except here it ends.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 05-ease.
- **Video trick:** genre-whiplash — 'wait this is a totally different game now' is the rewatch trigger — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='genre-shuffle'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'genres'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# GENRE SHUFFLE — copy
## TikTok (3 captions ≤100 chars, hook-first, genre-whiplash — 'wait this is a totally different game now' is the rewatch trigger angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: genre-shuffle" when finished.
