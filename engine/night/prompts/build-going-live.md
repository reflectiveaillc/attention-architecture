...
1. `web/site/games/going-live/index.html` — the game (single self-contained file)
2. `web/site/games/going-live/copy.md` — social captions
...
- **Conditioned path (the hook):** live countdown / going live. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 'LIVE' badge pulses. Tap to start exactly when the ring completes. Start early and you get zero viewers; start late and you miss the trend window. The countdown is irregular.
...
- Includes in order: `window.LOOP_GAME='going-live'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: going-live" when finished.
