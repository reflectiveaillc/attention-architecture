...
1. `web/site/games/sand-pour/index.html` — the game (single self-contained file)
2. `web/site/games/sand-pour/copy.md` — social captions
...
- **Conditioned path (the hook):** kinetic-sand ASMR videos where the slow pour and piling crunch is the whole loop. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap and hold anywhere to pour a stream of particle sand that piles with soft crunch audio. Release to stop. The pile settles physics-first with a mound shape; the canvas holds one beautiful pile before gently dispersing and ending the session.
- **Engine:** calm. **Circuits:** 05-ease, 01-rpe, 08-investment.
...
- Includes in order: `window.LOOP_GAME='sand-pour'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sand-pour" when finished.
