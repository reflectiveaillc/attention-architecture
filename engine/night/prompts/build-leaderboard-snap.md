...
1. `web/site/games/leaderboard-snap/index.html` — the game (single self-contained file)
2. `web/site/games/leaderboard-snap/copy.md` — social captions
...
## The game: Leaderboard Snap

- **Conditioned path (the hook):** leaderboard screenshot sharing. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Play a 30-second round. The game renders a one-time leaderboard image with your score and three empty friend slots. Share it as a dare; friends beat it or not, then the snap expires and is no longer valid.
- **Engine:** viral. **Circuits:** 01-rpe, 03-social, 06-zeigarnik.
- **Video trick:** ego bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='leaderboard-snap'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: leaderboard-snap" when finished.
