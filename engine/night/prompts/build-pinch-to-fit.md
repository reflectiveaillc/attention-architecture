...
1. `web/site/games/pinch-to-fit/index.html` — the game (single self-contained file)
2. `web/site/games/pinch-to-fit/copy.md` — social captions
...
## The game: Pinch to Fit

- **Conditioned path (the hook):** pinch/zoom. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A target shape pulses in the center. Pinch to shrink it, spread to grow it, then release when it perfectly frames the ghost outline. The outline flashes for one frame like a thirst trap you almost caught.
...
- Includes in order: `window.LOOP_GAME='pinch-to-fit'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pinch-to-fit" when finished.
