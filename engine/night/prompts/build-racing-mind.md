...
1. `web/site/games/racing-mind/index.html` — the game (single self-contained file)
2. `web/site/games/racing-mind/copy.md` — social captions
...
## The game: Racing Mind

- **Conditioned path (the hook):** the 2am thought-loop firing faster than you can settle it. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Thought bubbles fire across the screen fast; tap each to set it down before it escapes. Every caught thought slows the fire rate and dims the screen, until the thoughts barely come and the screen is black. Score is thoughts set down — the racing mind caught and slowed to sleep.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 07-timing.
- **Video trick:** completion-satisfaction — catch every thought, then they stop — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='racing-mind'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'thoughts'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Racing Mind — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — catch every thought, then they stop angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: racing-mind" when finished.
