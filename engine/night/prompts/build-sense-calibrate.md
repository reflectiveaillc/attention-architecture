...
1. `web/site/games/sense-calibrate/index.html` — the game (single self-contained file)
2. `web/site/games/sense-calibrate/copy.md` — social captions
...
## The game: SENSE CALIBRATE

- **Conditioned path (the hook):** sensory-processing content — threads on sound sensitivity, texture grading, and sensory preference mapping. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A soft tone plays; slide a dial to the exact frequency that matches it. The dial gives consistent haptic-style visual feedback as you approach the target — a waveform that settles to flat when aligned. 8 tones, each a different frequency. Last tone calibrated, the dial sequence plays back as a chord.
- **Engine:** calm. **Circuits:** 01-rpe, 05-ease, 06-zeigarnik.
- **Video trick:** near-perfection itch — almost flat, one hair to the left — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='sense-calibrate'` → loop-events.js → juice.js → share.js.
...
```
# SENSE CALIBRATE — copy
## TikTok (3 captions ≤100 chars, hook-first, near-perfection itch — almost flat, one hair to the left angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sense-calibrate" when finished.
