...
1. `web/site/games/scroll-to-still/index.html` — the game (single self-contained file)
2. `web/site/games/scroll-to-still/copy.md` — social captions
...
## The game: Scroll to Still

- **Conditioned path (the hook):** the infinite-swipe thumb that keeps moving past every stopping point. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Swipe up slowly; the feed moves but each swipe adds friction and dims the screen a shade. The more you scroll, the heavier it gets, until the feed won't move at all and settles into black. The scroll habit is braked to a full stop instead of rewarded for speed.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 06-zeigarnik.
- **Video trick:** the feed that ends — friction as the off-ramp — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='scroll-to-still'` → loop-events.js → juice.js → share.js.
...
```
# Scroll to Still — copy
## TikTok (3 captions ≤100 chars, hook-first, the feed that ends — friction as the off-ramp angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: scroll-to-still" when finished.
