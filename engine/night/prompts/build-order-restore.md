...
1. `web/site/games/order-restore/index.html` — the game (single self-contained file)
2. `web/site/games/order-restore/copy.md` — social captions
...
## The game: ORDER RESTORE

- **Conditioned path (the hook):** the compulsive re-read of a thread to catch everything and feel nothing was missed. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A small grid of tiles is shuffled into disorder. Drag any tile to any open slot — all moves are reversible, nothing locks until you want it to. A gentle shimmer shows when the grid is fully ordered; tap DONE to close it. No timer, no wrong moves, no fail state. Restore it at your own pace.
- **Engine:** calm. **Circuits:** 05-ease, 06-zeigarnik, 08-investment.
- **Video trick:** calm completion — the shimmer signaling order restored — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='order-restore'` → loop-events.js → juice.js → share.js.
...
```
# ORDER RESTORE — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the shimmer signaling order restored angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: order-restore" when finished.
