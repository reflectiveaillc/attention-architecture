...
1. `web/site/games/breath-dim/index.html` — the game (single self-contained file)
2. `web/site/games/breath-dim/copy.md` — social captions
...
- **Conditioned path (the hook):** late-night arousal held in the chest, with no pacing cue to bring it down. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold to inhale (screen brightens a touch), release to exhale (screen dims). Each full breath lowers the brightness baseline a step, so the resting level itself decays toward black across the session. Your breath is the dimmer, and sleep-onset is the decaying baseline.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 01-rpe.
- **Video trick:** calm completion — biofeedback dimming to sleep — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='breath-dim'` → loop-events.js → juice.js → share.js.
...
```
# Breath Dim — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — biofeedback dimming to sleep angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: breath-dim" when finished.
