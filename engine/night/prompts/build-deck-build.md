...
1. `web/site/games/deck-build/index.html` — the game (single self-contained file)
2. `web/site/games/deck-build/copy.md` — social captions
...
- **Conditioned path (the hook):** trading-card deck-building content that dangles the one card completing your strategy. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Cards slide in one at a time from a shuffled draw pile of 20. Tap each to add it to your hand or discard; fill a 10-card deck with one of each type shown in a tracker. The draw pile has exactly 20 cards — no infinite pack, no premium pulls. Complete the deck and it snaps face-up in a full-set spread.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 08-investment.
- **Video trick:** completion-satisfaction — the full deck laid out — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='deck-build'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'cards'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# DECK BUILD — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the full deck laid out angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: deck-build" when finished.
