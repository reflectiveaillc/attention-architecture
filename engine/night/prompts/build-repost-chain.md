...
1. `web/site/games/repost-chain/index.html` — the game (single self-contained file)
2. `web/site/games/repost-chain/copy.md` — social captions
...
## The game: REPOST CHAIN

- **Conditioned path (the hook):** repost/regram (the chain-posted meme that lives only if someone keeps reposting it). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A post travels along a conveyor; tap the repost button the instant it crosses the trigger line to pass it forward. Each clean repost extends the chain; a mistimed tap breaks it and the chain dies. The conveyor speeds up as the chain grows.
- **Engine:** viral. **Circuits:** 04-loss, 01-rpe, 08-investment.
- **Video trick:** loss aversion — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='repost-chain'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: repost-chain" when finished.
