...
1. `web/site/games/perfect-pour/index.html` — the game (single self-contained file)
2. `web/site/games/perfect-pour/copy.md` — social captions
...
## The game: Perfect Pour

- **Conditioned path (the hook):** cooking / bartending 'perfect pour' clips that make you want to nail the line. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold to pour liquid into a vessel; release exactly when the fill line is hit. Overpour or underpour ends the run. PB is the closest fill % to the target across attempts.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 07-timing.
- **Video trick:** just-one-more — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='perfect-pour'` → loop-events.js → juice.js → share.js.
...
```
# Perfect Pour — copy
## TikTok (3 captions ≤100 chars, hook-first, just-one-more angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: perfect-pour" when finished.
