...
1. `web/site/games/next-episode/index.html` — the game (single self-contained file)
2. `web/site/games/next-episode/copy.md` — social captions
...
- **Conditioned path (the hook):** binge "next episode" hooks. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Swipe up through micro-chapters; each ends on a two-choice beat that unlocks the next scene. The final chapter resolves the arc and the session closes instead of cliffhanging forever.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 02-novelty.
- **Video trick:** cliffhanger chain with a real ending — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='next-episode'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'chapters'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Next Episode — copy
## TikTok (3 captions ≤100 chars, hook-first, cliffhanger chain with a real ending angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: next-episode" when finished.
