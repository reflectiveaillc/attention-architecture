...
1. `web/site/games/swipe-reveal/index.html` — the game (single self-contained file)
2. `web/site/games/swipe-reveal/copy.md` — social captions
...
## The game: Swipe Reveal

- **Conditioned path (the hook):** wait-for-it / satisfying reveal videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag your thumb across a pixelated cover to uncover the hidden image underneath. Clear enough to guess correctly and the loop closes with the payoff; clear too much and the reveal is wasted.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 05-ease.
- **Video trick:** wait-for-it payoff — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='swipe-reveal'` → loop-events.js → juice.js → share.js.
...
```
# Swipe Reveal — copy
## TikTok (3 captions ≤100 chars, hook-first, wait-for-it payoff angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: swipe-reveal" when finished.
