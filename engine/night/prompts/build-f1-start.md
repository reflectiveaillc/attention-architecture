...
1. `web/site/games/f1-start/index.html` — the game (single self-contained file)
2. `web/site/games/f1-start/copy.md` — social captions
...
- **Conditioned path (the hook):** sports highlight countdowns and race-start hype edits. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Five red lights count down; tap the instant they all go out. The delay between lights-out and green is random, punishing anticipation and rewarding pure reaction.
...
- Includes in order: `window.LOOP_GAME='f1-start'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: f1-start" when finished.
