...
1. `web/site/games/scene-rewind/index.html` — the game (single self-contained file)
2. `web/site/games/scene-rewind/copy.md` — social captions
...
## The game: SCENE REWIND

- **Conditioned path (the hook):** 'spot what changed between these two screenshots' nostalgia-diff posts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A scene of illustrated objects shows for 4 seconds; it resets with 3 objects removed and 2 added. Tap the objects that were NOT in the original scene. Score is correct identifications; dare-link prints the change count to beat.
...
- Includes in order: `window.LOOP_GAME='scene-rewind'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'found'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: scene-rewind" when finished.
