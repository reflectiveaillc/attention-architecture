...
1. `web/site/games/restore-order/index.html` — the game (single self-contained file)
2. `web/site/games/restore-order/copy.md` — social captions
...
## The game: RESTORE ORDER

- **Conditioned path (the hook):** messy-desk and clutter-clearing ASMR that stops right before the last item is put away. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A room scene has objects scattered out of place; tap each misplaced object and tap its correct spot to return it. As each item snaps home a calm ambient note plays; when the last object is placed the room settles into perfect stillness.
...
- Includes in order: `window.LOOP_GAME='restore-order'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: restore-order" when finished.
