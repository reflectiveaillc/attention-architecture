...
1. `web/site/games/rhythm-wean/index.html` — the game (single self-contained file)
2. `web/site/games/rhythm-wean/copy.md` — social captions
...
## The game: RHYTHM WEAN

- **Conditioned path (the hook):** seamless audio loops that never resolve — the same 8-bar phrase repeating with no end signal. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A gentle looping beat plays; tap on each pulse to build a soft glowing ring. After 32 beats the ring is complete and the track resolves with a final chord — a hard, satisfying end instead of another loop. Calm flow with a real finish.
...
- Includes in order: `window.LOOP_GAME='rhythm-wean'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rhythm-wean" when finished.
