...
1. `web/site/games/cloud-name/index.html` — the game (single self-contained file)
2. `web/site/games/cloud-name/copy.md` — social captions
...
- **Conditioned path (the hook):** doomscrolling for reassurance — searching for named threats to contain them. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Soft clouds drift across the screen. Tap each cloud to name it with a one-tap label ("work", "health", "future"). Once named, the cloud turns a calm pastel and floats off the top of the screen. When the sky is empty, new clouds drift in slowly. You control which you name and when; none vanish until you name them.
- **Engine:** calm. **Circuits:** 05-ease, 06-zeigarnik, 08-investment.
- **Video trick:** calm completion — a clear sky after the last cloud floats away — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='cloud-name'` → loop-events.js → juice.js → share.js.
...
```
# CLOUD NAME — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — a clear sky after the last cloud floats away angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: cloud-name" when finished.
