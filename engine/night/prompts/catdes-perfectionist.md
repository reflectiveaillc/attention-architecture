...
`engine/state/designs/cat-perfectionist.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "perfectionist"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: perfectionist · label: Mastery · engine_lean: viral
trait: precision-driven, self-improvement loop; chasing the personal best
algo_hook: fed skill-tutorial and 'you're doing it wrong' rabbit-holes that never let them feel finished
wean: a high-skill-ceiling precision game with a clean PB to beat — masterable, measurable, and yours
mechanics: precision timing, perfect-run chasing, high skill ceiling, PB tracking, one-more-attempt mastery
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: perfectionist x20" when finished.
