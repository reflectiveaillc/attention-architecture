...
1. `web/site/games/pulse-hold/index.html` — the game (single self-contained file)
2. `web/site/games/pulse-hold/copy.md` — social captions
...
- **Conditioned path (the hook):** sustained-note audio memes where the payoff is holding the tone exactly as long as it lasts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A note sustains — press and hold for exactly its duration, then release. The error (too short or too long) is shown in ms. Ten notes of varying length per session; PB is the lowest total error.
...
- Includes in order: `window.LOOP_GAME='pulse-hold'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pulse-hold" when finished.
