...
1. `web/site/games/shelf-restock/index.html` — the game (single self-contained file)
2. `web/site/games/shelf-restock/copy.md` — social captions
...
## The game: SHELF RESTOCK

- **Conditioned path (the hook):** store-restock and grocery-facing ASMR that loops before the last row is done. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** An empty shelf grid sits center-screen; products drift in from the side and you tap each one to place it in the correct slot by color or size. When every slot is filled the shelf glows complete and the loop ends instead of resetting.
...
- Includes in order: `window.LOOP_GAME='shelf-restock'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: shelf-restock" when finished.
