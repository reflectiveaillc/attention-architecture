...
1. `web/site/games/one-more-stop/index.html` — the game (single self-contained file)
2. `web/site/games/one-more-stop/copy.md` — social captions
...
## The game: One More Stop

- **Conditioned path (the hook):** the FYP 'one more video' autoplay lure that keeps you scrolling past every exit. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Cards autoplay like a FYP, each tagged 'one more?'. The only move is to NOT swipe — every second you hold still, the screen dims a shade; one swipe and it brightens back. Score is how many seconds you hold the dimmer down to black. The win is resisting the swipe that the algorithm lives on.
- **Engine:** viral. **Circuits:** 05-ease, 04-loss, 07-timing.
- **Video trick:** loss aversion inverted — the score is how early you stop — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='one-more-stop'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'seconds'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# One More Stop — copy
## TikTok (3 captions ≤100 chars, hook-first, loss aversion inverted — the score is how early you stop angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: one-more-stop" when finished.
