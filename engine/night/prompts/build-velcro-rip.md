...
1. `web/site/games/velcro-rip/index.html` — the game (single self-contained file)
2. `web/site/games/velcro-rip/copy.md` — social captions
...
- **Conditioned path (the hook):** velcro-ripping ASMR speed clips where the tear length determines the sound hit. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag across a velcro strip as fast as possible; the rip sound scales in pitch and crackle intensity with your drag speed. Score is peak drag speed in pixels-per-second. Three rips per session, best score is the shareable dare.
...
- Includes in order: `window.LOOP_GAME='velcro-rip'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'px/s'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: velcro-rip" when finished.
