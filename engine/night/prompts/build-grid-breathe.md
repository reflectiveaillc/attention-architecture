...
1. `web/site/games/grid-breathe/index.html` — the game (single self-contained file)
2. `web/site/games/grid-breathe/copy.md` — social captions
...
## The game: GRID BREATHE

- **Conditioned path (the hook):** aesthetic-grid curation (the same grid-conditioning, freed from judgment). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 3x3 grid of soft color tiles; tap any two to swap them at your own pace. No meter scores you — when the grid feels balanced to you, it gently locks and pulses once with your breath, then rests. Curation without a verdict.
...
- Includes in order: `window.LOOP_GAME='grid-breathe'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: grid-breathe" when finished.
