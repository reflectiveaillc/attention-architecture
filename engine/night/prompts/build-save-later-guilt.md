...
1. `web/site/games/save-later-guilt/index.html` — the game (single self-contained file)
2. `web/site/games/save-later-guilt/copy.md` — social captions
...
## The game: Save Later Guilt

- **Conditioned path (the hook):** saved/liked-later collections. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Posts fly by. Swipe right to save, left to skip. Saved posts pile at the bottom; if the stack reaches the top, you drown. The real win is learning to stop saving.
- **Engine:** viral. **Circuits:** 04-loss, 06-zeigarnik, 08-investment.
...
- Includes in order: `window.LOOP_GAME='save-later-guilt'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: save-later-guilt" when finished.
