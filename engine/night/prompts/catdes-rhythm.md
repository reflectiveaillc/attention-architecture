...
`engine/state/designs/cat-rhythm.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "rhythm"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: rhythm · label: Rhythm & Flow · engine_lean: viral
trait: flow-state seeker; locks into music and repetition
algo_hook: fed audio memes and looping sounds that hijack the flow-craving into endless replay
wean: a tight rhythm/timing game that delivers real flow in a bounded track that ends on a beat
mechanics: beat-matched taps, rhythm timing, flow combo, tempo ramp, on-the-beat
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: rhythm x20" when finished.
