...
1. `web/site/games/pattern-fill/index.html` — the game (single self-contained file)
2. `web/site/games/pattern-fill/copy.md` — social captions
...
## The game: PATTERN FILL

- **Conditioned path (the hook):** satisfying-completion loops — mosaic, pixel-art fill, or number-colour-by-number that the algorithm feeds endlessly. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A grid of cells has a hidden repeating tile pattern with gaps. Tap each empty cell to fill it in the color the pattern demands. Consistent audio click per correct fill; a wrong tap greys out. When the grid is complete it pulses once and the session ends.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 01-rpe.
- **Video trick:** calm completion — the pattern resolves to whole — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='pattern-fill'` → loop-events.js → juice.js → share.js.
...
```
# PATTERN FILL — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the pattern resolves to whole angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pattern-fill" when finished.
