...
1. `web/site/games/plot-twist/index.html` — the game (single self-contained file)
2. `web/site/games/plot-twist/copy.md` — social captions
...
- **Conditioned path (the hook):** plot twist storytime videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap to choose the next story beat from two options. The third choice flips everything and reveals the twist ending; once it lands, the story is over.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 02-novelty.
- **Video trick:** the twist at the end — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='plot-twist'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'twists'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Plot Twist — copy
## TikTok (3 captions ≤100 chars, hook-first, the twist at the end angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: plot-twist" when finished.
