...
`engine/state/designs/cat-ocd.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "ocd"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: ocd · label: Order & Symmetry · engine_lean: calm
trait: the 'not-quite-right' pull; symmetry, alignment, completion, restoring order
algo_hook: fed oddly-satisfying, restock, power-wash, and cleaning loops that scratch the order-itch endlessly
wean: let them ACTUALLY finish it — align, sort, snap-to-grid, close the loop — and the satisfaction resolves instead of scrolling forever
mechanics: align/center, sort by rule, symmetry match, fill the grid, snap into place, restore order
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: ocd x20" when finished.
