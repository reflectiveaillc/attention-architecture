...
1. `web/site/games/pulse-match/index.html` — the game (single self-contained file)
2. `web/site/games/pulse-match/copy.md` — social captions
...
## The game: PULSE MATCH

- **Conditioned path (the hook):** the news-refresh loop — checking for updates to synchronize inner state with outer reality. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A slow pulse glows at the center of the screen. Tap in time with it — not to beat it, just to match it. If you match for three pulses in a row the pulse slows one notch. Keep matching and keep slowing it until the pulse is barely visible and the screen dims to black. You decelerate the beat by following it.
- **Engine:** calm. **Circuits:** 07-timing, 05-ease, 01-rpe.
- **Video trick:** calm completion — the beat you slowed to stillness — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='pulse-match'` → loop-events.js → juice.js → share.js.
...
```
# PULSE MATCH — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the beat you slowed to stillness angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pulse-match" when finished.
