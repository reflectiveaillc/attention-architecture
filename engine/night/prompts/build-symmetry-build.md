...
1. `web/site/games/symmetry-build/index.html` — the game (single self-contained file)
2. `web/site/games/symmetry-build/copy.md` — social captions
...
## The game: SYMMETRY BUILD

- **Conditioned path (the hook):** mandala-drawing and geometric-pattern ASMR that loops before the shape closes. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap on the left half of the canvas to place a shape; its mirror image appears on the right simultaneously. Build out a symmetric composition tile by tile until a full mandala blooms and the canvas fades to rest.
...
- Includes in order: `window.LOOP_GAME='symmetry-build'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: symmetry-build" when finished.
