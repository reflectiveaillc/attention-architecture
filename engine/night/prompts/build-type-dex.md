...
1. `web/site/games/type-dex/index.html` — the game (single self-contained file)
2. `web/site/games/type-dex/copy.md` — social captions
...
- **Conditioned path (the hook):** Pokédex / trading-card completionist feeds that dangle the one card you don’t have. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Silhouettes of 20 creatures drift across the screen; tap each type icon that matches the silhouette to capture it. Capture all 20 and the dex seals with a satisfying full-roster flash. Each captured entry is free, and the roster has exactly 20.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 02-novelty.
- **Video trick:** completion-satisfaction — the dex 100% filled — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='type-dex'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'captured'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# TYPE DEX — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the dex 100% filled angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: type-dex" when finished.
