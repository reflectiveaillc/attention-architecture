...
1. `web/site/games/slow-pour/index.html` — the game (single self-contained file)
2. `web/site/games/slow-pour/copy.md` — social captions
...
- **Conditioned path (the hook):** the pull-to-refresh lever used as a self-soothing gesture when nothing new helps but the gesture comforts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold to tip a vessel; liquid pours at exactly the speed you tilt. Too fast and it splashes; too slow and it barely drips. The session is about finding the middle — a steady, predictable flow into the vessel below. When the vessel is full it chimes once and rests. Full control of the pour rate.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 01-rpe.
- **Video trick:** calm completion — the vessel reaching full is the exhale — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='slow-pour'` → loop-events.js → juice.js → share.js.
...
```
# SLOW POUR — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the vessel reaching full is the exhale angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: slow-pour" when finished.
