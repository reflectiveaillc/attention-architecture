...
1. `web/site/games/streak-release/index.html` — the game (single self-contained file)
2. `web/site/games/streak-release/copy.md` — social captions
...
- **Mechanic:** A streak number gently counts down day by day. You do nothing. When it reaches zero it turns into a seed and plants a small garden mark. Letting go is the only action.
- **Engine:** calm. **Circuits:** 04-loss, 08-investment, 05-ease.
...
- Includes in order: `window.LOOP_GAME='streak-release'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: streak-release" when finished.
