...
1. `web/site/games/snowfall-settle/index.html` — the game (single self-contained file)
2. `web/site/games/snowfall-settle/copy.md` — social captions
...
## The game: Snowfall Settle

- **Conditioned path (the hook):** the sensory loop that won't stop stimulating when you need stillness. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Snowflakes drift down; tap each one to gently settle it to rest. Each settled flake stops moving, darkens the sky a shade, and quiets the wind. When every flake is at rest and the sky is black, the storm has passed and the session ends.
- **Engine:** calm. **Circuits:** 02-novelty, 07-timing, 06-zeigarnik.
- **Video trick:** calm completion — settle the scene to still — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='snowfall-settle'` → loop-events.js → juice.js → share.js.
...
```
# Snowfall Settle — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — settle the scene to still angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: snowfall-settle" when finished.
