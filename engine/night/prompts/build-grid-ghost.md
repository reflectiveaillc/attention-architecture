...
1. `web/site/games/grid-ghost/index.html` — the game (single self-contained file)
2. `web/site/games/grid-ghost/copy.md` — social captions
...
- **Conditioned path (the hook):** hidden-object / 'what\'s different' clip format that parks the pattern in short-term memory. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A grid of symbols lights up briefly, then goes dark; tap the cells that were lit in any order. Every correct tap scores; every wrong tap ends the round. The lit pattern grows each level, exercising spatial working memory.
...
- Includes in order: `window.LOOP_GAME='grid-ghost'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'cells'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: grid-ghost" when finished.
