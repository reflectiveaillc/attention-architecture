...
1. `web/site/games/soft-check/index.html` — the game (single self-contained file)
2. `web/site/games/soft-check/copy.md` — social captions
...
- **Conditioned path (the hook):** the compulsive notification check — opening the app to confirm everything is okay and closing having seen nothing that helps. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A single soft circle sits on screen labeled CHECK. Tap it once: a gentle "all clear" animation plays — a slow checkmark draws itself, a soft chime sounds, the circle pulses green. That is the whole session. One tap, one confirmation, done. The check is granted immediately, reliably, every time. No feed, no scroll, just the check.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 01-rpe.
- **Video trick:** calm completion — the checkmark that actually resolves the loop — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='soft-check'` → loop-events.js → juice.js → share.js.
...
```
# SOFT CHECK — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the checkmark that actually resolves the loop angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: soft-check" when finished.
