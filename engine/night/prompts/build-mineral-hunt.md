...
1. `web/site/games/mineral-hunt/index.html` — the game (single self-contained file)
2. `web/site/games/mineral-hunt/copy.md` — social captions
...
## The game: MINERAL HUNT

- **Conditioned path (the hook):** rock and crystal unboxing hauls that tease the missing specimen in a collection. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap to chip a tile off a stone face; each chip reveals a fragment of a mineral specimen. Chip enough to identify it and it adds to a display case of 8 specimens. Find all 8 types and the case locks with a complete collection badge. No infinite pull, just 8 hidden minerals.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 02-novelty.
- **Video trick:** reveal bait — what mineral is hiding under this rock? — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='mineral-hunt'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'specimens'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# MINERAL HUNT — copy
## TikTok (3 captions ≤100 chars, hook-first, reveal bait — what mineral is hiding under this rock? angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: mineral-hunt" when finished.
