...
1. `web/site/games/dimming-window/index.html` — the game (single self-contained file)
2. `web/site/games/dimming-window/copy.md` — social captions
...
## The game: Dimming Window

- **Conditioned path (the hook):** blue-light / day-spectrum exposure that tells the brain it is still daytime. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A window looks out on a sky; hold to advance the sun from day toward night. The sky darkens, the color temperature warms down, and ambient daytime sound fades to silence. When the sky reaches full night and black, the body has its cue and the session closes.
- **Engine:** calm. **Circuits:** 07-timing, 05-ease, 02-novelty.
- **Video trick:** calm completion — run the sunset yourself — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='dimming-window'` → loop-events.js → juice.js → share.js.
...
```
# Dimming Window — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — run the sunset yourself angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: dimming-window" when finished.
