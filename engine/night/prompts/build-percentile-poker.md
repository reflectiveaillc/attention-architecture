...
1. `web/site/games/percentile-poker/index.html` — the game (single self-contained file)
2. `web/site/games/percentile-poker/copy.md` — social captions
...
## The game: Percentile Poker

- **Conditioned path (the hook):** hidden like counts / guess-the-engagement. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Play one round blind, then bet whether your score sits in the top 50%, 25%, or 10% of today's dares. Reveal, win the bet, and send the dare-link. Wrong bet ends the session with no share.
...
- Includes in order: `window.LOOP_GAME='percentile-poker'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: percentile-poker" when finished.
