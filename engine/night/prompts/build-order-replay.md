...
1. `web/site/games/order-replay/index.html` — the game (single self-contained file)
2. `web/site/games/order-replay/copy.md` — social captions
...
## The game: ORDER REPLAY

- **Conditioned path (the hook):** viral 'put these in order' ranking challenges that bait debate and recall. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Five items appear in random order for 4 seconds, then scramble; drag them back into the exact original order from memory. Miss-order distance is the score — lower is better, and rounds get longer lists.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 08-investment.
- **Video trick:** participation-comment-bait — 'I got it perfect, did you?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='order-replay'` → loop-events.js → juice.js → share.js.
...
```
# ORDER REPLAY — copy
## TikTok (3 captions ≤100 chars, hook-first, participation-comment-bait — 'I got it perfect, did you?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: order-replay" when finished.
