...
1. `web/site/games/pulse-parry/index.html` — the game (single self-contained file)
2. `web/site/games/pulse-parry/copy.md` — social captions
...
## The game: PULSE PARRY

- **Conditioned path (the hook):** parry-based combat edits and 'perfect dodge' gaming moments. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** An attack pulses toward you; tap on the exact beat to parry. Parry too early and you expose yourself; too late and you take the hit. Tempo escalates each round.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 07-timing.
- **Video trick:** last-instant bait — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='pulse-parry'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'parries'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# PULSE PARRY — copy
## TikTok (3 captions ≤100 chars, hook-first, last-instant bait angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pulse-parry" when finished.
