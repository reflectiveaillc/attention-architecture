...
1. `web/site/games/face-morph-lock/index.html` — the game (single self-contained file)
2. `web/site/games/face-morph-lock/copy.md` — social captions
...
- **Mechanic:** A face mesh morphs through filters. Tap when it lands on the target look for a split second. Morphs speed up and add decoy matches that look almost right.
...
- Includes in order: `window.LOOP_GAME='face-morph-lock'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'locks'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: face-morph-lock" when finished.
