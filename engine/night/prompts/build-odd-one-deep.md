...
1. `web/site/games/odd-one-deep/index.html` — the game (single self-contained file)
2. `web/site/games/odd-one-deep/copy.md` — social captions
...
## The game: ODD ONE DEEP

- **Conditioned path (the hook):** pattern-deviation rabbit-holes — the thread cataloguing every exception to a rule in a system. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A matrix of items has one that violates the system rule. The rule is not stated. Watch the matrix for one full cycle, then tap the outlier. Each round adds a dimension (shape AND colour AND count AND rotation) — the rule grows more specific, the outlier more subtle. 10 rounds and the outlier chain resolves.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 01-rpe, 05-ease.
- **Video trick:** play-along comment bait — did you catch the outlier before round 7? — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='odd-one-deep'` → loop-events.js → juice.js → share.js.
...
```
# ODD ONE DEEP — copy
## TikTok (3 captions ≤100 chars, hook-first, play-along comment bait — did you catch the outlier before round 7? angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: odd-one-deep" when finished.
