...
1. `web/site/games/tap-rain/index.html` — the game (single self-contained file)
2. `web/site/games/tap-rain/copy.md` — social captions
...
- **Conditioned path (the hook):** rain-on-surface ASMR where each drop landing triggers an individual soft tap sound. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Rain drops fall randomly onto the canvas, each emitting a synthesized tap on impact. Slide your thumb along the bottom to tilt the canvas left or right, directing the flow into a pool. When the pool fills, the rain slows to a stop and the session ends.
...
- Includes in order: `window.LOOP_GAME='tap-rain'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tap-rain" when finished.
