...
1. `web/site/games/reply-window/index.html` — the game (single self-contained file)
2. `web/site/games/reply-window/copy.md` — social captions
...
## The game: Reply Window

- **Conditioned path (the hook):** reply-before-it-disappears. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A story ring shrinks like an ephemeral reply timer. Type the shown phrase before the ring closes. Each correct reply extends the timer by a hair; each typo burns time.
- **Engine:** viral. **Circuits:** 04-loss, 06-zeigarnik, 01-rpe.
...
- Includes in order: `window.LOOP_GAME='reply-window'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: reply-window" when finished.
