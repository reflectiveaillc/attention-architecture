...
1. `web/site/games/badge-collect/index.html` — the game (single self-contained file)
2. `web/site/games/badge-collect/copy.md` — social captions
...
## The game: BADGE COLLECT

- **Conditioned path (the hook):** achievement badge walls on apps that always show the one badge you haven’t earned. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** 10 badge outlines glow on a board. One badge lights up live; tap it to claim it before it dims. Claim it and it fills permanently. Badges light in sequence but with random timing. Claim all 10 and the board shines complete — no new badge ever appears.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 04-loss.
- **Video trick:** completion-satisfaction — the last badge claimed, the board full — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='badge-collect'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'badges'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# BADGE COLLECT — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the last badge claimed, the board full angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: badge-collect" when finished.
