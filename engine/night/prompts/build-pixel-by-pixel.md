...
1. `web/site/games/pixel-by-pixel/index.html` — the game (single self-contained file)
2. `web/site/games/pixel-by-pixel/copy.md` — social captions
...
## The game: Pixel by Pixel

- **Conditioned path (the hook):** the lit screen as a field of wakefulness you can't switch off wholesale. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** The screen is a dense field of lit pixels; each tap extinguishes a small cluster. The screen progressively darkens as you clear clusters, the hum quieting with each. Turn off the last lit pixel and the screen is fully black — sleep as pixel-by-pixel surrender.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** calm completion — darken the screen one tap at a time — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='pixel-by-pixel'` → loop-events.js → juice.js → share.js.
...
```
# Pixel by Pixel — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — darken the screen one tap at a time angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pixel-by-pixel" when finished.
