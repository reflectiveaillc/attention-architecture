...
1. `web/site/games/slow-tap/index.html` — the game (single self-contained file)
2. `web/site/games/slow-tap/copy.md` — social captions
...
- **Conditioned path (the hook):** fast-loop audio memes that spike heart rate by compressing beat intervals. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** The beat starts fast and you must tap along — but each perfectly-timed tap lengthens the next interval by a fraction. You are literally slowing the track with your own precision. The session ends when the tempo reaches 40 BPM and holds.
...
- Includes in order: `window.LOOP_GAME='slow-tap'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: slow-tap" when finished.
