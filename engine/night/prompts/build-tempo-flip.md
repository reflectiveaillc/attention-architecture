...
1. `web/site/games/tempo-flip/index.html` — the game (single self-contained file)
2. `web/site/games/tempo-flip/copy.md` — social captions
...
- **Conditioned path (the hook):** trending-audio swap — the same dance clip re-cut to a faster/slower sound every post. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap a pulsing dot on the beat; every 8 beats the required tempo flips — double-time, then half-time, then off-beat-only. Quick-win bursts of rhythm adaptation. The novelty is the tempo itself churning. Six tempos and it resolves.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 07-timing.
- **Video trick:** participation-comment-bait — 'I cracked at tempo 5' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='tempo-flip'` → loop-events.js → juice.js → share.js.
...
```
# TEMPO FLIP — copy
## TikTok (3 captions ≤100 chars, hook-first, participation-comment-bait — 'I cracked at tempo 5' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tempo-flip" when finished.
