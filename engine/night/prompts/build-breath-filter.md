...
1. `web/site/games/breath-filter/index.html` — the game (single self-contained file)
2. `web/site/games/breath-filter/copy.md` — social captions
...
## The game: BREATH FILTER

- **Conditioned path (the hook):** filter face-transform (the reach for a filter, replaced by the breath as the only 'adjustment'). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold the screen to inhale — the color temperature warms and the image clarifies; release to exhale and it cools and softens. There is no face to fix; the 'filter' is your own breath cycling the screen. Pace it slowly; the session closes after several breaths.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 01-soft-rpe.
...
- Includes in order: `window.LOOP_GAME='breath-filter'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: breath-filter" when finished.
