...
1. `web/site/games/shelf-fill/index.html` — the game (single self-contained file)
2. `web/site/games/shelf-fill/copy.md` — social captions
...
- **Conditioned path (the hook):** haul unboxing videos that show a shelf slowly populating with a set. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap floating items one at a time to place them onto a shelf grid. Each item lands with a soft click and the shelf fills left-to-right, top-to-bottom. When the last slot is filled the shelf glows gently and the session ends — a finite set, completely yours.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** calm completion — the shelf full at last — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='shelf-fill'` → loop-events.js → juice.js → share.js.
...
```
# SHELF FILL — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the shelf full at last angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: shelf-fill" when finished.
