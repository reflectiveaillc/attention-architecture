...
1. `web/site/games/vault-sort/index.html` — the game (single self-contained file)
2. `web/site/games/vault-sort/copy.md` — social captions
...
- **Conditioned path (the hook):** collection organization and sorting videos where chaos becomes order. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A pile of 18 collectible tiles tumbles onto the screen; drag each into the matching labeled vault slot. No timer, no score — just the satisfaction of every item in its place. When the last tile slots in the vault door closes softly and the session ends.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** calm completion — the vault organized and sealed — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='vault-sort'` → loop-events.js → juice.js → share.js.
...
```
# VAULT SORT — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the vault organized and sealed angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: vault-sort" when finished.
