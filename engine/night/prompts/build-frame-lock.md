...
1. `web/site/games/frame-lock/index.html` — the game (single self-contained file)
2. `web/site/games/frame-lock/copy.md` — social captions
...
- **Conditioned path (the hook):** 'wait for it' / perfectly-timed loop clips where one frame is the money shot. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A short animation loops; one frame per cycle has a perfect alignment. Tap exactly on that frame. Miss-timing is shown in milliseconds from perfect so you chase the zero.
...
- Includes in order: `window.LOOP_GAME='frame-lock'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: frame-lock" when finished.
