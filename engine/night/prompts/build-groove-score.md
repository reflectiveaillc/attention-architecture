...
1. `web/site/games/groove-score/index.html` — the game (single self-contained file)
2. `web/site/games/groove-score/copy.md` — social captions
...
## The game: GROOVE SCORE

- **Conditioned path (the hook):** trending "rate your rhythm" challenge clips that bait self-assessment and sharing. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 30-second groove plays; tap freely to any sub-beat you feel. An AI-feel meter scores how consistently your taps align to the micro-groove vs. the macro beat. Share your Groove Score — the shareable number drives the dare.
...
- Includes in order: `window.LOOP_GAME='groove-score'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'score'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: groove-score" when finished.
