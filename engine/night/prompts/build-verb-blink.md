...
1. `web/site/games/verb-blink/index.html` — the game (single self-contained file)
2. `web/site/games/verb-blink/copy.md` — social captions
...
- **Conditioned path (the hook):** the one-tap verb UI of Stories/Reels — tap, hold, swipe-up mapped to a dozen actions. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A single verb flashes center-screen for 2 seconds then swaps: TAP, HOLD, SWIPE UP, DON'T MOVE, DOUBLE-TAP. Execute the verb on the target before it changes; one wrong move ends it. The novelty is the rule churning faster than you can settle into it — then it stops.
- **Engine:** viral. **Circuits:** 02-novelty, 05-ease, 01-rpe.
- **Video trick:** anticipation agony — 'what's the next word gonna be' keeps eyes locked — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='verb-blink'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'verbs'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# VERB BLINK — copy
## TikTok (3 captions ≤100 chars, hook-first, anticipation agony — 'what's the next word gonna be' keeps eyes locked angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: verb-blink" when finished.
