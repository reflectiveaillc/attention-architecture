...
`engine/state/designs/cat-CAT_ID.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "CAT_ID"`. Internalize its trait, algo_hook, wean, and mechanics.
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: CAT_ID x20" when finished.
