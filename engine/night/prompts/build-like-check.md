...
1. `web/site/games/like-check/index.html` — the game (single self-contained file)
2. `web/site/games/like-check/copy.md` — social captions
...
- **Conditioned path (the hook):** engagement check impulse. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Your post sits center-screen and the heart count flickers. Tap only when it actually rises; never tap on a hold or a fake surge. Each round adds more phantom jumps.
...
- Includes in order: `window.LOOP_GAME='like-check'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'likes'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: like-check" when finished.
