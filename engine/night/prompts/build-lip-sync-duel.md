...
1. `web/site/games/lip-sync-duel/index.html` — the game (single self-contained file)
2. `web/site/games/lip-sync-duel/copy.md` — social captions
...
- **Mechanic:** Lyrics scroll across a split screen. Tap when the active word crosses your marker to 'duet' perfectly. Decoy words flash early to bait a bad tap; each round scrolls faster.
- **Engine:** viral. **Circuits:** 03-social, 01-rpe, 05-ease.
...
- Includes in order: `window.LOOP_GAME='lip-sync-duel'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'bars'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: lip-sync-duel" when finished.
