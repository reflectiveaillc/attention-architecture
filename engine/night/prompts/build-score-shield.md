...
1. `web/site/games/score-shield/index.html` — the game (single self-contained file)
2. `web/site/games/score-shield/copy.md` — social captions
...
## The game: Score Shield

- **Conditioned path (the hook):** defending a pinned top comment / rank. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Set a score as your 'shield' and share the link. Challengers get one run to break it. If they fail, your shield stands; if they win, their score becomes the new shield and you get one notification, then it's done.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 08-investment.
...
- Includes in order: `window.LOOP_GAME='score-shield'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: score-shield" when finished.
