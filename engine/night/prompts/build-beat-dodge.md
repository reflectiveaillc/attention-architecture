...
1. `web/site/games/beat-dodge/index.html` — the game (single self-contained file)
2. `web/site/games/beat-dodge/copy.md` — social captions
...
- **Conditioned path (the hook):** beat-drop edits where obstacles slam on the drop. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A beat line pulses at the bottom; tap to duck under barriers that slam down on every beat. The BPM climbs and off-beat traps force late reactions.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 04-loss.
- **Video trick:** beat-drop anticipation — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='beat-dodge'` → loop-events.js → juice.js → share.js.
...
```
# BEAT DODGE — copy
## TikTok (3 captions ≤100 chars, hook-first, beat-drop anticipation angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: beat-dodge" when finished.
