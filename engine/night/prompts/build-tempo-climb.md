...
1. `web/site/games/tempo-climb/index.html` — the game (single self-contained file)
2. `web/site/games/tempo-climb/copy.md` — social captions
...
## The game: TEMPO CLIMB

- **Conditioned path (the hook):** trending tempo-ramp audio where the beat accelerates until the listener physically can't keep up. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Dots scroll down a lane at the beat tempo; tap each as it crosses the line. Every 8 beats the BPM notches up. Miss two in a row and the track cuts. How far up the tempo ladder can you ride before it breaks you?
- **Engine:** viral. **Circuits:** 01-rpe, 07-timing, 04-loss.
...
- Includes in order: `window.LOOP_GAME='tempo-climb'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'BPM'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tempo-climb" when finished.
