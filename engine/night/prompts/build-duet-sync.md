...
1. `web/site/games/duet-sync/index.html` — the game (single self-contained file)
2. `web/site/games/duet-sync/copy.md` — social captions
...
- **Conditioned path (the hook):** duet/remix (the TikTok split-screen duet where you answer the other half on beat). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** The left half of the screen plays a beat pattern; the right half is yours. Tap your half to lay your hits so they land exactly in the gaps of the left beat. A sync meter scores how tightly your half locks into theirs.
...
- Includes in order: `window.LOOP_GAME='duet-sync'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: duet-sync" when finished.
