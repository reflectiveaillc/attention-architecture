...
1. `web/site/games/known-next/index.html` — the game (single self-contained file)
2. `web/site/games/known-next/copy.md` — social captions
...
- **Conditioned path (the hook):** the story-bar scan — checking who posted so you know exactly what comes next before you watch. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A sequence of colored tiles is revealed one at a time in a known pattern. After four tiles the player must predict tile five by tapping a color. Correct: sequence continues and builds. Wrong: the predictable pattern is broken and the session ends. Score is the longest streak of correct predictions — comforted by knowing what comes next.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 02-novelty.
- **Video trick:** participation-comment-bait — "call the next color before it flips" — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='known-next'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'streak'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# KNOWN NEXT — copy
## TikTok (3 captions ≤100 chars, hook-first, participation-comment-bait — "call the next color before it flips" angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: known-next" when finished.
