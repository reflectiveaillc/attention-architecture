...
1. `web/site/games/color-flip/index.html` — the game (single self-contained file)
2. `web/site/games/color-flip/copy.md` — social captions
...
- **Conditioned path (the hook):** thumbnail churn — trending color palettes that re-novlety the whole feed at once. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** The whole screen cycles through colors; a target color flashes for 1 second — tap the instant the background matches it. The target color changes each round and the cycle speeds up. 12 quick-win bursts, then a clean stop. The novelty hit is a fresh target every beat.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 07-timing.
- **Video trick:** near-perfection itch — the almost-match you mistimed haunts the rewatch — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='color-flip'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'matches'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# COLOR FLIP — copy
## TikTok (3 captions ≤100 chars, hook-first, near-perfection itch — the almost-match you mistimed haunts the rewatch angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: color-flip" when finished.
