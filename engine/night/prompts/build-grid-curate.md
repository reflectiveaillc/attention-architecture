...
1. `web/site/games/grid-curate/index.html` — the game (single self-contained file)
2. `web/site/games/grid-curate/copy.md` — social captions
...
## The game: GRID CURATE

- **Conditioned path (the hook):** aesthetic-grid curation (the IG profile 3x3 grid people agonize over for color harmony). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 3x3 profile grid fills with random photo tiles; tap any two to swap them. A live harmony meter reads the color balance of the grid — chase the highest harmony % you can before you lock it.
...
- Includes in order: `window.LOOP_GAME='grid-curate'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: grid-curate" when finished.
