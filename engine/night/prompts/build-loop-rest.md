...
1. `web/site/games/loop-rest/index.html` — the game (single self-contained file)
2. `web/site/games/loop-rest/copy.md` — social captions
...
- **Mechanic:** A slow loop of nature shapes repeats. The instruction is to watch one full loop, then close your eyes for the next. The loop is the cue; the closed-eye pause is the game. No input required.
...
- Includes in order: `window.LOOP_GAME='loop-rest'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: loop-rest" when finished.
