...
1. `web/site/games/badge-panic/index.html` — the game (single self-contained file)
2. `web/site/games/badge-panic/copy.md` — social captions
...
- **Mechanic:** A number ticks up in the corner like unread notifications. Tap to clear it, but only when the badge turns solid red. Tap a phantom shimmer and you lose a life. The compulsion to clear is the trap.
...
- Includes in order: `window.LOOP_GAME='badge-panic'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: badge-panic" when finished.
