...
1. `web/site/games/spot-the-change/index.html` — the game (single self-contained file)
2. `web/site/games/spot-the-change/copy.md` — social captions
...
## The game: SPOT THE CHANGE

- **Conditioned path (the hook):** 'before and after' transformation clips that train the eye to hunt for what changed. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Two nearly identical screens flash alternating; tap the element that changed before the cycle repeats. Each round the change shrinks and the cycle speeds — the recall reflex is exercised, not just baited.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 06-zeigarnik.
- **Video trick:** play-along comment bait — 'pause and spot it before I reveal' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='spot-the-change'` → loop-events.js → juice.js → share.js.
...
```
# SPOT THE CHANGE — copy
## TikTok (3 captions ≤100 chars, hook-first, play-along comment bait — 'pause and spot it before I reveal' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: spot-the-change" when finished.
