...
1. `web/site/games/map-recall/index.html` — the game (single self-contained file)
2. `web/site/games/map-recall/copy.md` — social captions
...
- **Conditioned path (the hook):** viral 'place the flag / country quiz' recall posts that weaponize geography memory for shares. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A simple illustrated map appears for 5 seconds with 5 marked spots; the map fades and the thumb must drag pins to the remembered positions. Distance from each original spot is scored — the dare unit is total error in map-units.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 03-social.
- **Video trick:** competitive rank — 'total error 47, think you can beat it?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='map-recall'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'error pts'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# MAP RECALL — copy
## TikTok (3 captions ≤100 chars, hook-first, competitive rank — 'total error 47, think you can beat it?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: map-recall" when finished.
