...
1. `web/site/games/off-beat-trap/index.html` — the game (single self-contained file)
2. `web/site/games/off-beat-trap/copy.md` — social captions
...
## The game: OFF-BEAT TRAP

- **Conditioned path (the hook):** syncopated audio memes that trip up listeners with deliberate off-beat hooks. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Most beats signal with a flash; the scoring beat is silent — tap in the gap between flashes. Tapping on any lit flash scores zero. The mechanic trains the body to feel the off-beat the way the track buries it.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 07-timing.
- **Video trick:** shock-honesty — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='off-beat-trap'` → loop-events.js → juice.js → share.js.
...
```
# OFF-BEAT TRAP — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: off-beat-trap" when finished.
