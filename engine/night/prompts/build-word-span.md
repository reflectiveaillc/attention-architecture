...
1. `web/site/games/word-span/index.html` — the game (single self-contained file)
2. `web/site/games/word-span/copy.md` — social captions
...
- **Conditioned path (the hook):** 'how many can you remember?' recall-challenge posts that spread via score-sharing. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A list of words flashes for 3 seconds, then vanishes; tap the words that appeared from a larger set of distractors. The list grows by one word each round until a miss prints the working-memory span — the honest score the brain earned.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 03-social.
- **Video trick:** benchmark ego bait — 'I hit 9, what\'s yours?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='word-span'` → loop-events.js → juice.js → share.js.
...
```
# WORD SPAN — copy
## TikTok (3 captions ≤100 chars, hook-first, benchmark ego bait — 'I hit 9, what\'s yours?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: word-span" when finished.
