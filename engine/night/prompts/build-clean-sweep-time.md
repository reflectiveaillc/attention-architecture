...
1. `web/site/games/clean-sweep-time/index.html` — the game (single self-contained file)
2. `web/site/games/clean-sweep-time/copy.md` — social captions
...
## The game: CLEAN SWEEP

- **Conditioned path (the hook):** speed-cleaning and clutter-blitz videos that end before every surface is wiped. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A cluttered room grid shows dirty zones; swipe to clean each cell in the fastest possible order. Your completion time is recorded and displayed — dare a friend to clean the same room faster.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 03-social.
...
- Includes in order: `window.LOOP_GAME='clean-sweep-time'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: clean-sweep-time" when finished.
