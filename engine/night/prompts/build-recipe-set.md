...
1. `web/site/games/recipe-set/index.html` — the game (single self-contained file)
2. `web/site/games/recipe-set/copy.md` — social captions
...
- **Conditioned path (the hook):** recipe collection and cookbook hauls where one dish is always missing from the repertoire. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Ingredient tiles drift across the screen; tap each one to add it to a recipe card. Each full recipe card unlocks and files into a recipe book with 8 slots. Fill the book’s 8 recipe slots and the book snaps shut — a finite, free, completable cookbook.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** calm completion — the recipe book full and filed — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='recipe-set'` → loop-events.js → juice.js → share.js.
...
```
# RECIPE SET — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the recipe book full and filed angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: recipe-set" when finished.
