...
1. `web/site/games/bubble-wrap-calm/index.html` — the game (single self-contained file)
2. `web/site/games/bubble-wrap-calm/copy.md` — social captions
...
- **Conditioned path (the hook):** bubble-wrap ASMR loops that play endlessly — the game version ends on its own terms instead. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A single large sheet of bubble wrap fills the canvas. Pop at your own pace — no timer, no score. After the last bubble pops the sheet folds itself away with a soft rustle and a gentle done-tone plays. The session has an actual end.
...
- Includes in order: `window.LOOP_GAME='bubble-wrap-calm'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: bubble-wrap-calm" when finished.
