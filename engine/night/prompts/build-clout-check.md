...
1. `web/site/games/clout-check/index.html` — the game (single self-contained file)
2. `web/site/games/clout-check/copy.md` — social captions
...
## The game: Clout Check

- **Conditioned path (the hook):** verified-badge / clout verification. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A friend's score appears as a blue-check dare. Tap to attempt the same 30-second challenge once. The game stamps 'verified' if you beat it, 'not verified' if you don't — one try, then the dare is settled.
- **Engine:** viral. **Circuits:** 03-social, 06-zeigarnik, 01-rpe.
...
- Includes in order: `window.LOOP_GAME='clout-check'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'checks'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: clout-check" when finished.
