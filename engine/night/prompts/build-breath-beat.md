...
1. `web/site/games/breath-beat/index.html` — the game (single self-contained file)
2. `web/site/games/breath-beat/copy.md` — social captions
...
## The game: BREATH BEAT

- **Conditioned path (the hook):** binaural / breathing-paced audio loops that hijack the inhale-exhale cycle into endless replay. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A soft pulse sets the inhale; hold during the beat, release on the rest. Each matched breath-beat pair calms the visual — the screen warms and slows. After 10 matched breath cycles the track ends and the screen holds still.
...
- Includes in order: `window.LOOP_GAME='breath-beat'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: breath-beat" when finished.
