...
1. `web/site/games/autoplay-stop/index.html` — the game (single self-contained file)
2. `web/site/games/autoplay-stop/copy.md` — social captions
...
## The game: Autoplay Stop

- **Conditioned path (the hook):** autoplay next. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Clips autoplay back-to-back. Tap exactly in the black gap between videos to 'pause' and score. The gap shrinks and fake transitions flash; stopping on a transition ends your streak.
- **Engine:** viral. **Circuits:** 05-ease, 01-rpe, 06-zeigarnik.
...
- Includes in order: `window.LOOP_GAME='autoplay-stop'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'stops'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: autoplay-stop" when finished.
