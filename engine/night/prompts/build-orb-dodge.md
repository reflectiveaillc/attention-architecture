...
1. `web/site/games/orb-dodge/index.html` — the game (single self-contained file)
2. `web/site/games/orb-dodge/copy.md` — social captions
...
- **Conditioned path (the hook):** ASMR/satisfying orb and ball-physics videos turned threat. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A glowing orb rockets toward center screen; tap to sidestep left or right at the last moment. The orb telegraphs late and fake-outs spin the wrong direction.
...
- Includes in order: `window.LOOP_GAME='orb-dodge'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'dodges'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: orb-dodge" when finished.
