...
1. `web/site/games/fence-walk/index.html` — the game (single self-contained file)
2. `web/site/games/fence-walk/copy.md` — social captions
...
- **Conditioned path (the hook):** the predictable-feed craving — seeking a feed that only shows familiar things, no surprises. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A path stretches forward on a gentle loop; drag your thumb to walk it. The path always curves the same way, reveals the same soft scenery — a fence, a tree, a gate. Nothing unexpected ever appears. The walk has no destination and no end unless you lift your thumb. Predictable sensory input, fully controllable pace.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 02-novelty.
- **Video trick:** calm completion — the reassuring sameness of the loop — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='fence-walk'` → loop-events.js → juice.js → share.js.
...
```
# FENCE WALK — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the reassuring sameness of the loop angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: fence-walk" when finished.
