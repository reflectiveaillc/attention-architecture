...
1. `web/site/games/undo-stack/index.html` — the game (single self-contained file)
2. `web/site/games/undo-stack/copy.md` — social captions
...
- **Conditioned path (the hook):** the re-read-before-send anxiety loop — drafting, deleting, redrafting because nothing feels safe to commit. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Soft action tiles stack up as you tap them. Every action is visible in a stack on the right. Swipe the stack to undo any step — every move is fully reversible in any order. There is no wrong sequence, no commit button unless you choose. The whole point is that nothing is permanent until you want it to be.
- **Engine:** calm. **Circuits:** 05-ease, 06-zeigarnik, 08-investment.
- **Video trick:** calm completion — the satisfaction of knowing every step can be taken back — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='undo-stack'` → loop-events.js → juice.js → share.js.
...
```
# UNDO STACK — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the satisfaction of knowing every step can be taken back angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: undo-stack" when finished.
