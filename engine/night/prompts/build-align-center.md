...
1. `web/site/games/align-center/index.html` — the game (single self-contained file)
2. `web/site/games/align-center/copy.md` — social captions
...
## The game: ALIGN CENTER

- **Conditioned path (the hook):** slightly-off text and design layouts that bother you on every scroll. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A block of text or an icon sits visibly off-center on a canvas; drag it millimeter by millimeter until the alignment guides snap and the element locks exactly in the middle. The tension dissolves audibly when center is found.
...
- Includes in order: `window.LOOP_GAME='align-center'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: align-center" when finished.
