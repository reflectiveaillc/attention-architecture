...
1. `web/site/games/red-dot-run/index.html` — the game (single self-contained file)
2. `web/site/games/red-dot-run/copy.md` — social captions
...
## The game: RED DOT RUN

- **Conditioned path (the hook):** red notification dots that trigger an instant tap reflex. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Red dots pop up randomly across the canvas; tap each before it disappears. Phantom grey dots flicker to bait false taps, and the spawn rate escalates.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 04-loss.
- **Video trick:** badge compulsion — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='red-dot-run'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'dots'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# RED DOT RUN — copy
## TikTok (3 captions ≤100 chars, hook-first, badge compulsion angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: red-dot-run" when finished.
