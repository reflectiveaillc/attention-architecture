...
`engine/state/designs/cat-autism.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "autism"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: autism · label: Patterns & Systems · engine_lean: calm
trait: systemizing brain; pattern recognition, rule-discovery, predictable sensory input, routine mastery
algo_hook: fed deep-dive niche rabbit-holes and hyper-specific loops that reward pattern-hunting without end
wean: a self-contained system to master — discover the rule, complete the pattern, predictable sensory feedback, a routine with a clear finish line
mechanics: pattern completion, rule discovery, sorting systems, sequence logic, consistent sensory feedback, categorization
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: autism x20" when finished.
