...
1. `web/site/games/tide-pull/index.html` — the game (single self-contained file)
2. `web/site/games/tide-pull/copy.md` — social captions
...
- **Conditioned path (the hook):** the pull-to-refresh conditioned gesture — the swipe down that promises something new but delivers more chaos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Pull down slowly and the tide rises up the screen; release and it ebbs back. The tide always behaves the same way — no variables, no surprise. Pull to bring it in, release to let it out. After six slow pull-release cycles the tide settles at a calm midline and the session ends with a soft wash sound. The gesture redeemed.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 06-zeigarnik.
- **Video trick:** calm completion — six tides and then the water settles — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='tide-pull'` → loop-events.js → juice.js → share.js.
...
```
# TIDE PULL — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — six tides and then the water settles angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tide-pull" when finished.
