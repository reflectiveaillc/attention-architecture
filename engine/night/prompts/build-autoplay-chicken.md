...
1. `web/site/games/autoplay-chicken/index.html` — the game (single self-contained file)
2. `web/site/games/autoplay-chicken/copy.md` — social captions
...
- **Mechanic:** A countdown bar sweeps across the bottom like the next-video autoplay timer. Tap to stop it as close to zero as possible without letting the next 'video' start. The screen jitters as if it is about to load.
...
- Includes in order: `window.LOOP_GAME='autoplay-chicken'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: autoplay-chicken" when finished.
