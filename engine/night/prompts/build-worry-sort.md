...
1. `web/site/games/worry-sort/index.html` — the game (single self-contained file)
2. `web/site/games/worry-sort/copy.md` — social captions
...
- **Conditioned path (the hook):** the news-feed scan for reassurance — reading headlines to confirm the world is still manageable. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Soft text cards drift up labeled with abstract worry words. Tap each to gently place it into one of two jars: "in my control" or "not in my control." No wrong answers, no timer. When all cards are sorted, both jars seal with a quiet click and the screen rests. Control imposed on chaos.
- **Engine:** calm. **Circuits:** 05-ease, 06-zeigarnik, 08-investment.
- **Video trick:** calm completion — both jars filling is the satisfying close — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='worry-sort'` → loop-events.js → juice.js → share.js.
...
```
# WORRY SORT — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — both jars filling is the satisfying close angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: worry-sort" when finished.
