...
1. `web/site/games/rubik-sequence/index.html` — the game (single self-contained file)
2. `web/site/games/rubik-sequence/copy.md` — social captions
...
## The game: Rubik Sequence

- **Conditioned path (the hook):** Rubik's solve / speedcubing PB clips that make you want a flawless memorized run. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A sequence of colored faces flashes; repeat it with taps in the exact order. Each perfect recall adds one more move. PB is the longest flawless sequence you can chain.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 06-zeigarnik.
- **Video trick:** sequence mastery — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='rubik-sequence'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'moves'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Rubik Sequence — copy
## TikTok (3 captions ≤100 chars, hook-first, sequence mastery angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rubik-sequence" when finished.
