...
`engine/state/designs/cat-sensory.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "sensory"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: sensory · label: Sensory / ASMR · engine_lean: calm
trait: craves tactile and audio satisfaction; the squish, the pop, the crunch
algo_hook: fed ASMR, slime, kinetic-sand, and pop-it loops that deliver sensory hits on an endless scroll
wean: generative tactile toys with rich synthesized sound — pop, stretch, ripple — a self-contained sensory session
mechanics: pop/squish, ripple/goo, tactile feedback, sound-reactive, generative texture
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: sensory x20" when finished.
