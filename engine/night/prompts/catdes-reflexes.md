...
`engine/state/designs/cat-reflexes.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "reflexes"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: reflexes · label: Reflexes · engine_lean: viral
trait: fast-twitch reaction; the brain that thrives on split-second response
algo_hook: fed rapid-cut hype edits and jump-scare bait tuned to their trigger speed
wean: give them the pure reaction hit (react-or-die, dodge, catch, F1 starts) in a bounded 60s burst that ENDS, instead of an infinite reflex-baiting feed
mechanics: reaction time, dodge, catch/deflect, stop-the-marker, last-instant tap
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: reflexes x20" when finished.
