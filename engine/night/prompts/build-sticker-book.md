...
1. `web/site/games/sticker-book/index.html` — the game (single self-contained file)
2. `web/site/games/sticker-book/copy.md` — social captions
...
## The game: STICKER BOOK

- **Conditioned path (the hook):** sticker-collection hauls and blind-pack openings that sell the incomplete album. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Sticker tiles flutter in one at a time; tap each to peel and place it on the correct blank in the album spread. No duplicates, no blank that never fills. When the last sticker lands the album shuts with a soft snap and the session ends.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** calm completion — the full album, finally sealed — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='sticker-book'` → loop-events.js → juice.js → share.js.
...
```
# STICKER BOOK — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the full album, finally sealed angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: sticker-book" when finished.
