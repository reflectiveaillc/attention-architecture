...
1. `web/site/games/infinite-fall/index.html` — the game (single self-contained file)
2. `web/site/games/infinite-fall/copy.md` — social captions
...
## The game: Infinite Fall

- **Conditioned path (the hook):** infinite scroll. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A tile falls endlessly through a feed of gaps. Swipe left/right to steer it through the open slots. Speed rises the deeper you go, turning the scroll habit into a vertical survival reflex.
...
- Includes in order: `window.LOOP_GAME='infinite-fall'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'floors'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: infinite-fall" when finished.
