...
1. `web/site/games/train-timetable/index.html` — the game (single self-contained file)
2. `web/site/games/train-timetable/copy.md` — social captions
...
## The game: TRAIN TIMETABLE

- **Conditioned path (the hook):** system-mastery loops — the algorithm that surfaces transit-nerd, schedule-nerd, or logistics-nerd content to people who love knowing exactly how a system ticks. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A timetable grid shows departure slots; drag each train token into its correct departure slot so no two trains share a platform at the same time. The rule is the constraint system itself — figure it out, fill it in, and the last train departs on a clean chime.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 05-ease, 08-investment.
- **Video trick:** completion-satisfaction — every slot filled, system running smooth — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='train-timetable'` → loop-events.js → juice.js → share.js.
...
```
# TRAIN TIMETABLE — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — every slot filled, system running smooth angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: train-timetable" when finished.
