...
1. `web/site/games/slow-scroll/index.html` — the game (single self-contained file)
2. `web/site/games/slow-scroll/copy.md` — social captions
...
## The game: Slow Scroll

- **Conditioned path (the hook):** read the caption to the end" posts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A short story advances one gentle tap at a time. The last line delivers the resolution, then the screen fades to close; the payoff lands and the session ends.
...
- Includes in order: `window.LOOP_GAME='slow-scroll'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: slow-scroll" when finished.
