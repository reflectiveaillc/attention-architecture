...
1. `web/site/games/repost-train/index.html` — the game (single self-contained file)
2. `web/site/games/repost-train/copy.md` — social captions
...
## The game: Repost Train

- **Conditioned path (the hook):** repost / share forwarding chain. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A post rolls along a track of friend stations; tap each station to repost it forward — the chain counter grows, but miss a station and the train halts.
...
- Includes in order: `window.LOOP_GAME='repost-train'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'reposts'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: repost-train" when finished.
