...
1. `web/site/games/duet-trace/index.html` — the game (single self-contained file)
2. `web/site/games/duet-trace/copy.md` — social captions
...
- **Conditioned path (the hook):** duet/remix. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A ghost hand traces a shape; your thumb must mirror it in real time. The closer the overlap, the higher the 'duet score'. It feels like TikTok stitch muscle memory turned competitive.
- **Engine:** viral. **Circuits:** 03-social, 01-rpe, 08-investment.
...
- Includes in order: `window.LOOP_GAME='duet-trace'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: duet-trace" when finished.
