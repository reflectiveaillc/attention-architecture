...
1. `web/site/games/color-pop/index.html` — the game (single self-contained file)
2. `web/site/games/color-pop/copy.md` — social captions
...
- **Conditioned path (the hook):** fast-paced 'find the odd one out' quiz videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A swarm of colored dots pops on screen; tap the one target color before it vanishes. Decoy colors look almost identical and the swarm grows each round.
...
- Includes in order: `window.LOOP_GAME='color-pop'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'pops'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: color-pop" when finished.
