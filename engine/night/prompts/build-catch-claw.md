...
1. `web/site/games/catch-claw/index.html` — the game (single self-contained file)
2. `web/site/games/catch-claw/copy.md` — social captions
...
- **Conditioned path (the hook):** satisfying grab/catch and unboxing drop videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Prizes fall from above; tap to extend the claw and catch each prize at the exact catch height. Catching too high or too low drops it, and the drop speed increases.
...
- Includes in order: `window.LOOP_GAME='catch-claw'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: catch-claw" when finished.
