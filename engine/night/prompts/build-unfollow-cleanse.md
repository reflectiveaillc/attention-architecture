...
1. `web/site/games/unfollow-cleanse/index.html` — the game (single self-contained file)
2. `web/site/games/unfollow-cleanse/copy.md` — social captions
...
## The game: Unfollow Cleanse

- **Conditioned path (the hook):** follower-count accumulation anxiety. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A follower list drifts down; tap gently to unfollow each account and watch your follower count fall — the relief is the count going DOWN. Breath-paced, no timer.
- **Engine:** calm. **Circuits:** 01-soft-rpe, 05-ease, 08-garden.
...
- Includes in order: `window.LOOP_GAME='unfollow-cleanse'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: unfollow-cleanse" when finished.
