...
1. `web/site/games/maze-exit/index.html` — the game (single self-contained file)
2. `web/site/games/maze-exit/copy.md` — social captions
...
- **Conditioned path (the hook):** satisfying maze solve videos. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag one thumb through a maze from start to finish. Each correct turn reveals the next corridor; reaching the center shows the exit payoff and ends the run.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 05-ease.
- **Video trick:** will it find the way out — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='maze-exit'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'mazes'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Maze Exit — copy
## TikTok (3 captions ≤100 chars, hook-first, will it find the way out angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: maze-exit" when finished.
