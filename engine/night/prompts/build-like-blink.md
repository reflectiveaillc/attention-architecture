...
1. `web/site/games/like-blink/index.html` — the game (single self-contained file)
2. `web/site/games/like-blink/copy.md` — social captions
...
- **Conditioned path (the hook):** double-tap like. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hearts drift across the canvas; double-tap them before they fade. The heart you missed stays ghosted for a beat, clawing at the FOMO of a lost like.
...
- Includes in order: `window.LOOP_GAME='like-blink'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: like-blink" when finished.
