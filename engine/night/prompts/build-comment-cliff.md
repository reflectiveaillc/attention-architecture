...
1. `web/site/games/comment-cliff/index.html` — the game (single self-contained file)
2. `web/site/games/comment-cliff/copy.md` — social captions
...
## The game: Comment Cliff

- **Conditioned path (the hook):** 'more replies' truncation. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A comment thread unrolls line by line. Tap to expand the next line before it collapses. Each level hides lines faster and plants fake 'read more' traps that steal your time.
...
- Includes in order: `window.LOOP_GAME='comment-cliff'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'replies'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: comment-cliff" when finished.
