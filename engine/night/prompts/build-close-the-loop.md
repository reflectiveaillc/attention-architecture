...
1. `web/site/games/close-the-loop/index.html` — the game (single self-contained file)
2. `web/site/games/close-the-loop/copy.md` — social captions
...
## The game: CLOSE THE LOOP

- **Conditioned path (the hook):** open-tab and notification-pile anxiety that the feed farms instead of resolving. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Circles with small gaps drift across the canvas; drag the open end of each arc closed so the circle snaps shut with a soft ring tone. Each closed loop dims and sinks, and when all loops are closed the canvas goes quiet.
...
- Includes in order: `window.LOOP_GAME='close-the-loop'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: close-the-loop" when finished.
