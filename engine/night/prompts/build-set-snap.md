...
1. `web/site/games/set-snap/index.html` — the game (single self-contained file)
2. `web/site/games/set-snap/copy.md` — social captions
...
- **Conditioned path (the hook):** gacha unboxing where each pull dangles the missing piece of the set. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap to draw a card from a no-cash deck of 12. Each new card snaps into a visible set grid; duplicates highlight in red so you feel the missing slots. Pull until the full set is complete — the deck has a bottom and every slot fills.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 08-investment.
- **Video trick:** completion-satisfaction — the last card snapping into place — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='set-snap'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'pieces'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# SET SNAP — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the last card snapping into place angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: set-snap" when finished.
