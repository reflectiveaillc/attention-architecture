...
1. `web/site/games/one-cut/index.html` — the game (single self-contained file)
2. `web/site/games/one-cut/copy.md` — social captions
...
- **Conditioned path (the hook):** satisfying perfect-cut / restock ASMR that sells the fantasy of one flawless motion. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A colored bar extends across the screen; tap once to stop it exactly at the target length. Length error is your score — the PB is the lowest total error across ten cuts.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 04-loss.
- **Video trick:** satisfaction-surge — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='one-cut'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'mm'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# One Cut — copy
## TikTok (3 captions ≤100 chars, hook-first, satisfaction-surge angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: one-cut" when finished.
