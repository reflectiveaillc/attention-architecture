...
1. `web/site/games/echo-fade/index.html` — the game (single self-contained file)
2. `web/site/games/echo-fade/copy.md` — social captions
...
- **Conditioned path (the hook):** nostalgia-ASMR 'remember this sound?' posts that park a memory and reopen it gently. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A short visual sequence pulses onto the screen, one element at a time, slowly; after it fades, the thumb traces the elements back in order at any pace. Each correct touch plays a soft chime. The sequence is short enough to always succeed — the exercise is pure gentle recall, not a test.
...
- Includes in order: `window.LOOP_GAME='echo-fade'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: echo-fade" when finished.
