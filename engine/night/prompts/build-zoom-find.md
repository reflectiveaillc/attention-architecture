...
1. `web/site/games/zoom-find/index.html` — the game (single self-contained file)
2. `web/site/games/zoom-find/copy.md` — social captions
...
- **Conditioned path (the hook):** zoom in on this" detail reveal posts. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Pinch to zoom into a busy photo. Find the hidden target detail and the image resolves into a surprising caption; the loop closes when the detail is found.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 06-zeigarnik.
- **Video trick:** look closer — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='zoom-find'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'finds'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Zoom Find — copy
## TikTok (3 captions ≤100 chars, hook-first, look closer angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: zoom-find" when finished.
