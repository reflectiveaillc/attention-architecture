...
1. `web/site/games/badge-sunset/index.html` — the game (single self-contained file)
2. `web/site/games/badge-sunset/copy.md` — social captions
...
- **Mechanic:** Red badges drift in from the edges. Don't tap them. They fade on their own if you wait. Each badge that dissolves by patience blooms into a soft particle and clears the corner.
...
- Includes in order: `window.LOOP_GAME='badge-sunset'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: badge-sunset" when finished.
