...
1. `web/site/games/circle-judge/index.html` — the game (single self-contained file)
2. `web/site/games/circle-judge/copy.md` — social captions
...
## The game: Circle Judge

- **Conditioned path (the hook):** perfect-circle challenges / handwriting perfection clips where the flawless stroke gets the clout. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Draw a circle in one thumb stroke. The game scores roundness, closure, and radius consistency as a single perfection %. PB is the highest %; 100 is the impossible ceiling.
- **Engine:** viral. **Circuits:** 01-rpe, 08-investment, 05-ease.
...
- Includes in order: `window.LOOP_GAME='circle-judge'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: circle-judge" when finished.
