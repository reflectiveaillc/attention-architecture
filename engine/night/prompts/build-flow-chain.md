...
1. `web/site/games/flow-chain/index.html` — the game (single self-contained file)
2. `web/site/games/flow-chain/copy.md` — social captions
...
- **Conditioned path (the hook):** looping ASMR beats that lock the brain into a repetition trance with no exit. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap a sequence of color-coded zones on the beat to build a chain. Each perfect hit extends the chain and deepens the visual glow. One off-beat tap breaks the chain and scores zero for that segment. 64-beat track, then it resolves.
- **Engine:** viral. **Circuits:** 01-rpe, 05-ease, 08-investment.
...
- Includes in order: `window.LOOP_GAME='flow-chain'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: flow-chain" when finished.
