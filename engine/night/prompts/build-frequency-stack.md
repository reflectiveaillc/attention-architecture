...
1. `web/site/games/frequency-stack/index.html` — the game (single self-contained file)
2. `web/site/games/frequency-stack/copy.md` — social captions
...
## The game: FREQUENCY STACK

- **Conditioned path (the hook):** data-visualisation rabbit-holes — the thread mapping frequency distributions and histograms of niche datasets. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tokens fall and you drag each to the correct frequency column on a histogram you build in real time. The distribution has a hidden shape (bell curve, skew, bimodal) you discover only as you place items. When the last token is placed the histogram shows its shape and a label prints — you figured out the distribution.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 01-rpe, 08-investment.
- **Video trick:** shock-honesty — the shape was bimodal the whole time — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='frequency-stack'` → loop-events.js → juice.js → share.js.
...
```
# FREQUENCY STACK — copy
## TikTok (3 captions ≤100 chars, hook-first, shock-honesty — the shape was bimodal the whole time angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: frequency-stack" when finished.
