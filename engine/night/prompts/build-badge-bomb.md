...
1. `web/site/games/badge-bomb/index.html` — the game (single self-contained file)
2. `web/site/games/badge-bomb/copy.md` — social captions
...
- **Conditioned path (the hook):** red badge compulsion. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Red notification dots swarm the screen. Tap them in order before they multiply. Tapping the wrong dot spawns three more. The brain cannot ignore a badge; here, clearing it is the game.
- **Engine:** viral. **Circuits:** 02-novelty, 04-loss, 06-zeigarnik.
...
- Includes in order: `window.LOOP_GAME='badge-bomb'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: badge-bomb" when finished.
