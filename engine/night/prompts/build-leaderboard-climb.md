...
1. `web/site/games/leaderboard-climb/index.html` — the game (single self-contained file)
2. `web/site/games/leaderboard-climb/copy.md` — social captions
...
## The game: Leaderboard Climb

- **Conditioned path (the hook):** leaderboard rank envy. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A ranked list scrolls past; tap your row to climb past each name above you, but every tap costs more energy and rivals re-pass you the moment you hesitate.
- **Engine:** viral. **Circuits:** 03-social, 01-rpe, 04-loss.
...
- Includes in order: `window.LOOP_GAME='leaderboard-climb'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: leaderboard-climb" when finished.
