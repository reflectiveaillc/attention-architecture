...
1. `web/site/games/filter-glow/index.html` — the game (single self-contained file)
2. `web/site/games/filter-glow/copy.md` — social captions
...
## The game: FILTER GLOW

- **Conditioned path (the hook):** filter face-transform (IG/TikTok beauty filters that smooth, enlarge, and glow-up the face). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A face sits in frame; every tap stacks another random filter layer (smooth, saturate, enlarge eyes, jaw-sharpen). Keep tapping to pile on vanity transforms until the face glitches and 'breaks' — the score is how many layers you stacked before it shatters.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 03-social.
- **Video trick:** vanity arms-race (invented) — push the filter to absurdity, the break is the punchline — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='filter-glow'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'layers'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# FILTER GLOW — copy
## TikTok (3 captions ≤100 chars, hook-first, vanity arms-race (invented) — push the filter to absurdity, the break is the punchline angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: filter-glow" when finished.
