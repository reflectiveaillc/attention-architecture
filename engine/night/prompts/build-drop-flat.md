...
1. `web/site/games/drop-flat/index.html` — the game (single self-contained file)
2. `web/site/games/drop-flat/copy.md` — social captions
...
- **Conditioned path (the hook):** satisfying landing / parkour perfect-landing clips where flat is the whole win. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A block falls and spins; tap to lock its rotation so it lands perfectly flat. Angle error is your score — 0 degrees is the PB, and falls get faster each attempt.
- **Engine:** viral. **Circuits:** 01-rpe, 05-ease, 04-loss.
- **Video trick:** satisfying landing — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='drop-flat'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'degrees'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Drop Flat — copy
## TikTok (3 captions ≤100 chars, hook-first, satisfying landing angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: drop-flat" when finished.
