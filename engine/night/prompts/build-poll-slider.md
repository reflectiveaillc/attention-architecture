...
1. `web/site/games/poll-slider/index.html` — the game (single self-contained file)
2. `web/site/games/poll-slider/copy.md` — social captions
...
## The game: Poll Slider

- **Conditioned path (the hook):** story emoji-slider poll. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A slider sweeps 0-100% while a poll's target % drifts; tap to lock the slider exactly on the moving target to win the vote — overshoot or undershoot and you lose.
...
- Includes in order: `window.LOOP_GAME='poll-slider'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: poll-slider" when finished.
