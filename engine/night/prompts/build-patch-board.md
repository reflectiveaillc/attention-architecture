...
1. `web/site/games/patch-board/index.html` — the game (single self-contained file)
2. `web/site/games/patch-board/copy.md` — social captions
...
## The game: PATCH BOARD

- **Conditioned path (the hook):** merit badge and achievement patch boards where the empty slots are always visible. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A board shows 12 patch outlines. Small missions drift across the screen as simple one-tap prompts; completing each earns a patch that fills the corresponding outline. No timers, no grinding. Fill all 12 and the board is yours, complete.
- **Engine:** calm. **Circuits:** 06-zeigarnik, 08-investment, 05-ease.
- **Video trick:** calm completion — the board full, no empty outlines — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='patch-board'` → loop-events.js → juice.js → share.js.
...
```
# PATCH BOARD — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the board full, no empty outlines angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: patch-board" when finished.
