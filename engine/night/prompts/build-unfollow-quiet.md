...
1. `web/site/games/unfollow-quiet/index.html` — the game (single self-contained file)
2. `web/site/games/unfollow-quiet/copy.md` — social captions
...
## The game: UNFOLLOW QUIET

- **Conditioned path (the hook):** following list / mute-unfollow (the curation of who fills your feed, toward less noise). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A grid of noisy tiles (loud colors, buzzing labels) fills the screen; tap a tile to gently unfollow it and it fades to calm. The screen audibly quiets as you remove noise — no score, just the relief of a calmer field. Stop whenever it feels quiet enough.
- **Engine:** calm. **Circuits:** 05-ease, 03-social, 08-garden.
...
- Includes in order: `window.LOOP_GAME='unfollow-quiet'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: unfollow-quiet" when finished.
