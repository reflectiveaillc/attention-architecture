...
1. `web/site/games/tuning-match/index.html` — the game (single self-contained file)
2. `web/site/games/tuning-match/copy.md` — social captions
...
## The game: Tuning Match

- **Conditioned path (the hook):** audio / perfect-pitch ASMR rabbit holes that promise the satisfaction of true unison. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag a pitch slider until two tones cancel into silence. Your distance from perfect unison is scored in cents. PB is the lowest cents — a race toward zero without time pressure.
...
- Includes in order: `window.LOOP_GAME='tuning-match'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: tuning-match" when finished.
