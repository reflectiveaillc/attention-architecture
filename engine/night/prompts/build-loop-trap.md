...
1. `web/site/games/loop-trap/index.html` — the game (single self-contained file)
2. `web/site/games/loop-trap/copy.md` — social captions
...
- **Conditioned path (the hook):** seamless video loops. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A short animation repeats. A single frame is slightly different each loop. Tap when you spot the glitch before the loop resets. The seamless loop lulls System 2; the glitch hijacks novelty.
...
- Includes in order: `window.LOOP_GAME='loop-trap'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: loop-trap" when finished.
