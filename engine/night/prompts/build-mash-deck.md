...
1. `web/site/games/mash-deck/index.html` — the game (single self-contained file)
2. `web/site/games/mash-deck/copy.md` — social captions
...
- **Conditioned path (the hook):** the shuffled Discover feed — no two cards in a row are the same topic. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A shuffled deck of 20 micro-tasks, each a different 3s verb-action (tap 3, hold 1s, swipe left, avoid red, catch 2). Tap to draw the next card; clear the whole deck to win. Each card is a fresh mini-hit and the deck has a bottom — the novelty hit, then a stop.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 05-ease.
- **Video trick:** variable-reward roulette — you never know which card is next — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='mash-deck'` → loop-events.js → juice.js → share.js.
...
```
# MASH DECK — copy
## TikTok (3 captions ≤100 chars, hook-first, variable-reward roulette — you never know which card is next angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: mash-deck" when finished.
