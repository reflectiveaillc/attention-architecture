...
`engine/state/designs/cat-anxiety.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 20 game designs, ALL for the single category below.

READ FIRST:
1. `engine/state/categories.json` — find the category with `id: "anxiety"`. Internalize its trait, algo_hook, wean, and mechanics.
...
id: anxiety · label: Calm Control · engine_lean: calm
trait: seeks control and predictability to self-soothe; comforted by knowing what comes next
algo_hook: doomscroll as a coping mechanism — chasing reassurance in an unpredictable feed that keeps the nervous system activated
wean: a fully controllable, no-fail, predictable space that settles the nervous system — the soothe without the spike
mechanics: no-fail sandbox, predictable rhythm, breathing pace, gentle control, reversible actions
...
Write the JSON file now. Do not modify ANY other file. Output only "CATEGORY DONE: anxiety x20" when finished.
