...
1. `web/site/games/ripple-pond/index.html` — the game (single self-contained file)
2. `web/site/games/ripple-pond/copy.md` — social captions
...
- **Conditioned path (the hook):** water-ripple ASMR clips where each tap-drop spreads concentric rings and a resonant plop. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap anywhere on a still water surface to drop a ripple with a deep synthesized plop. Rings expand and overlap, playing harmonic tones when two collide. After twelve taps the pond slowly stills and dims, signaling the end of the session.
...
- Includes in order: `window.LOOP_GAME='ripple-pond'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: ripple-pond" when finished.
