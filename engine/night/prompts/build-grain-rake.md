...
1. `web/site/games/grain-rake/index.html` — the game (single self-contained file)
2. `web/site/games/grain-rake/copy.md` — social captions
...
- **Conditioned path (the hook):** zen-garden and sand-rake ASMR where drawing clean lines through granules gives a tactile reset. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Drag slowly through a field of procedural sand particles; a rake leaves parallel furrows with a soft bristle-on-grain audio. Crossing old furrows erases them. Fill the canvas with clean parallel lines to complete the garden and end the session.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 08-investment.
...
- Includes in order: `window.LOOP_GAME='grain-rake'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: grain-rake" when finished.
