...
1. `web/site/games/fyp-breath/index.html` — the game (single self-contained file)
2. `web/site/games/fyp-breath/copy.md` — social captions
...
- **Mechanic:** Cards drift by slowly. You do not swipe. After each card the screen holds a breath-prompt circle until it fades. The reward is the space between, not the next card.
...
- Includes in order: `window.LOOP_GAME='fyp-breath'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: fyp-breath" when finished.
