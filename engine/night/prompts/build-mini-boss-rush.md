...
1. `web/site/games/mini-boss-rush/index.html` — the game (single self-contained file)
2. `web/site/games/mini-boss-rush/copy.md` — social captions
...
## The game: MINI BOSS RUSH

- **Conditioned path (the hook):** viral 'final boss' trend chains — each clip escalates to a harder version of the last. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Eight mini-bosses, each a distinct 10s micro-challenge: dodge a spiral, tap a shrinking zone, hold through static, steer a falling shard. Beat one and the next slams in. The novelty is a brand-new boss every 10s — and beating the 8th prints FIN.
- **Engine:** viral. **Circuits:** 02-novelty, 01-rpe, 04-loss.
- **Video trick:** competitive rank — 'I got to boss 7, beat that' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='mini-boss-rush'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'bosses'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# MINI BOSS RUSH — copy
## TikTok (3 captions ≤100 chars, hook-first, competitive rank — 'I got to boss 7, beat that' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: mini-boss-rush" when finished.
