...
`engine/state/designs/batch-a.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 25 game designs.
...
FEED AND GESTURE MECHANICS: hijack the motor conditioning — infinite scroll, pull-to-refresh, swipe-up stories, autoplay-next, seamless video loops, double-tap-to-like, pinch/zoom, long-press. The thumb already knows these moves; your games make the move itself the game.
