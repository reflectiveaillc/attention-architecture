...
1. `web/site/games/pattern-vault/index.html` — the game (single self-contained file)
2. `web/site/games/pattern-vault/copy.md` — social captions
...
## The game: Pattern Vault

- **Conditioned path (the hook):** phone unlock pattern / lock-screen muscle memory. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Trace the hidden pattern across a grid of dots. Each correct unlock slides open a new secret panel; the third panel opens the vault and reveals the treasure inside.
- **Engine:** viral. **Circuits:** 06-zeigarnik, 01-rpe, 05-ease.
- **Video trick:** what's inside the vault — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='pattern-vault'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'panels'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Pattern Vault — copy
## TikTok (3 captions ≤100 chars, hook-first, what's inside the vault angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: pattern-vault" when finished.
