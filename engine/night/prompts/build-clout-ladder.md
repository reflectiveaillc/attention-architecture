...
1. `web/site/games/clout-ladder/index.html` — the game (single self-contained file)
2. `web/site/games/clout-ladder/copy.md` — social captions
...
## The game: Clout Ladder

- **Conditioned path (the hook):** rank / clout climbing loops. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A 10-rung ladder appears; each rung is a one-shot micro-challenge. Reach the top rung in one session, then dare a friend to knock you off in a single attempt. The ladder vanishes after the dare is settled.
...
- Includes in order: `window.LOOP_GAME='clout-ladder'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'rungs'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: clout-ladder" when finished.
