...
1. `web/site/games/memory-jar/index.html` — the game (single self-contained file)
2. `web/site/games/memory-jar/copy.md` — social captions
...
- **Conditioned path (the hook):** memory/archive feature (the 'save a memory' instinct, as gentle accumulation not recap anxiety). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Hold the screen and a glass jar slowly fills with soft glowing memory-motes rising from below; release and the motes settle gently to the bottom. Fill it at your own breath pace — there's no quota and the jar never overflows. Accumulation as rest.
...
- Includes in order: `window.LOOP_GAME='memory-jar'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: memory-jar" when finished.
