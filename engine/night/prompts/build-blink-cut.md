...
1. `web/site/games/blink-cut/index.html` — the game (single self-contained file)
2. `web/site/games/blink-cut/copy.md` — social captions
...
- **Conditioned path (the hook):** rapid-cut jump-scare edits that train the finger to flinch. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A strobe of cuts flashes on screen; tap the instant the "safe" color frame appears between jump-scare frames. Each round cuts faster and adds more decoy flashes.
...
- Includes in order: `window.LOOP_GAME='blink-cut'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: blink-cut" when finished.
