...
1. `web/site/games/rhythm-trace/index.html` — the game (single self-contained file)
2. `web/site/games/rhythm-trace/copy.md` — social captions
...
## The game: RHYTHM TRACE

- **Conditioned path (the hook):** the looping ASMR scroll where you replay the same soothing clip hoping the calm sticks this time. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A dotted line pulses across the screen in a slow, perfectly predictable wave. Trace it with your thumb — the line glows wherever you match it. There is no score, no gap penalty, no miss. The line repeats the same pattern indefinitely until you lift your thumb. Gentle control of a predictable rhythm.
- **Engine:** calm. **Circuits:** 05-ease, 07-timing, 01-rpe.
- **Video trick:** calm completion — the glowing trail you leave behind — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='rhythm-trace'` → loop-events.js → juice.js → share.js.
...
```
# RHYTHM TRACE — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the glowing trail you leave behind angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: rhythm-trace" when finished.
