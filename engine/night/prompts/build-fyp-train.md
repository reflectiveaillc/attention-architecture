...
1. `web/site/games/fyp-train/index.html` — the game (single self-contained file)
2. `web/site/games/fyp-train/copy.md` — social captions
...
- **Conditioned path (the hook):** FYP 'not interested' training (the long-press-to-dismiss feed-shaping god-complex). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Thumbnails stream in labeled with topic tags; long-press to mark 'not interested' and tap to keep. Your feed purity score rises as you weed out the off-topic ones — but misjudge and you nuke a good topic. Race to the cleanest FYP in 60s.
- **Engine:** viral. **Circuits:** 01-rpe, 02-novelty, 03-social.
- **Video trick:** algorithm god-complex (invented) — you are the algorithm, shape the feed — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='fyp-train'` → loop-events.js → juice.js → share.js.
...
```
# FYP TRAIN — copy
## TikTok (3 captions ≤100 chars, hook-first, algorithm god-complex (invented) — you are the algorithm, shape the feed angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: fyp-train" when finished.
