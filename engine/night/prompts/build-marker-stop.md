...
1. `web/site/games/marker-stop/index.html` — the game (single self-contained file)
2. `web/site/games/marker-stop/copy.md` — social captions
...
## The game: MARKER STOP

- **Conditioned path (the hook):** hype-edit progress bars and countdown sweepers. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A marker sweeps back and forth across a track; tap to stop it inside the target zone. The sweep accelerates and the target zone narrows every level.
- **Engine:** viral. **Circuits:** 01-rpe, 05-ease, 07-timing.
- **Video trick:** stop-the-marker tension — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='marker-stop'` → loop-events.js → juice.js → share.js.
...
```
# MARKER STOP — copy
## TikTok (3 captions ≤100 chars, hook-first, stop-the-marker tension angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: marker-stop" when finished.
