...
1. `web/site/games/beat-lock/index.html` — the game (single self-contained file)
2. `web/site/games/beat-lock/copy.md` — social captions
...
- **Conditioned path (the hook):** looping audio meme that hijacks the brain into replaying the same 15-second clip on repeat. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A glowing bar pulses left-to-right on a synthesized beat; tap once per pulse exactly as it peaks. Clean locks stack a visible combo multiplier. Miss one and the combo resets with a jarring off-beat flash. 32 beats, then it ends on the drop.
...
- Includes in order: `window.LOOP_GAME='beat-lock'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'combo'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: beat-lock" when finished.
