...
1. `web/site/games/crinkle-press/index.html` — the game (single self-contained file)
2. `web/site/games/crinkle-press/copy.md` — social captions
...
## The game: CRINKLE PRESS

- **Conditioned path (the hook):** crinkle-paper and foil-pressing ASMR where the slow compress-and-release of a surface is the sensory payload. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Press and slowly release anywhere on a procedural foil surface; it dents with a high-frequency crinkle audio scaled to press pressure. Lighter presses give delicate whisper crinkles; hard presses give a satisfying full crumple. Fifteen presses smooth the foil back flat and end the session.
...
- Includes in order: `window.LOOP_GAME='crinkle-press'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: crinkle-press" when finished.
