...
`engine/state/designs/batch-c.json` — a PURE JSON array (no markdown, no comments, no trailing commas) of exactly 25 game designs.
...
ANTICIPATION AND INTERMITTENCY: hijack the waiting brain — notification red badges, unread counts, FYP unpredictability (next-video roulette), streak anxiety, 'typing…' indicators, 24h expiry FOMO, refresh roulette, delayed rewards. The tension between trigger and payoff IS the game.
