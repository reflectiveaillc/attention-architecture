...
1. `web/site/games/ratio-king/index.html` — the game (single self-contained file)
2. `web/site/games/ratio-king/copy.md` — social captions
...
- **Conditioned path (the hook):** ratio dynamics (replies vs likes on a post). The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A post's like-counter and reply-counter both climb automatically; tap the reply counter to pump replies and flip replies > likes to 'win the ratio' before likes outpace you.
- **Engine:** viral. **Circuits:** 03-social, 01-rpe, 06-zeigarnik.
...
- Includes in order: `window.LOOP_GAME='ratio-king'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'ratio wins'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: ratio-king" when finished.
