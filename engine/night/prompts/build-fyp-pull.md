...
1. `web/site/games/fyp-pull/index.html` — the game (single self-contained file)
2. `web/site/games/fyp-pull/copy.md` — social captions
...
- **Conditioned path (the hook):** pull-to-refresh lever. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Pull down to load a new card. Each swipe either drops points, a dud, or a bomb. Stop after three duds or cash out. The lever becomes a slot machine you control with one thumb.
...
- Includes in order: `window.LOOP_GAME='fyp-pull'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: fyp-pull" when finished.
