...
1. `web/site/games/combo-sustain/index.html` — the game (single self-contained file)
2. `web/site/games/combo-sustain/copy.md` — social captions
...
## The game: COMBO SUSTAIN

- **Conditioned path (the hook):** rhythm-game combo-counter clips shared as status signals of flow mastery. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap on every beat of a 60-second track to sustain a combo counter. Each perfect tap adds to the multiplier; an early or late tap resets to zero. The session ends exactly at beat 60 — the wean is the track having a last beat.
- **Engine:** viral. **Circuits:** 01-rpe, 08-investment, 04-loss.
- **Video trick:** competitive rank — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='combo-sustain'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'max combo'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# COMBO SUSTAIN — copy
## TikTok (3 captions ≤100 chars, hook-first, competitive rank angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: combo-sustain" when finished.
