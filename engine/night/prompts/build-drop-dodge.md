...
1. `web/site/games/drop-dodge/index.html` — the game (single self-contained file)
2. `web/site/games/drop-dodge/copy.md` — social captions
...
- **Conditioned path (the hook):** vertical falling-object videos that demand a side-step reaction. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Objects drop from the top of the feed; tap left or right to dodge the falling hazards. Speed and density ramp up until the 60-second burst ends.
- **Engine:** viral. **Circuits:** 01-rpe, 04-loss, 05-ease.
- **Video trick:** react-or-die hook — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='drop-dodge'` → loop-events.js → juice.js → share.js.
...
```
# DROP DODGE — copy
## TikTok (3 captions ≤100 chars, hook-first, react-or-die hook angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: drop-dodge" when finished.
