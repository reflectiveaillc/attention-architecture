...
1. `web/site/games/drop-catch/index.html` — the game (single self-contained file)
2. `web/site/games/drop-catch/copy.md` — social captions
...
- **Conditioned path (the hook):** bass-drop audio memes that condition anticipation of the exact moment the beat hits. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** The screen counts down with rising visual noise; tap the exact frame the drop lands. Hold too long before or after and it registers as a miss. Score is deviation in milliseconds from the drop — share your ms and dare a friend.
...
- Includes in order: `window.LOOP_GAME='drop-catch'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: drop-catch" when finished.
