...
1. `web/site/games/garden-grow/index.html` — the game (single self-contained file)
2. `web/site/games/garden-grow/copy.md` — social captions
...
## The game: GARDEN GROW

- **Conditioned path (the hook):** plant-collection and seed-haul videos that always tease the next species. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** Tap soil patches to plant seeds; each tap costs one water drop from a finite daily jar. Seeds sprout into one of 10 plant species displayed on a completion bar. When all 10 species are grown the garden is full and the session closes peacefully.
- **Engine:** calm. **Circuits:** 08-investment, 06-zeigarnik, 05-ease.
- **Video trick:** calm completion — the garden finished, nothing more to add — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='garden-grow'` → loop-events.js → juice.js → share.js.
...
```
# GARDEN GROW — copy
## TikTok (3 captions ≤100 chars, hook-first, calm completion — the garden finished, nothing more to add angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: garden-grow" when finished.
