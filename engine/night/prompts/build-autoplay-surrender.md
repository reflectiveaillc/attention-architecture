...
1. `web/site/games/autoplay-surrender/index.html` — the game (single self-contained file)
2. `web/site/games/autoplay-surrender/copy.md` — social captions
...
## The game: Autoplay Surrender

- **Conditioned path (the hook):** the autoplay countdown bar that dares you to let the next video load. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A countdown bar sweeps toward the next 'video' like the autoplay timer. The move is to NOT tap stop — let each countdown run out, and each one dims the screen. Score is how many autoplays you surrender before the screen goes black. The discipline is letting the countdown win so you can lose the feed.
- **Engine:** viral. **Circuits:** 05-ease, 04-loss, 07-timing.
- **Video trick:** anticipation agony — endure the countdown instead of dodging it — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='autoplay-surrender'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'surrendered'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# Autoplay Surrender — copy
## TikTok (3 captions ≤100 chars, hook-first, anticipation agony — endure the countdown instead of dodging it angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: autoplay-surrender" when finished.
