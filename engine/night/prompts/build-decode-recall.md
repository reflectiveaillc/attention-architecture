...
1. `web/site/games/decode-recall/index.html` — the game (single self-contained file)
2. `web/site/games/decode-recall/copy.md` — social captions
...
## The game: DECODE RECALL

- **Conditioned path (the hook):** cryptic-clue / code-recall viral challenges that prove memory under pressure. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A symbol-to-letter cipher is shown for 10 seconds; it hides and a word appears in symbols. Decode it from memory by tapping the letters. Longer words and more cipher entries ramp the working-memory load each round.
- **Engine:** viral. **Circuits:** 01-rpe, 06-zeigarnik, 08-investment.
- **Video trick:** anticipation agony — 'can you hold the cipher long enough to crack the word?' — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='decode-recall'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'words'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
```
# DECODE RECALL — copy
## TikTok (3 captions ≤100 chars, hook-first, anticipation agony — 'can you hold the cipher long enough to crack the word?' angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: decode-recall" when finished.
