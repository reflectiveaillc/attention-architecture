...
1. `web/site/games/frame-catch/index.html` — the game (single self-contained file)
2. `web/site/games/frame-catch/copy.md` — social captions
...
## The game: FRAME CATCH

- **Conditioned path (the hook):** rapid-cut transition edits that hide the one perfect frame. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A montage cycles at high speed; tap when the target shape briefly matches the ghost outline. The cuts accelerate and the match window shrinks to a single frame.
...
- Includes in order: `window.LOOP_GAME='frame-catch'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'frames'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: frame-catch" when finished.
