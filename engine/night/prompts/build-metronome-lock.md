...
1. `web/site/games/metronome-lock/index.html` — the game (single self-contained file)
2. `web/site/games/metronome-lock/copy.md` — social captions
...
## The game: Metronome Lock

- **Conditioned path (the hook):** music / rhythm perfect-play covers where one missed beat breaks the flawless run. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A metronome ticks; tap exactly on each beat. Drift is shown in milliseconds and the track ends after 32 bars, giving a clean PB time to chase on every replay.
...
- Includes in order: `window.LOOP_GAME='metronome-lock'` → loop-events.js → juice.js → share.js.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: metronome-lock" when finished.
