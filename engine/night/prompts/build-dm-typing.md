...
1. `web/site/games/dm-typing/index.html` — the game (single self-contained file)
2. `web/site/games/dm-typing/copy.md` — social captions
...
- **Mechanic:** A three-dot typing indicator bobs. Tap when the dots stop and the message 'sends'. The pause length is random; the compulsion is the same held breath you feel in a chat window.
...
- Includes in order: `window.LOOP_GAME='dm-typing'` → loop-events.js → juice.js → share.js.
...
- Dares: score card shows "⚡ DARE A FRIEND" → SHARE.send({score, unit: 'msgs'}); incoming ?dare= shows "BEAT <n>" banner + SHARE.beaten() when passed.
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: dm-typing" when finished.
