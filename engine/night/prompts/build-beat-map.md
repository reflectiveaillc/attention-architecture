...
1. `web/site/games/beat-map/index.html` — the game (single self-contained file)
2. `web/site/games/beat-map/copy.md` — social captions
...
- **Conditioned path (the hook):** music-structure rabbit-holes — the deep-dives into song construction, verse-chorus maps, and time-signature analysis. The player's thumb already knows this move from Instagram/TikTok — the game IS that move.
- **Mechanic:** A song structure map loads as empty bars. Listen to a short audio loop and tap each bar to label it (intro/verse/chorus/bridge/outro) based on what you hear. Correct labels snap with a satisfying tick; mismatches push back. Map the full structure and the song plays through once more, labelled.
- **Engine:** calm. **Circuits:** 01-rpe, 06-zeigarnik, 05-ease.
- **Video trick:** completion-satisfaction — the map that explains the song — the demo/hook clip must land this.
...
- Includes in order: `window.LOOP_GAME='beat-map'` → loop-events.js → juice.js → share.js.
...
```
# BEAT MAP — copy
## TikTok (3 captions ≤100 chars, hook-first, completion-satisfaction — the map that explains the song angle)
## Reels (3)
## Shorts titles (3)
## Hashtags (5)
```
...
Write both files now. Do not modify ANY other file. No build steps. Output "BUILD DONE: beat-map" when finished.
