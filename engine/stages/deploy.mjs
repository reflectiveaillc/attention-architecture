...
<footer>
  <nav style="display:block;max-width:960px;margin:0 auto 14px;line-height:2" aria-label="Game collections">
    <a href="/daily" style="color:var(--gold);margin:0 8px;white-space:nowrap;font-weight:700">🔥 Daily Tilt</a>
    ${TESTS.map((x) => `<a href="/test/${x.slug}" style="color:rgba(255,255,255,.45);margin:0 8px;white-space:nowrap">${x.h1}</a>`).join('\n    ')}
...
(function () {
  ...
  // ---------- distribution: daily ritual + benchmark test pages ----------
  renderDaily(siteDir, registry);
  renderTests(siteDir, registry);

  renderSitemapAndRobots(siteDir, registry);
}

// ---------------- Daily Tilt (Wordle ritual mechanics) ----------------
function renderDaily(siteDir, registry) {
  // curated pool: viral games with a concrete challenge, stable deterministic order
  const pool = registry.games
    .filter((g) => g.engine === 'viral' && g.challenge)
    .map((g) => ({ id: g.id, name: g.name, tagline: g.tagline || '', challenge: g.challenge }))
    .sort((a, b) => a.id.localeCompare(b.id));
  // shuffle with a fixed LCG seed so the order is stable across builds
  let s = 1337;
  ...
  // Daily #1 = 2026-07-22 (UTC)
  const DAY0 = Math.floor(Date.UTC(2026, 6, 22) / 864e5) - 1;
  const title = 'Daily Tilt — Today’s Game. One Challenge. Everyone Plays the Same.';
  const desc = 'One free browser game per day — the same for everyone. Play it, share your emoji result, keep your streak alive. New game at midnight UTC.';
  const ld = [faqLd([
    { q: 'What is Daily Tilt?', a: 'One mini game per day, the same for every player worldwide. Play it, share your spoiler-free emoji result, and keep your streak going. A new game unlocks at midnight UTC.' },
    { q: 'Is Daily Tilt free?', a: 'Yes — free in your browser, no download, no account. Your streak is stored on your device.' }
  ])];
  const page = `<!DOCTYPE html>
<!-- GENERATED by engine/stages/deploy.mjs -->
<html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
<title>${title}</title>${metaHead({ path: '/daily', title, desc, ld })}
<style>${CSS}
  .wrap { max-width:520px; margin:0 auto; text-align:center; padding:max(40px,env(safe-area-inset-top)) 20px 60px; }
  .kicker { font-size:13px; letter-spacing:2px; text-transform:uppercase; color:var(--gold); font-weight:800; }
  h1 { font-size:34px; font-weight:900; margin:10px 0 4px; letter-spacing:-1px; }
  .gamename { font-size:26px; font-weight:900; margin:26px 0 4px; color:var(--verm); }
  .tagline { color:rgba(255,255,255,.55); font-size:14px; margin:0 0 6px; }
  .dare { font-size:15px; font-weight:800; color:var(--gold); }
  .play { display:block; background:var(--verm); color:#fff; font-size:20px; font-weight:900; padding:17px; border-radius:14px; margin:22px auto 12px; max-width:340px; }
  .play:active { transform:scale(.97); }
  .streak { font-size:15px; color:rgba(255,255,255,.6); margin-top:16px; }
  .streak b { color:#fff; }
  .count { font-size:12px; color:rgba(255,255,255,.35); margin-top:26px; }
  .back { display:inline-block; color:rgba(255,255,255,.5); font-size:14px; font-weight:700; margin-top:30px; }
</style></head><body>
<div class="wrap">
  <div class="kicker">Daily Tilt</div>
  <h1 id="dnum">#…</h1>
  <div class="gamename" id="gname">…</div>
  <p class="tagline" id="gtag"></p>
  <div class="dare" id="gdare"></div>
  <a class="play" id="playbtn" href="#">▶&nbsp; PLAY TODAY’S GAME</a>
  <div class="streak" id="streak"></div>
  <div class="count">same game for everyone · new one at midnight UTC · ${pool.length} in rotation</div>
  <a class="back" href="/">← all games</a>
</div>
<script>
  var POOL = ${JSON.stringify(pool)};
  var DAY0 = ${DAY0};
  var n = Math.floor(Date.now() / 864e5) - DAY0;
  var g = POOL[((n - 1) % POOL.length + POOL.length) % POOL.length];
  document.getElementById('dnum').textContent = '#' + n;
  document.getElementById('gname').textContent = g.name;
  document.getElementById('gtag').textContent = g.tagline;
  document.getElementById('gdare').textContent = g.challenge ? ('today’s dare: ' + g.challenge) : '';
  var btn = document.getElementById('playbtn');
  btn.href = '/games/' + g.id + '/?daily=' + n + '&src=daily';
  var st = {};
  try { st = JSON.parse(localStorage.getItem('loop_daily') || '{}'); } catch (e) {}
  var streak = (st.n === n) ? st.streak : (st.n === n - 1 ? st.streak : 0);
  if (streak > 0) document.getElementById('streak').innerHTML = '🔥 <b>' + streak + ' day' + (streak > 1 ? 's' : '') + '</b> streak' + (st.n === n ? ' — played today' : ' — play to keep it');
  btn.addEventListener('click', function () {
    var next = (st.n === n) ? st.streak : ((st.n === n - 1 ? (st.streak || 0) : 0) + 1);
    try { localStorage.setItem('loop_daily', JSON.stringify({ n: n, streak: next })); } catch (e) {}
  });
</script>
<script src="/js/loop-events.js"></script>
</body></html>`;
  fs.writeFileSync(path.join(siteDir, 'daily.html'), page);
}

// ---------------- /test/ benchmark vertical (humanbenchmark play) ----------------
const TESTS = [
  { slug: 'reaction-time', game: 'reflex-duel',
    title: 'Reaction Time Test — Free, Instant, Millisecond Score',
    h1: 'Reaction Time Test',
    desc: 'Test your reaction time free: F1-style start lights, millisecond scoring, instant browser test. No download — get your number in 10 seconds.',
    intro: 'When the lights go out, tap. That’s the whole test — and it measures you to the millisecond, the same way Formula 1 measures race starts. Most people land around 250ms on a visual cue; trained athletes and top gamers live below 200. Touchscreens add a little latency, so compare against yourself on the same device.',
    stats: [['Average adult (visual)', '~250 ms'], ['Excellent', 'under 200 ms'], ['F1 driver territory', '~150–200 ms'], ['Your score', 'one tap away']],
    faqs: [
      { q: 'What is the average reaction time?', a: 'Around 250 milliseconds for a visual stimulus. Under 200ms is excellent. Audio cues are typically 20-40ms faster than visual ones.' },
      { q: 'Is this reaction time test free?', a: 'Yes — it runs instantly in your browser with no download or sign-up, and you can retry as many times as you like.' },
      { q: 'Why is my reaction time slower on my phone?', a: 'Touchscreens add input latency compared to a mouse or keyboard. For a fair benchmark, compare runs on the same device.' }] },
  { slug: 'draw-a-perfect-circle', game: 'perfect-circle',
    title: 'Draw a Perfect Circle Test — Get Judged Instantly (Free)',
    h1: 'Draw a Perfect Circle',
    desc: 'Can you draw a perfect circle? Draw one freehand and get graded as a percentage instantly. Free in your browser — most people score in the 80s.',
    intro: 'Freehand circles are famously humbling: your brain knows exactly what a circle is and your hand refuses to cooperate. Draw one and get graded to the decimal, instantly. Most people land in the 80s. Above 95% is genuinely rare. Slow, constant speed, and drawing from the shoulder — not the wrist — is the trick.',
    stats: [['Typical score', '80–90%'], ['Genuinely rare', 'above 95%'], ['The dare in this one', 'beat 89.0%'], ['Attempts it usually takes', 'more than you think']],
    faqs: [
      { q: 'How do you draw a perfect circle by hand?', a: 'Constant speed matters more than accuracy — go slow and steady, pivot from the shoulder rather than the wrist, and don’t stop mid-stroke. The grader scores roundness as a percentage instantly.' },
      { q: 'Is a 90% circle good?', a: 'Yes — most people score in the 80s. Above 95% is rare enough that it’s worth a screenshot.' }] },
  { slug: 'stop-at-10-seconds', game: 'ten-seconds',
    title: 'Stop the Timer at Exactly 10 Seconds — Precision Test',
    h1: 'Stop at Exactly 10.00 Seconds',
    desc: 'Stop the clock at exactly 10.00 seconds. Sounds easy — the last digits are hidden. A free precision timing test that shows how close you really came.',
    intro: 'Start the clock, feel ten seconds pass, stop it at exactly 10.00. The catch: the final digits hide while it runs, so you’re navigating on your internal clock alone. Seeing 9.98 on the result screen is the most “one more try” feeling we know. Perfect 10.00s do happen — rarely.',
    stats: [['Counts as sharp', 'within ±0.15s'], ['Genuinely impressive', 'within ±0.05s'], ['The perfect run', 'exactly 10.00'], ['What it tests', 'your internal clock']],
    faqs: [
      { q: 'How accurate is the human internal clock?', a: 'Over a 10-second span, most people land within a few tenths of a second once they’ve calibrated with a couple of tries. Within ±0.05s is genuinely sharp.' },
      { q: 'Any trick to stopping at exactly 10 seconds?', a: 'Count with a steady anchor — many people use “one-one-thousand” pacing or a song they know at 60 BPM — and commit to the tap instead of hesitating at the end.' }] },
  { slug: 'scroll-speed', game: 'scroll-sprint',
    title: 'Scroll Speed Test — How Far Can Your Thumb Go in 10 Seconds?',
    h1: 'Scroll Speed Test',
    desc: 'Measure your doomscrolling in feet: how far can your thumb scroll in 10 seconds? A free, slightly uncomfortable speed test for your scrolling hand.',
    intro: 'You scroll all day — but how far, exactly? This test measures the physical distance your thumb covers in ten seconds, in feet and inches. It started as a joke about doomscrolling and became weirdly competitive. Your daily scroll distance would frighten you; at least here it counts as a score.',
    stats: [['Casual scroller', '20–30 ft / 10s'], ['The dare', 'beat 41.2 ft'], ['Unit of measurement', 'real feet + inches'], ['Side effect', 'mild self-awareness']],
    faqs: [
      { q: 'How far does the average person scroll per day?', a: 'Widely cited estimates put daily phone scrolling around the height of a building — this test measures your 10-second sprint pace in feet so you can do the scary math yourself.' },
      { q: 'Does scroll speed matter?', a: 'As a benchmark it’s pure fun — but seeing your speed in physical feet is a surprisingly effective way to notice how automatic scrolling has become.' }] },
  { slug: 'number-memory', game: 'number-vault',
    title: 'Number Memory Test — How Many Digits Can You Hold?',
    h1: 'Number Memory Test',
    desc: 'Test your short-term memory: memorize a number, type it back, one digit longer each round. Free digit-span test — the classic limit is 7±2.',
    intro: 'A number flashes, you type it back, and each round adds a digit. This is the classic digit-span test behind the famous “seven, plus or minus two” finding — most people’s short-term memory holds 5 to 9 items. The test finds your exact ceiling in about two minutes, and the score is very shareable.',
    stats: [['Classic human range', '5–9 digits'], ['The famous finding', '7 ± 2'], ['Strong score', '10+ digits'], ['Time to find your limit', '~2 minutes']],
    faqs: [
      { q: 'How many digits can the average person remember?', a: 'The classic research answer is seven plus or minus two — most people reliably hold 5 to 9 digits in short-term memory. Chunking (grouping digits) is how people push past it.' },
      { q: 'Can I improve my digit span?', a: 'Practice improves your score at the test itself, mostly through chunking strategies. Broader memory transfer is debated — but climbing your own ceiling is the fun part.' }] }
];

function renderTests(siteDir, registry) {
  const base = siteBase();
  const dir = path.join(siteDir, 'test');
  fs.mkdirSync(dir, { recursive: true });
  const byId = Object.fromEntries(registry.games.map((g) => [g.id, g]));
  for (const t of TESTS) {
    const g = byId[t.game];
    if (!g) continue;
    const others = TESTS.filter((x) => x.slug !== t.slug);
    const ld = [
      { '@context': 'https://schema.org', '@type': 'WebApplication', name: t.h1,
        url: `${base}/test/${t.slug}`, applicationCategory: 'Game', operatingSystem: 'Any',
        description: t.desc, offers: { '@type': 'Offer', price: '0', priceCurrency: 'USD' } },
      faqLd(t.faqs)
    ];
    const rows = t.stats.map(([k, v]) => `<tr><td>${k}</td><td><b>${v}</b></td></tr>`).join('\n    ');
    const page = `<!DOCTYPE html>
<!-- GENERATED by engine/stages/deploy.mjs -->
<html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
<title>${t.title}</title>${metaHead({ path: `/test/${t.slug}`, title: t.title, desc: t.desc, ld })}
<style>${CSS}
  .wrap { max-width:560px; margin:0 auto; padding:max(28px,env(safe-area-inset-top)) 20px 60px; }
  .back { display:inline-block; color:rgba(255,255,255,.5); font-size:14px; font-weight:700; padding:6px 0; }
  .kicker { font-size:12px; letter-spacing:2px; text-transform:uppercase; color:var(--gold); font-weight:800; margin-top:10px; }
  h1 { font-size:32px; font-weight:900; margin:6px 0 12px; letter-spacing:-.5px; }
  .intro { color:rgba(255,255,255,.6); font-size:15px; line-height:1.65; margin:0 0 20px; }
  .start { display:block; text-align:center; background:var(--verm); color:#fff; font-size:20px; font-weight:900; padding:17px; border-radius:14px; margin:0 0 22px; }
  .start:active { transform:scale(.97); }
  table { width:100%; border-collapse:collapse; background:var(--card); border-radius:12px; overflow:hidden; }
  td { padding:11px 14px; font-size:14px; border-bottom:1px solid rgba(255,255,255,.06); color:rgba(255,255,255,.7); }
  td b { color:#fff; }
  h3 { font-size:13px; letter-spacing:1px; color:rgba(255,255,255,.4); margin:28px 0 10px; text-transform:uppercase; }
...
<div class="wrap">
  <a class="back" href="/">← all games</a>
  <div class="kicker">free instant test</div>
  <h1>${t.h1}</h1>
  <p class="intro">${t.intro}</p>
  <a class="start" href="/games/${t.game}/?src=test-${t.slug}">▶&nbsp; START THE TEST</a>
  <table>
    ${rows}
  </table>
  <h3>Questions</h3>
  ${t.faqs.map((f) => `<details class="faq"><summary>${f.q}</summary><p>${f.a}</p></details>`).join('\n  ')}
  <h3>More tests</h3>
  <p class="others">${others.map((o) => `<a href="/test/${o.slug}">${o.h1}</a>`).join('\n  ')}<a href="/best/reaction-time-games">All reaction games</a> <a href="/best/memory-test-games">All memory games</a></p>
</div>
<script src="/js/loop-events.js"></script>
</body></html>`;
...
function renderSitemapAndRobots(siteDir, registry) {
  ...
  const urls = [
    ...
    { loc: `${base}/daily`, priority: '0.9' },
    { loc: `${base}/press`, priority: '0.5' },
    { loc: `${base}/creators`, priority: '0.5' },
    ...TESTS.map((t) => ({ loc: `${base}/test/${t.slug}`, priority: '0.9' })),
