...
function metaHead({ path = '/', title = 'Tilt — tiny games, honest hooks', desc = '383 tiny games engineered for attention. Play one. Loop forever.', includeAnalytics = true, ld = [] } = {}) {
  const base = siteBase();
...
<title>Viral Free Games — ${registry.games.length} Tiny Browser Games, No Download</title>${metaHead({
    path: '/',
    title: `Viral Free Games — ${registry.games.length} Tiny Browser Games, No Download`,
    desc: `${registry.games.length} free online mini games that load instantly: reaction tests, calming games, fidget games, face-controlled games and more. No download, no sign-up.`,
    ld: [{
      '@context': 'https://schema.org', '@type': 'WebSite',
      name: 'Viral Free Games', alternateName: 'Tilt', url: siteBase()
    }]
  })}
<style>${CSS}
...
<footer>
  <nav style="display:block;max-width:960px;margin:0 auto 14px;line-height:2" aria-label="Game collections">
    ${COLLECTIONS.map((c) => `<a href="/best/${c.slug}" style="color:rgba(255,255,255,.45);margin:0 8px;white-space:nowrap">${c.h1}</a>`).join('\n    ')}
  </nav>
...
(function () {
  ...
  // ---------- SEO: intent collections + sitemap + robots ----------
  renderCollections(siteDir, registry);
  renderSitemapAndRobots(siteDir, registry);
}

function renderCollections(siteDir, registry) {
  const base = siteBase();
  const dir = path.join(siteDir, 'best');
    ...
    const ld = [
      {
        ...
        name: c.h1, url: `${base}/best/${c.slug}`, description: c.desc,
        mainEntity: {
          '@type': 'ItemList',
...
<html lang="en"><head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, viewport-fit=cover">
<title>${c.title}</title>${metaHead({ path: `/best/${c.slug}`, title: c.title, desc: c.desc, ld })}
<style>${CSS}
...
<div class="wrap">
  <a class="back" href="/">← all ${registry.games.length} games</a>
  <h1>${c.h1}</h1>
...
${cards}
  ...
  <p class="others">${others.map((o) => `<a href="/best/${o.slug}">${o.h1}</a>`).join('\n  ')}</p>
</div>
<script>
  var io = new IntersectionObserver(function (es) {
    es.forEach(function (en) {
...
function renderSitemapAndRobots(siteDir, registry) {
  ...
  const urls = [
    ...
    ...COLLECTIONS.filter((c) => registry.games.some(c.match))
      .map((c) => ({ loc: `${base}/best/${c.slug}`, priority: '0.8' })),
