...
| 2026-07-20 | **External deploy gate OPENED per Manuel**: Tilt site live on Vercel (`attention-architecture.vercel.app`), git repo `reflectiveaillc/attention-architecture`, Git integration connected, dashboard password-protected, Vercel Web Analytics script wired, Hotjar loader ready (needs site ID) | the 1.4GB of local hook clips deployed via CLI; future git-based deploys won't include clips unless they're committed or moved to external storage |
| 2026-07-20 | **Experiment driver shipped**: `engine/lib/experiment.mjs` + `node engine/loop.mjs experiment` CLI + deterministic variant assignment + synthetic cohort A/B split + dashboard experiments panel | every game can now breed A/B variants from its own P/D/A index profile |
